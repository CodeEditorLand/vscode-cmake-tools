...
export class EnvironmentUtils {
	public static create(
		from?: Map<string, string> | EnvironmentWithNull | null,
		options?: EnvironmentOptions,
	): Environment {
		const env = new EnvironmentPrivate(options);
		const p = new Proxy(env, {
			defineProperty: (target, p, attributes) =>
				Reflect.defineProperty(target[envProperty], p, attributes),
			deleteProperty: (target, p) =>
				Reflect.deleteProperty(target[envProperty], p),
			get: (target, p) => {
				if (typeof p === "string") {
					return target.get(p);
				}
	...
	public static createPreserveNull(
		from?: Map<string, string> | EnvironmentWithNull | null,
	): EnvironmentWithNull {
	...
	public static merge(
		envs: (EnvironmentWithNull | null | undefined)[],
		options?: EnvironmentOptions,
	): Environment {
	...
	public static mergePreserveNull(
		envs: (EnvironmentWithNull | null | undefined)[],
	): EnvironmentWithNull {
