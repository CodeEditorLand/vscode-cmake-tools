...
import {
	getExtensionActiveCommands,
	getExtensionLocalizedStrings,
	onExtensionActiveCommandsChanged,
} from "./extension";
...
export class PinnedCommands {
	...
	constructor(
		...
		this.treeDataProvider = new PinnedCommandsTreeDataProvider(
			configReader,
			extensionContext,
		);
	...
	async showPinnableCommands(): Promise<PinnedCommandsQuickPickItem | null> {
		...
		if (!chosenItem) {
			log.debug(
				localize(
					"user.cancelled.add.pinned.cmake.command",
	...
	implements vscode.TreeDataProvider<PinnedCommandNode>, vscode.Disposable
{
	...
	constructor(
		...
		this.pinDefaultTasks =
			this.extensionContext.globalState.get(mementoKey) === undefined; // the user has not unpinned any of the tasks commands yet.
		onExtensionActiveCommandsChanged(this.doConfigureSettingsChange, this);
	}

	get onDidChangeTreeData(): vscode.Event<
		PinnedCommandNode | void | undefined
	> {
	...
	async initialize(): Promise<void> {
			...
			for (const commandName of settingsPinnedCommands) {
				...
				if (this.findNode(label) === -1) {
					...
					this.pinnedCommands.push(
						new PinnedCommandNode(
							label,
							commandName,
							activeCommands.has(commandName),
						),
					);
				}
			}
		...
		if (this.pinDefaultTasks) {
			if (
				this.pinnedCommands.filter((x) =>
					defaultTaskCommands.includes(x.commandName),
				).length !== defaultTaskCommands.length
			) {
	...
	async addCommand(chosen: PinnedCommandsQuickPickItem) {
		...
		if (this.findNode(chosen.label) === -1) {
			const node = new PinnedCommandNode(
				chosen.label,
				chosen.command,
				true,
			);
	...
	async removeCommand(node: PinnedCommandNode) {
		...
		if (
			this.pinDefaultTasks &&
			defaultTaskCommands.includes(node.commandName)
		) {
	...
	async updateSettings() {
		if (this.config) {
			const newValue: string[] = this.pinnedCommands.map(
				(x) => x.commandName,
			);
