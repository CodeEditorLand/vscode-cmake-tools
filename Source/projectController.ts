...
import { getStatusBar, setContextAndStore } from "@cmt/extension";
...
export class ProjectController implements vscode.Disposable {
	...
	async updateActiveProject(
		...
		const projects: CMakeProject[] | undefined =
			this.getProjectsForWorkspaceFolder(workspaceFolder);
			...
			if (openEditor) {
				...
				if (!this.activeProject) {
					if (
						util.isFileInsideFolder(
							openEditor.document.uri,
							projects[0].workspaceFolder.uri.fsPath,
						)
					) {
		...
		this.subscriptions = [
			vscode.workspace.onDidChangeWorkspaceFolders((e) =>
				rollbar.invokeAsync(
					...
					() => this.doWorkspaceFolderChange(e),
				),
			),
			vscode.workspace.onDidOpenTextDocument(
				(textDocument: vscode.TextDocument) =>
					this.doOpenTextDocument(textDocument),
			),
			vscode.workspace.onDidSaveTextDocument(
				(textDocument: vscode.TextDocument) =>
					this.doSaveTextDocument(textDocument),
			),
	...
	getProjectsForWorkspaceFolder(
		ws: vscode.WorkspaceFolder | string[] | undefined,
	...
	async getProjectForFolder(
		folder: string,
	): Promise<CMakeProject | undefined> {
		const sourceDir = util.platformNormalizePath(
			await util.normalizeAndVerifySourceDir(
				folder,
				CMakeDriver.sourceDirExpansionOptions(folder),
			),
		);
		...
		for (const project of allCMakeProjects) {
			if (
				util.platformNormalizePath(project.sourceDir) === sourceDir ||
	 ...
	 */
	public static async createCMakeProjectsForWorkspaceFolder(
		workspaceContext: DirectoryContext,
		projectController?: ProjectController,
	): Promise<CMakeProject[]> {
		const sourceDirectories: string[] =
			workspaceContext.config.sourceDirectory;
	...
	private static async checkBuildDirectories(
		...
		if (
			unresolvedBuildDirectory.includes("${sourceDirectory}") ||
			unresolvedBuildDirectory.includes("${sourceDir}")
		) {
			return;
		} else {
			const sameBinaryDir = localize(
				"duplicate.build.directory.1",
				"Multiple CMake projects in this folder are using the same CMAKE_BINARY_DIR.",
			);

			const mayCauseProblems = localize(
			...
			if (!ProjectController.duplicateMessageShown) {
				...
				void vscode.window
					.showInformationMessage(
						`${sameBinaryDir} ${mayCauseProblems}`,
						moreInfo,
					)
					.then((response) => {
	...
	public useCMakePresetsForFolder(folder: vscode.WorkspaceFolder): boolean {
		const cmakeProjects: CMakeProject[] | undefined =
	...
	private async addFolder(
		...
		let projects: CMakeProject[] | undefined =
			this.getProjectsForWorkspaceFolder(folder);
		...
		if (projects) {
			...
			if (config) {
				this.sourceDirectorySub.set(
					folder,
					config.onChange(
						"sourceDirectory",
						async (sourceDirectories: string | string[]) =>
							this.doSourceDirectoryChange(
								folder,
								sourceDirectories,
								config.options,
							),
				...
				this.useCMakePresetsSub.set(
					...
					config.onChange(
						"useCMakePresets",
						async (useCMakePresets: string) =>
							this.doUseCMakePresetsChange(
								folder,
								useCMakePresets,
							),
					),
				...
				this.hideDebugButtonSub.set(
					...
					config.onChange("options", async (options: OptionConfig) =>
						this.doStatusChange(folder, options),
					),
				);
			}
		}
	...
	private async doSourceDirectoryChange(
		folder: vscode.WorkspaceFolder,
		value: string | string[],
		options: OptionConfig,
	) {
		...
		if (projects) {
			...
			const multiProjectChange: boolean =
				sourceDirectories.length > 1 !== projects.length > 1;

			// Remove projects.
			for (let i = projects.length - 1; i >= 0; i--) {
				if (
					!multiProjectChange &&
					sourceDirectories.indexOf(projects[i].sourceDir) !== -1
				) {
					sourceDirectories.splice(
						sourceDirectories.indexOf(projects[i].sourceDir),
						1,
					);
				} else {
					if (
						this.activeProject?.sourceDir === projects[i].sourceDir
					) {
			...
			for (let i = 0; i < sourceDirectories.length; i++) {
				const cmakeProject: CMakeProject = await CMakeProject.create(
					workspaceContext,
					sourceDirectories[i],
					this,
					sourceDirectories.length > 1,
	...
	private async refreshDriverSettings(
		config: ConfigurationReader,
		folder: vscode.WorkspaceFolder,
	) {
		const projects: CMakeProject[] | undefined =
	...
	private async doUseCMakePresetsChange(
		folder: vscode.WorkspaceFolder,
	 ...
	 */
	private async doWorkspaceFolderChange(
	...
	private async doSaveTextDocument(
		textDocument: vscode.TextDocument,
	): Promise<void> {
		await this.doCMakeFileChangeReconfigure(textDocument.uri);
	}

	private async onDidRenameFiles(
		renamedFileEvt: vscode.FileRenameEvent,
	): Promise<void> {
	...
	private async doCMakeFileChangeReconfigure(uri: vscode.Uri): Promise<void> {
		const activeProject: CMakeProject | undefined =
			this.getActiveCMakeProject();

		if (activeProject) {
			const isFileInsideActiveProject: boolean = util.isFileInsideFolder(
				uri,
				activeProject.isMultiProjectFolder
					? activeProject.folderPath
					: activeProject.workspaceFolder.uri.fsPath,
			);
