...
export class CPackDriver implements vscode.Disposable {
	...
	private async getCPackArgs(
		...
		if (packagePreset.variables) {
			for (const varName in packagePreset.variables) {
				cpackArgs.push(
					`-D ${varName}=${packagePreset.variables[varName]}`,
				);
			}
		}
	...
	public async runCPack(
		...
		const cpackpath = await this.ws.getCPackPath(
		...
		if (driver.useCMakePresets && !driver.packagePreset) {
			log.error(
				localize("package.preset.not.set", "Package preset is not set"),
			);
			...
			const configs: string =
				packagePreset?.configurations?.join(";") || configuration;
		...
		const child = driver.executeCommand(
			...
			{
				environment: await driver.getCPackCommandEnvironment(),
		...
		if (res.retc === null) {
			log.info(
				localize("cpack.run.terminated", "CPack run was terminated"),
			);
			...
			log.info(
				localize(
					"cpack.finished.with.code",
					"CPack finished with return code {0}",
					res.retc,
				),
			);
		}
