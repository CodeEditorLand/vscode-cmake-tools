...
async function getDefaultPresetName(
	...
	switch (commandType) {
		...
		case CommandType.workflow:
			result = resolve
				? ((await vscode.commands.executeCommand(
						"cmake.activeWorkflowPresetName",
						[],
					)) as string)
...
export class CMakeTaskProvider implements vscode.TaskProvider {
	...
	public static async provideTask(
		commandType: CommandType,
	...
	public async resolveTask(task: CMakeTask): Promise<CMakeTask | undefined> {
		...
		if (!execution) {
			...
			const scope: vscode.WorkspaceFolder | vscode.TaskScope =
				vscode.TaskScope.Workspace;
			const resolvedTask: CMakeTask = new vscode.Task(
				definition,
				scope,
				definition.label,
				CMakeTaskProvider.CMakeSourceStr,
	...
	public static async findBuildTask(
		presetName?: string,
		targets?: string[],
		...
		const tasks: (CMakeTask | undefined)[] = await Promise.all(
			allTasks.map(async (task: any) => {
				if (
					!task.definition.label ||
					!task.group ||
					(task.group && task.group.id !== vscode.TaskGroup.Build.id)
				) {
					return undefined;
				}
				...
				if (expansionOptions && task.definition.targets) {
					taskTargets = await expand.expandStrings(
						task.definition.targets,
						expansionOptions,
					);
					if (task.definition.options?.cwd) {
						task.definition.options.cwd = await expand.expandString(
							task.definition.options.cwd,
							expansionOptions,
						);
					}
				} else {
				...
				const buildTask: CMakeTask = new vscode.Task(
					definition,
					vscode.TaskScope.Workspace,
					task.definition.label,
					CMakeTaskProvider.CMakeSourceStr,
		...
		const buildTasks: CMakeTask[] = tasks.filter(
			(task) => task !== undefined,
		) as CMakeTask[];
		...
		if (presetName) {
			matchingTargetTasks = buildTasks.filter(
				(task) =>
					task.definition.preset === presetName ||
					task.definition.preset ===
						"${command:cmake.activeBuildPresetName}",
			);
		} else {
			matchingTargetTasks = buildTasks.filter((task) => {
				...
				return (
					taskTargets.length === inputTargets.length &&
					taskTargets.every(
						(item, index) => item === inputTargets[index],
					)
				);
			});
		}
		...
		if (matchingTargetTasks.length > 0) {
			...
			if (matchingTargetTasks.length === 1) {
				...
				const defaultTask: CMakeTask[] = matchingTargetTasks.filter(
					(task) => task.isDefault,
				);
				if (defaultTask.length >= 1) {
					...
					if (
						matchingTargetTasks.length === 1 ||
						matchingTargetTasks.length === 2
					) {
						return matchingTargetTasks[
							matchingTargetTasks.length - 1
						];
					}
				}
		...
		const items: TaskMenu[] = matchingTargetTasks.map<TaskMenu>((task) => ({
			label: task.name,
			task: task,
		...
		const selection = await vscode.window.showQuickPick(items, {
			placeHolder: localize("select.build.task", "Select a build task"),
		});
		return selection?.task;
	}
}

export class CustomBuildTaskTerminal
	implements vscode.Pseudoterminal, proc.OutputConsumer
{
	...
	private async correctTargets(
		project: CMakeProject,
		commandType: CommandType,
	): Promise<string[]> {
		let targets: string[] = this.targets;
		const targetIsDefined: boolean =
	...
	private async isTaskCompatibleWithPresets(
		project: CMakeProject,
	): Promise<boolean> {
		const useCMakePresets: boolean = project.useCMakePresets;
		const presetDefined: boolean =
			this.preset !== undefined && this.preset !== null;
	...
	private async resolvePresetName(
	...
	private async runConfigTask(): Promise<any> {
		this.writeEmitter.fire(
			localize("config.started", "Config task started...") + endOfLine,
		);
		...
		if (cmakeDriver) {
			...
			const configPreset: preset.ConfigurePreset | undefined =
				await project?.expandConfigPresetbyName(this.preset);
			const result = await cmakeDriver.configure(
				ConfigureTrigger.taskProvider,
				[],
				this,
				undefined,
				false,
	...
	private async runBuildTask(
		commandType: CommandType,
		doCloseEmitter: boolean = true,
		generateLog: boolean = true,
		project?: CMakeProject,
	): Promise<number> {
		...
		if (!project) {
			...
			if (
				!project ||
				!(await this.isTaskCompatibleWithPresets(project))
			) {
				return -1;
			}
		}
		...
		if (cmakeDriver) {
			...
			if (this.preset) {
				const buildPreset: preset.BuildPreset | undefined =
					await project?.expandBuildPresetbyName(this.preset);
				if (!buildPreset) {
					log.debug(
						localize(
				...
				fullCommand =
					await cmakeDriver.generateBuildCommandFromSettings(targets);
				if (fullCommand) {
					cmakePath = fullCommand.command;
		...
		try {
			...
			if (result.retc) {
				...
				result.stderr ||
				(result.stdout && result.stdout.includes(": warning"))
			) {
				this.writeEmitter.fire(
					localize(
	...
	private async runTestTask(): Promise<any> {
		this.writeEmitter.fire(
			localize("test.started", "Test task started...") + endOfLine,
		);
	...
	private async runPackageTask(): Promise<any> {
		this.writeEmitter.fire(
			localize("package.started", "Package task started...") + endOfLine,
		);
		...
		if (cmakeDriver) {
			...
			if (this.preset) {
				packagePreset = await project?.expandPackagePresetbyName(
					this.preset,
				);
				if (!packagePreset) {
					log.debug(
	...
	private async runWorkflowTask(): Promise<any> {
		this.writeEmitter.fire(
			localize("workflow.started", "Workflow task started...") +
				endOfLine,
		);
		...
		if (cmakeDriver) {
			...
			if (this.preset) {
				workflowPreset = await project?.expandWorkflowPresetbyName(
					this.preset,
				);
				if (!workflowPreset) {
					log.debug(
	...
	private async runCleanRebuildTask(): Promise<any> {
		...
		const cleanResult = await this.runBuildTask(
			CommandType.clean,
			false,
			false,
			project,
