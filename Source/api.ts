...
export class CMakeToolsApiImpl implements api.CMakeToolsApi {
	...
	async getProject(
		uri: vscode.Uri,
	): Promise<CMakeProjectWrapper | undefined> {
		const project: CMakeProject | undefined =
			await this.manager.projectController.getProjectForFolder(
				uri.fsPath,
			);
	...
	private async setUIElementVisibility(
...
class CMakeProjectWrapper implements api.Project {
	...
	configure(): Promise<void> {
		return withErrorCheck(
			"configure",
			async () => (await this.project.configure()).result,
		);
	}
	...
	reconfigure(): Promise<void> {
		return withErrorCheck(
			"reconfigure",
			async () => (await this.project.cleanConfigure()).result,
		);
	}
