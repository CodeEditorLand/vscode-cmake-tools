...
function levelEnabled(level: LogLevel): boolean {
	const strlevel = vscode.workspace
...
class SingletonLogger {
	...
	private get _channel() {
		return channelManager.get(
			`CMake/${localize("build.logging.title", "Build")}`,
		);
	}
	...
	private _log(level: LogLevel, ...args: Stringable[]) {
		...
		this._logStream
			.then((strm) => strm.write(raw_message + "\n"))
			.catch((e) => {
				console.error(
					"Unhandled error while writing CMakeTools log file",
					e,
				);
			});
...
export class Logger {
	...
	showChannel(error_to_show?: boolean) {
		const reveal_log = vscode.workspace
			.getConfiguration("cmake")
			.get<RevealLogKey>("revealLog", "always");
