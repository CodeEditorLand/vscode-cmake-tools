...
export class PresetsController implements vscode.Disposable {
	...
	static async init(
		...
		presetsController._presetsParser = new PresetsParser(
			project.folderPath,
			await expandSourceDir(project.sourceDir),
			project.workspaceFolder.uri.fsPath,
			presetsController.reportPresetsFileErrors,
			presetsController.showPresetsFileVersionError,
			(filePath: string) => {
				collections.presets.set(vscode.Uri.file(filePath), undefined);
			},
