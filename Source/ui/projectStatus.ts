...
export class ProjectStatus {
	...
	async openVisibilitySettings(): Promise<void> {
		await vscode.commands.executeCommand(
			...
			{ revealSetting: { key: "cmake.options.advanced" } },
		);
	}
	...
	implements vscode.TreeDataProvider<Node>, vscode.Disposable
{
	...
	constructor() {
		this.treeView = vscode.window.createTreeView("cmake.projectStatus", {
...
class ConfigPreset extends Node {
	async initialize(): Promise<void> {
		if (
			!treeDataProvider.cmakeProject ||
			!treeDataProvider.cmakeProject.useCMakePresets
		) {
			return;
		}
		this.label =
			treeDataProvider.cmakeProject.configurePreset?.displayName ||
			treeDataProvider.cmakeProject.configurePreset?.name ||
			noConfigPresetSelected;
		this.tooltip = localize(
			"configPreset.change",
			"Change Configure Preset",
		);
...
class WorkflowPreset extends Node {
	async initialize(): Promise<void> {
		...
		this.tooltip = localize(
			"workflowPreset.change",
			"Change Workflow Preset",
		);
...
class BuildTarget extends Node {
	async initialize(): Promise<void> {
		...
		this.label =
			(await treeDataProvider.cmakeProject.buildTargetName()) ||
