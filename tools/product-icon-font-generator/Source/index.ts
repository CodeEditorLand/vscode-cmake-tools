...
class WebfontGenerator {
	private readonly _FILE_MATCHER = new RegExp(
		"^u(?<code>[0-9a-f]{4})[-_]?(?<name>.*)\\.svg$",
		"i",
	);
		...
		for (let fp of fs.readdirSync(baseDir)) {
			...
			if (match?.groups && fs.statSync(fp).isFile()) {
				this._svgIcons.push(
					new SvgIcon({
						fp: fp,
						glyph_code: Number.parseInt(
							`0x${match.groups["code"]}`,
						),
						name: parsed_fp.name,
					}),
				);
	...
	public async generate(
		...
		if (!this._enabledFormats.size) {
			console.warn(
				"No enabled font formats to output; nothing to generate.",
			);
		...
		for (const format of this._enabledFormats) {
			switch (format) {
				...
				default:
					throw Error(
						`Unsupported font format '${FontFormat[format]}'.`,
					);
			}
		}
		...
		await Fontagon({
			...
			codepoints: this._svgIcons.reduce(
				(a, svg_icon) => ({
					...a,
					[svg_icon.name]: svg_icon.glyphCode,
				}),
				{},
			),
			dist: tmp_directory,
...
commander.program
	...
	.requiredOption(
		...
		"The root directory where to recursively import icons from. Filenames must start with a hexadecimal glyph codepoint (e.g. '0xE001.svg')",
	)
	.option(
		"-o, --output-directory <output-directory>",
		"Directory where to output the webfont file. Defaults to the source directory if not set.",
	)
	.option(
		"--name <name>",
		"(File)name of the font. Defaults to the directory name if this option is not set.",
	)
