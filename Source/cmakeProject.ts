...
export class CMakeProject {
	...
	async expandConfigPresetbyName(
		...
		const expandedConfigurePreset = await preset.expandConfigurePreset(
			this.folderPath,
			configurePreset,
	...
	async expandBuildPresetbyName(
		...
		const expandedBuildPreset = await preset.expandBuildPreset(
			this.folderPath,
			buildPreset,
			lightNormalizePath(this.folderPath || "."),
			this.sourceDir,
	...
	async expandTestPresetbyName(
		...
		const expandedTestPreset = await preset.expandTestPreset(
			this.folderPath,
			testPreset,
			lightNormalizePath(this.folderPath || "."),
			this.sourceDir,
	...
	async expandPackagePresetbyName(
		...
		const expandedPackagePreset = await preset.expandPackagePreset(
			this.folderPath,
			packagePreset,
			lightNormalizePath(this.folderPath || "."),
			this.sourceDir,
	...
	async expandWorkflowPresetbyName(
		...
		const expandedWorkflowPreset = await preset.expandWorkflowPreset(
			this.folderPath,
			workflowPreset,
			lightNormalizePath(this.folderPath || "."),
			this.sourceDir,
				...
				return await vscode.window.withProgress(
					...
					async (progress, cancel) => {
						...
						const fileDiags: FileDiagnostic[] | undefined = drv!
							...
							? consumer!.compileConsumer.resolveDiagnostics(
									drv!.binaryDir,
								)
							: [];
						if (fileDiags) {
