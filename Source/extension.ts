...
import { FolderProjectType, ProjectController } from "@cmt/projectController";
import {
	ProjectNode,
	ProjectOutline,
	SourceFileNode,
	TargetNode,
	WorkspaceFolderNode,
} from "@cmt/projectOutline/projectOutline";
...
} from "./debug/debugConfigurationProvider";
import {
	DebuggerInformation,
	getDebuggerPipeName,
...
type QueryCMakeProject = (
	project: CMakeProject,
) => Thenable<string | string[] | null>;
type FolderTargetNameArgsType =
	| vscode.WorkspaceFolder
	| string
	| { folder?: vscode.WorkspaceFolder | string; targetName?: string };
...
export class ExtensionManager implements vscode.Disposable {
	...
	private extensionActiveCommandsInfo: ExtensionActiveCommandsInfo | null =
		null;
	private localizedStrings: { [key: string]: string } = {};
	private onDidChangeActiveTextEditorSub: vscode.Disposable =
		new DummyDisposable();
	private readonly extensionActiveCommandsEmitter =
		new vscode.EventEmitter<void>();
	private readonly workspaceConfig: ConfigurationReader =
		ConfigurationReader.create();
	...
	private updateTouchBarVisibility(config: TouchBarConfig) {
		...
		void setContextAndStore(
			"cmake:enableTouchBar.build",
			touchBarVisible && !(config.advanced?.build === "hidden"),
		);
		void setContextAndStore(
			"cmake:enableTouchBar.configure",
			touchBarVisible && !(config.advanced?.configure === "hidden"),
		);
		void setContextAndStore(
			"cmake:enableTouchBar.debug",
			touchBarVisible && !(config.advanced?.debug === "hidden"),
		);
		void setContextAndStore(
			"cmake:enableTouchBar.launch",
			touchBarVisible && !(config.advanced?.launch === "hidden"),
		);
	}
	/**
	...
	public async init() {
		...
		this.workspaceConfig.onChange("touchbar", (config) =>
			this.updateTouchBarVisibility(config),
		);
		...
		if (
			...
			const workspaceContext = DirectoryContext.createForDirectory(
				vscode.workspace.workspaceFolders[0],
				new StateManager(
		...
		await util.setContextValue(
			"cmake:testExplorerIntegrationEnabled",
			this.workspaceConfig.testExplorerIntegrationEnabled,
		);
		this.workspaceConfig.onChange("ctest", async (value) => {
			await util.setContextValue(
				"cmake:testExplorerIntegrationEnabled",
				value.testExplorerIntegrationEnabled,
			);
		...
		this.onDidChangeActiveTextEditorSub =
			vscode.window.onDidChangeActiveTextEditor(
				(e) => this.onDidChangeActiveTextEditor(e),
				this,
			);

		this.projectController.onAfterAddFolder(
			async (folderProjectMap: FolderProjectType) => {
				...
				await setContextAndStore(
					multiProjectModeKey,
					this.projectController.hasMultipleProjects,
				);
				this.projectOutline.addFolder(folder);
				if (this.codeModelUpdateSubs.get(folder.uri.fsPath)) {
					this.codeModelUpdateSubs
						.get(folder.uri.fsPath)
						?.forEach((sub) => sub.dispose());
					this.codeModelUpdateSubs.delete(folder.uri.fsPath);
				}
				const subs: vscode.Disposable[] = [];
				for (const project of folderProjectMap.projects) {
					subs.push(
						project.onCodeModelChanged(FireLate, () =>
							this.updateCodeModel(project),
						),
					);
					subs.push(
						project.onTargetNameChanged(FireLate, () =>
							this.updateCodeModel(project),
						),
					);
					subs.push(
						project.onLaunchTargetNameChanged(FireLate, () =>
							this.updateCodeModel(project),
						),
					);
					subs.push(
						project.onActiveBuildPresetChanged(FireLate, () =>
							this.updateCodeModel(project),
						),
					);
					...
					rollbar.takePromise(
						...
						this.postWorkspaceOpen(project),
					);
				}
			},
		);
		...
		this.projectController.onAfterRemoveFolder(async (folder) => {
			console.assert(
				(vscode.workspace.workspaceFolders === undefined &&
					this.projectController.numOfWorkspaceFolders === 0) ||
					(vscode.workspace.workspaceFolders !== undefined &&
						vscode.workspace.workspaceFolders.length ===
							this.projectController.numOfWorkspaceFolders),
			);
			this.codeModelUpdateSubs
				.get(folder.uri.fsPath)
				?.forEach((sub) => sub.dispose());
			...
			if (!vscode.workspace.workspaceFolders?.length) {
				...
				await enableFullFeatureSet(
					this.workspaceHasAtLeastOneProject(),
				);
			}
		...
		this.workspaceConfig.onChange("autoSelectActiveFolder", (v) => {
			if (this.projectController.hasMultipleProjects) {
				telemetry.logEvent("configChanged.autoSelectActiveFolder", {
					autoSelectActiveFolder: `${v}`,
				});
			}
		...
		this.workspaceConfig.onChange(
			"additionalCompilerSearchDirs",
			async (_) => {
				KitsController.additionalCompilerSearchDirs =
					await this.getAdditionalCompilerDirs();
			},
		);
		this.workspaceConfig.onChange("mingwSearchDirs", async (_) => {
			// Deprecated in 1.14, replaced by additionalCompilerSearchDirs, but kept for backwards compatibility
			KitsController.additionalCompilerSearchDirs =
				await this.getAdditionalCompilerDirs();
		});
		KitsController.additionalCompilerSearchDirs =
		...
		if (isMultiProject) {
			telemetryProperties["autoSelectActiveFolder"] =
				`${this.workspaceConfig.autoSelectActiveFolder}`;
		}
	...
	public updateContextValues(key: string, value: string) {
		...
		if (
			this.extensionActiveCommandsInfo &&
			(!this.extensionActiveCommandsInfo.contextUsed.hasOwnProperty(
				key,
			) ||
				this.extensionActiveCommandsInfo.contextUsed[key] !== value)
		) {
			this.setExtensionActiveCommands();
	...
	public onExtensionActiveCommandsChanged(
		listener: () => any,
		thisObject: any | null,
	) {
	...
	get extensionActiveCommands(): string[] {
		return this.extensionActiveCommandsInfo
			? this.extensionActiveCommandsInfo.extensionActiveCommands
			: [];
	}
	...
	public setExtensionActiveCommands() {
		this.extensionActiveCommandsInfo = {
			contextUsed: this.contextValues ? { ...this.contextValues } : {},
			extensionActiveCommands: this.contextValues
				? util.thisExtensionActiveCommands(this.contextValues)
				: [],
		} as ExtensionActiveCommandsInfo;
	}
	...
	// <fspath, sub>
	private readonly codeModelUpdateSubs = new Map<
		string,
		vscode.Disposable[]
	>();
	...
	private readonly projectOutlineTreeView = vscode.window.createTreeView(
		"cmake.outline",
		{
	...
	private getProjectsForWorkspaceFolder(
		folder?: vscode.WorkspaceFolder,
	): CMakeProject[] | undefined {
	...
	private getWorkspaceFolder(
		...
		if (util.isString(folder)) {
			...
			return vscode.workspace.getWorkspaceFolder(
				vscode.Uri.file(folder as string),
			);
		}
		const workspaceFolder = folder as vscode.WorkspaceFolder;
		if (
			util.isNullOrUndefined(folder) ||
			util.isNullOrUndefined(workspaceFolder.uri)
		) {
	 ...
	 */
	private async ensureActiveConfigurePresetOrKit(
		...
		if (cmakeProject.useCMakePresets) {
			...
			const didChoosePreset = await this.selectConfigurePreset(
				cmakeProject.workspaceFolder,
			);
			...
			const hascmakelists = await util.globForFileName(
				"CMakeLists.txt",
				3,
			...
			if (
				!this.workspaceConfig.enableAutomaticKitScan ||
				!hascmakelists
			) {
				await cmakeProject.kitsController.setKitByName(
					SpecialKits.Unspecified,
				);
				return true;
			}
			// Ask the user what they want.
			const didChooseKit = await this.selectKit(
				cmakeProject.workspaceFolder,
			);
	 ...
	 */
	private readonly ensureActiveBuildPreset = async (
		project?: CMakeProject,
	): Promise<boolean> => {
		if (!project) {
		...
		if (project.useCMakePresets) {
			...
			const didChoosePreset = await this.selectBuildPreset(
	...
	private readonly ensureActiveTestPreset = async (
		project?: CMakeProject,
	): Promise<boolean> => {
		if (!project) {
			project = this.getActiveProject();
		...
		if (project.useCMakePresets) {
			...
			const didChoosePreset = await this.selectTestPreset(
				project.workspaceFolder,
			);
	...
	private readonly ensureActivePackagePreset = async (
		project?: CMakeProject,
	): Promise<boolean> => {
		if (!project) {
			project = this.getActiveProject();
		...
		if (project.useCMakePresets) {
			...
			const didChoosePreset = await this.selectPackagePreset(
				project.workspaceFolder,
			);
	...
	private readonly ensureActiveWorkflowPreset = async (
		project?: CMakeProject,
	): Promise<boolean> => {
		if (!project) {
			project = this.getActiveProject();
		...
		if (project.useCMakePresets) {
			...
			const didChoosePreset = await this.selectWorkflowPreset(
				project.workspaceFolder,
			);
	...
	dispose() {
		rollbar.invokeAsync(
			localize("dispose.cmake.tools", "Dispose of CMake Tools"),
			() => this.asyncDispose(),
		);
	}
	...
	async asyncDispose() {
		this.disposeSubs();
		this.codeModelUpdateSubs.forEach((subs) =>
	...
	async configureExtensionInternal(
		trigger: ConfigureTrigger,
		project: CMakeProject,
	): Promise<void> {
		if (
			trigger !== ConfigureTrigger.configureWithCache &&
			!(await this.ensureActiveConfigurePresetOrKit(project))
		) {
			return;
		}
	...
	async postWorkspaceOpen(project?: CMakeProject) {
		...
		const silentScanForKitsNeeded: boolean =
			vscode.workspace.workspaceFolders !== undefined &&
			vscode.workspace.workspaceFolders[0] === rootFolder &&
			(await scanForKitsIfNeeded(project));

		const shouldConfigure =
			project?.workspaceContext.config.configureOnOpen;
		...
		if (!project.hasCMakeLists()) {
			if (shouldConfigure && hascmakelists) {
				await project.cmakePreConditionProblemHandler(
			...
			if (shouldConfigure) {
				...
				log.debug(
					localize(
						"configuring.workspace.on.open",
						"Configuring workspace on open {0}",
						project.folderPath,
					),
				);
				await this.configureExtensionInternal(
					ConfigureTrigger.configureOnOpen,
					project,
				);
			} else {
				const configureButtonMessage = localize(
				...
				if (silentScanForKitsNeeded) {
					...
					result = await vscode.window.showWarningMessage(
						localize(
							...
							"It is recommended to reconfigure after upgrading to a new kits definition.",
						),
						configureButtonMessage,
					);
				}
				if (result === configureButtonMessage) {
					await this.configureExtensionInternal(
						ConfigureTrigger.buttonNewKitsDefinition,
						project,
					);
				} else {
					log.debug(
						localize(
							"using.cache.to.configure.workspace.on.open",
							"Attempting to use cache to configure workspace {0}",
							rootFolder.uri.toString(),
						),
					);
					await this.configureExtensionInternal(
						ConfigureTrigger.configureWithCache,
						project,
					);
				}
			}
	...
	private async onDidChangeActiveTextEditor(
		...
		if (
			...
			if (folder) {
				if (
					!this.activeCMakeWorkspaceFolder() ||
					folder.uri.fsPath !== this.activeFolderPath()
				) {
	...
	async selectActiveFolder(project?: CMakeProject | string[]) {
		...
		if (project instanceof CMakeProject) {
			...
			project.length > 0 &&
			typeof project[0] === "string"
		) {
			const projects: CMakeProject[] =
				this.projectController.getAllCMakeProjects();
			if (projects.length !== 0) {
				selection = projects.find(
					(proj) => proj.folderName === project[0],
				);
			}
	...
	private async initActiveProject(): Promise<CMakeProject | undefined> {
		...
		if (
			...
			await this.updateActiveProject(
				...
				folder ? vscode.window.activeTextEditor : undefined,
			);
			return this.getActiveProject();
		}
		const activeFolder =
			this.extensionContext.workspaceState.get<string>("activeFolder");
		if (activeFolder) {
			folder = vscode.workspace.getWorkspaceFolder(
				vscode.Uri.parse(activeFolder),
			);
		}
		...
		if (defaultActiveFolder) {
			...
			folder = vscode.workspace.workspaceFolders!.find((candidate) =>
				candidate.uri.path.endsWith(defaultActiveFolder),
			);
		}
	...
	private async updateActiveProject(
		workspaceFolder?: vscode.WorkspaceFolder,
		editor?: vscode.TextEditor,
	): Promise<void> {
		await this.projectController.updateActiveProject(
			workspaceFolder,
			editor,
			this.workspaceConfig.options,
		);
	...
	private async postUpdateActiveProject() {
		...
		if (activeProject) {
			this.statusBar.setActiveProjectName(
			...
			if (!useCMakePresets) {
				this.statusBar.setActiveKitName(
					activeProject.activeKit?.name || "",
				);
			}
			this.projectOutline.setActiveFolder(activeProject.folderPath);
			this.setupSubscriptions();
			this.onActiveProjectChangedEmitter.fire(
				vscode.Uri.file(activeProject.folderPath),
			);
			const currentActiveFolderPath = this.activeFolderPath();
			await this.extensionContext.workspaceState.update(
				"activeFolder",
	...
	private updateCodeModel(cmakeProject?: CMakeProject) {
		...
		rollbar.invokeAsync(
			localize(
				"update.code.model.for.cpptools",
				"Update code model for cpptools",
			),
			{},
			async () => {
				if (
					vscode.workspace
						.getConfiguration("C_Cpp", folder.uri)
						.get<string>("intelliSenseEngine")
						?.toLocaleLowerCase() === "disabled"
				) {
					log.debug(
						localize(
							"update.intellisense.disabled",
							"Not updating the configuration provider because {0} is set to {1}",
				...
				if (!this.cppToolsAPI && !util.isTestMode()) {
					try {
						this.cppToolsAPI = await cpt.getCppToolsApi(
							cpt.Version.latest,
						);
					} catch (err) {
						log.debug(
							localize(
								"failed.to.get.cpptools.api",
								"Failed to get cppTools API",
							),
						);
					}
				...
				if (
					this.cppToolsAPI &&
					(cmakeProject.activeKit || cmakeProject.configurePreset)
				) {
					const cpptools = this.cppToolsAPI;
					let cache: CMakeCache;
					try {
						cache = await CMakeCache.fromPath(
							await cmakeProject.cachePath,
						);
					} catch (e: any) {
						rollbar.exception(
							localize(
								"filed.to.open.cache.file.on.code.model.update",
								"Failed to open CMake cache file on code model update",
							),
							e,
						);
						return;
					}
					const drv: CMakeDriver | null =
						await cmakeProject.getCMakeDriverInstance();
					const configureEnv = await drv?.getConfigureEnvironment();

					const configurationTypes = cache.get(
					...
					const actualBuildType =
						await cmakeProject.currentBuildType();

					const clCompilerPath =
						await findCLCompilerPath(configureEnv);
					...
					if (cmakeProject.codeModelContent) {
						...
						this.configProvider.updateConfigurationData({
							cache,
							codeModelContent,
							clCompilerPath,
							activeTarget: cmakeProject.defaultBuildTarget,
							activeBuildTypeVariant: actualBuildType,
							folder: cmakeProject.folderPath,
						});
	...
	private setupSubscriptions() {
		...
		if (!cmakeProject) {
			...
			this.statusMessageSub = cmakeProject.onStatusMessageChanged(
				FireNow,
				(s) => this.statusBar.setStatusMessage(s),
			);
			this.targetNameSub = cmakeProject.onTargetNameChanged(
				FireNow,
				(t) => {
			...
			this.buildTypeSub = cmakeProject.onActiveVariantNameChanged(
				FireNow,
				(bt) => this.statusBar.setVariantLabel(bt),
			);
			this.launchTargetSub = cmakeProject.onLaunchTargetNameChanged(
				FireNow,
				(t) => {
			...
			this.ctestEnabledSub = cmakeProject.onCTestEnabledChanged(
				FireNow,
				(e) => this.statusBar.setCTestEnabled(e),
			);
			this.isBusySub = cmakeProject.onIsBusyChanged(FireNow, (b) =>
				this.statusBar.setIsBusy(b),
			);
			this.statusBar.setActiveKitName(
				cmakeProject.activeKit ? cmakeProject.activeKit.name : "",
			);
			this.activeConfigurePresetSub =
				cmakeProject.onActiveConfigurePresetChanged(FireNow, (p) => {
					this.statusBar.setConfigurePresetName(
						p?.displayName || p?.name || "",
					);
					cmakeProject.notifyOnSelectedConfigurationChanged(
						api.ConfigurationType.ConfigurePreset,
					);
				});
			this.activeBuildPresetSub = cmakeProject.onActiveBuildPresetChanged(
				FireNow,
				(p) => {
					this.statusBar.setBuildPresetName(
						p?.displayName || p?.name || "",
					...
					cmakeProject.notifyOnSelectedConfigurationChanged(
						api.ConfigurationType.BuildPreset,
					);
				},
			);
			this.activeTestPresetSub = cmakeProject.onActiveTestPresetChanged(
				FireNow,
				(p) => {
					this.statusBar.setTestPresetName(
						p?.displayName || p?.name || "",
			...
			this.activePackagePresetSub =
				cmakeProject.onActivePackagePresetChanged(FireNow, (p) => {
					this.statusBar.setPackagePresetName(
						p?.displayName || p?.name || "",
					);
				});
			this.activeWorkflowPresetSub =
				cmakeProject.onActiveWorkflowPresetChanged(FireNow, (p) => {
					this.statusBar.setWorkflowPresetName(
						p?.displayName || p?.name || "",
					);
				});
	...
	private readonly kitsWatcher = util.chokidarOnAnyChange(
		...
		(_) =>
			rollbar.takePromise(
				localize("rereading.kits", "Re-reading kits"),
				{},
				KitsController.readUserKits(this.getActiveProject()),
			),
	);
		...
		if (duplicateRemoved) {
			...
			for (const project of this.projectController.getAllCMakeProjects()) {
				...
				if (activeKit) {
					...
					if (
						definition &&
						(definition.startsWith("VisualStudio.15") ||
							definition.startsWith("VisualStudio.16"))
					) {
	...
	private async getAdditionalCompilerDirs(): Promise<string[]> {
		...
		for (const dir of this.workspaceConfig.additionalCompilerSearchDirs) {
			const expandedDir: string = util.lightNormalizePath(
				await expandString(dir, { vars: optsVars }),
			);
	...
	viewPackageSettings(): void {
		void vscode.commands.executeCommand(
			...
			"@id:cmake.cpackArgs, @id:cmake.cpackEnvironment, @id:cmake.environment",
		);
	}
	...
	async selectKit(folder?: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.kit.in.test.mode",
					"Running CMakeTools in test mode. selectKit is disabled.",
				),
			);
			return false;
		}
	...
	async setKitByName(kitName: string, folder?: vscode.WorkspaceFolder) {
		const projects = folder
			? this.projectController.getProjectsForWorkspaceFolder(folder)
			: this.projectController.getAllCMakeProjects();
		for (const project of projects || []) {
	...
	async setConfigurePreset(
		...
		if (folder) {
			if (!this.useCMakePresets(folder)) {
				log.info(
					localize(
						"skip.set.config.preset",
						"Using kits, skip setting configure preset: {0}",
						presetName,
					),
				);
				return;
			}
			await this.getActiveProject()?.presetsController.setConfigurePreset(
				presetName,
			);
		} else {
			for (const project of this.projectController.getAllCMakeProjects()) {
	...
	async setBuildPreset(presetName: string, folder?: vscode.WorkspaceFolder) {
		if (folder) {
			if (!this.useCMakePresets(folder)) {
				log.info(
					localize(
						"skip.set.build.preset",
						"Using kits, skip setting build preset: {0}",
						presetName,
					),
				);
				return;
			}
			await this.getActiveProject()?.presetsController.setBuildPreset(
				presetName,
			);
		} else {
			for (const project of this.projectController.getAllCMakeProjects()) {
	...
	async setTestPreset(presetName: string, folder?: vscode.WorkspaceFolder) {
		if (folder) {
			if (!this.useCMakePresets(folder)) {
				log.info(
					localize(
						"skip.set.test.preset",
						"Using kits, skip setting test preset: {0}",
						presetName,
					),
				);
				return;
			}
			await this.getActiveProject()?.presetsController.setTestPreset(
				presetName,
			);
		} else {
			for (const project of this.projectController.getAllCMakeProjects()) {
	...
	async setPackagePreset(
		...
		if (folder) {
			if (!this.useCMakePresets(folder)) {
				log.info(
					localize(
						"skip.set.package.preset",
						"Using kits, skip setting package preset: {0}",
						presetName,
					),
				);
				return;
			}
			await this.getActiveProject()?.presetsController.setPackagePreset(
				presetName,
			);
		} else {
			for (const project of this.projectController.getAllCMakeProjects()) {
	...
	async setWorkflowPreset(
		...
		if (folder) {
			if (!this.useCMakePresets(folder)) {
				log.info(
					localize(
						"skip.set.workflow.preset",
						"Using kits, skip setting workflow preset: {0}",
						presetName,
					),
				);
				return;
			}
			await this.getActiveProject()?.presetsController.setWorkflowPreset(
				presetName,
			);
		} else {
			for (const project of this.projectController.getAllCMakeProjects()) {
	...
	async runCMakeCommandForProject(
		command: RunCMakeCommand,
		cmakeProject = this.getActiveProject(),
		precheck?: (cmakeProject: CMakeProject) => Promise<boolean>,
	): Promise<any> {
		if (!cmakeProject) {
			rollbar.error(
				localize("no.active.cmake.project", "No active CMake project."),
			);
			return -2;
		}
	...
	async runCMakeCommandForAll(
		command: RunCMakeCommand,
		precheck?: (cmakeProject: CMakeProject) => Promise<boolean>,
		cleanOutputChannel?: boolean,
	): Promise<any> {
	...
	private getProjectFromFolder(
		folder?: vscode.WorkspaceFolder | string,
		sourceDir?: string,
	) {
		const workspaceFolder: vscode.WorkspaceFolder | undefined =
			this.getWorkspaceFolder(folder);
		if (workspaceFolder) {
			const activeProject: CMakeProject | undefined =
				this.getActiveProject();
			const projects: CMakeProject[] | undefined =
				this.projectController.getProjectsForWorkspaceFolder(
					workspaceFolder,
				);
					...
					projects.find(
						(project) =>
							...
							path.normalize(sourceDir) ===
								path.normalize(project.folderPath),
					) ??
					// Choose project by folder of active project
					projects.find(
						(project) =>
							activeProject?.folderPath === project.folderPath,
					) ??
	...
	cleanConfigure(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) =>
				cmakeProject.cleanConfigure(
					ConfigureTrigger.commandCleanConfigure,
				),
			folder,
			undefined,
			true,
	...
	cleanConfigureWithDebuggerInternal(
		...
		return this.runCMakeCommand(
			(cmakeProject) =>
				cmakeProject.cleanConfigureWithDebugger(
					ConfigureTrigger.commandCleanConfigureWithDebugger,
					debuggerInformation,
				),
			folder,
			undefined,
	...
	cleanConfigureAll() {
		...
		return this.runCMakeCommandForAll(
			(cmakeProject) =>
				cmakeProject.cleanConfigure(
					ConfigureTrigger.commandCleanConfigureAll,
				),
			undefined,
			true,
		);
	...
	cleanConfigureAllWithDebuggerInternal(
		debuggerInformation: DebuggerInformation,
		trigger?: ConfigureTrigger,
	) {
		telemetry.logEvent("deleteCacheAndReconfigureWithDebugger");
		return this.runCMakeCommandForAll(
			(cmakeProject) =>
				cmakeProject.cleanConfigureWithDebugger(
					trigger ??
						ConfigureTrigger.commandCleanConfigureAllWithDebugger,
					debuggerInformation,
				),
			undefined,
			true,
	...
	configure(
		...
		return this.runCMakeCommand(
			async (cmakeProject) =>
				(
					await cmakeProject.configureInternal(
						ConfigureTrigger.commandConfigure,
						[],
						showCommandOnly
							? ConfigureType.ShowCommandOnly
		...
		return this.runCMakeCommand(
			async (cmakeProject) =>
				(
					await cmakeProject.configureInternal(
						trigger ??
							ConfigureTrigger.commandConfigureWithDebugger,
						[],
						showCommandOnly
							? ConfigureType.ShowCommandOnly
							: ConfigureType.NormalWithDebugger,
						debuggerInformation,
					)
				).result,
			folder,
	...
	configureAllWithDebuggerInternal(
		debuggerInformation: DebuggerInformation,
		trigger?: ConfigureTrigger,
	) {
		...
		return this.runCMakeCommandForAll(
			async (cmakeProject) =>
				(
					await cmakeProject.configureInternal(
						trigger ??
							ConfigureTrigger.commandConfigureAllWithDebugger,
						[],
						ConfigureType.NormalWithDebugger,
						debuggerInformation,
					)
	...
	build(
		...
		return this.runCMakeCommand(
			(cmakeProject) => {
				...
				return cmakeProject.build(
					targets,
					showCommandOnly,
					isBuildCommand === undefined ? true : isBuildCommand,
				);
			},
			folder,
	...
	buildAll(name?: string | string[]) {
		...
		return this.runCMakeCommandForAll(
			(cmakeProject) => {
				const targets = util.isArrayOfString(name)
					? name
					: util.isString(name)
						? [name]
						: undefined;
	...
	setDefaultTarget(
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.setDefaultTarget(name),
			folder,
			undefined,
			undefined,
			sourceDir,
	...
	setVariant(folder?: vscode.WorkspaceFolder, name?: string) {
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.setVariant(name),
			folder,
		);
	}
	...
	async setVariantAll() {
		...
		if (choice) {
			return this.runCMakeCommandForAll((cmakeProject) =>
				cmakeProject.setVariant(choice.label),
			);
		}
		return false;
	}
	...
	install(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.install(),
			folder,
			undefined,
			true,
		);
	...
	editCache(folder: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.editCache(),
			folder,
		);
	}
	...
	cleanRebuild(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.cleanRebuild(),
			folder,
			this.ensureActiveBuildPreset,
			true,
		);
	...
	async buildWithTarget() {
		telemetry.logEvent("build", {
			command: "buildWithTarget",
			all: "false",
		});
		this.cleanOutputChannel();
	...
	private async pickCMakeProject(): Promise<CMakeProject | undefined> {
		...
		const selection = await vscode.window.showQuickPick(items, {
			placeHolder: localize(
				"select.a.cmake.project",
				"Select a cmake project",
			),
		});
		if (selection) {
			console.assert(
				selection.cmakeProject.folderName,
				"Project not found in project controller.",
			);
	...
	async compileFile(file?: string | vscode.Uri) {
		...
		void vscode.window.showErrorMessage(
			localize(
				"compilation information.not.found",
				"Unable to find compilation information for this file",
			),
		);
	}
	...
	ctest(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.ctest(),
			folder,
			this.ensureActiveTestPreset,
		);
	}
	...
	cpack(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.cpack(),
			folder,
			this.ensureActivePackagePreset,
		);
	}
	...
	workflow(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.workflow(),
			folder,
			this.ensureActiveWorkflowPreset,
		);
	}
	...
	revealTestExplorer(folder?: vscode.WorkspaceFolder) {
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.revealTestExplorer(),
			folder,
			this.ensureActiveTestPreset,
		);
	}
	...
	refreshTests(folder?: vscode.WorkspaceFolder) {
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.refreshTests(),
			folder,
		);
	}
	...
	refreshTestsAll() {
		return this.runCMakeCommandForAll((cmakeProject) =>
			cmakeProject.refreshTests(),
		);
	}

	stop(folder?: vscode.WorkspaceFolder) {
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.stop(),
			folder,
		);
	}
	...
	quickStart(folder?: vscode.WorkspaceFolder) {
		...
		return this.runCMakeCommandForProject((cmakeProject) =>
			cmakeProject.quickStart(folder),
		);
	}

	resolveFolderTargetNameArgs(
		args?: FolderTargetNameArgsType,
	): [folder?: vscode.WorkspaceFolder | string, targetName?: string] {
	...
	launchTargetDirectory(args?: FolderTargetNameArgsType) {
		...
		telemetry.logEvent("substitution", {
			command: "launchTargetDirectory",
		});
		return this.queryCMakeProject(async (cmakeProject) => {
			if (targetName !== undefined && targetName !== null) {
				await cmakeProject.setLaunchTargetByName(targetName);
	...
	launchTargetName(args?: FolderTargetNameArgsType) {
		...
		return this.queryCMakeProject(async (cmakeProject) => {
			...
			const targetFilename =
				await cmakeProject.launchTargetNameForSubstitution();
			return targetFilename;
		}, folder);
	}
	...
	getLaunchTargetDirectory(args?: FolderTargetNameArgsType) {
		...
		telemetry.logEvent("substitution", {
			command: "getLaunchTargetDirectory",
		});
		return this.queryCMakeProject(async (cmakeProject) => {
			if (targetName !== undefined && targetName !== null) {
				await cmakeProject.setLaunchTargetByName(targetName);
			...
			const targetDirectory =
				await cmakeProject.getLaunchTargetDirectory();
			return targetDirectory;
		}, folder);
	}
	...
	buildTargetName(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			(cmakeProject) => cmakeProject.buildTargetName(),
			folder,
		);
	}
	...
	buildType(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			(cmakeProject) => cmakeProject.currentBuildType(),
			folder,
		);
	}
	...
	buildDirectory(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			(cmakeProject) => cmakeProject.buildDirectory(),
			folder,
		);
	}
	...
	buildKit(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			(cmakeProject) => cmakeProject.buildKit(),
			folder,
		);
	}
	...
	executableTargets(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			async (cmakeProject) =>
				(await cmakeProject.executableTargets).map(
					(target) => target.name,
				),
			folder,
		);
	...
	tasksBuildCommand(folder?: vscode.WorkspaceFolder | string) {
		...
		return this.queryCMakeProject(
			(cmakeProject) => cmakeProject.tasksBuildCommand(),
			folder,
		);
	}
	...
	debugTarget(
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.debugTarget(name),
			folder,
			undefined,
			true,
			sourceDir,
	...
	async debugTargetAll(): Promise<(vscode.DebugSession | null)[]> {
		...
		for (const cmakeProject of this.projectController.getAllCMakeProjects()) {
			debugSessions.push(
				await this.runCMakeCommandForProject(
					(cmakeProject) => cmakeProject.debugTarget(),
					cmakeProject,
				),
			);
		}
	...
	launchTarget(
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.launchTarget(name),
			folder,
			undefined,
			true,
			sourceDir,
	...
	async launchTargetAll(): Promise<(vscode.Terminal | null)[]> {
		...
		for (const cmakeProject of this.projectController.getAllCMakeProjects()) {
			terminals.push(
				await this.runCMakeCommandForProject(
					(cmakeProject) => cmakeProject.launchTarget(),
					cmakeProject,
				),
			);
		}
	...
	selectLaunchTarget(
		...
		return this.runCMakeCommand(
			(cmakeProject) => cmakeProject.selectLaunchTarget(name),
			folder,
			undefined,
			undefined,
			sourceDir,
	...
	async resetState(folder?: vscode.WorkspaceFolder) {
		...
		if (folder) {
			await this.runCMakeCommand(
				(cmakeProject) => cmakeProject.resetState(),
				folder,
			);
		} else {
			await this.runCMakeCommandForAll((cmakeProject) =>
				cmakeProject.resetState(),
			);
		}
	...
	activeConfigurePresetName(): string {
		telemetry.logEvent("substitution", {
			command: "activeConfigurePresetName",
		});
	...
	activePackagePresetName(): string {
		telemetry.logEvent("substitution", {
			command: "activePackagePresetName",
		});
	...
	activeWorkflowPresetName(): string {
		telemetry.logEvent("substitution", {
			command: "activeWorkflowPresetName",
		});
	...
	async addConfigurePreset(folder: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"add.config.preset.in.test.mode",
					"Running CMakeTools in test mode. addConfigurePreset is disabled.",
				),
			);
			return false;
		}
	...
	async addBuildPreset(folder: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"add.build.preset.in.test.mode",
					"Running CMakeTools in test mode. addBuildPreset is disabled.",
				),
			);
			return false;
		}
	...
	async addTestPreset(folder: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"add.test.preset.in.test.mode",
					"Running CMakeTools in test mode. addTestPreset is disabled.",
				),
			);
			return false;
		}
	...
	async addPackagePreset(folder: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"add.package.preset.in.test.mode",
					"Running CMakeTools in test mode. addPackagePreset is disabled.",
				),
			);
			return false;
		}
	...
	async addWorkflowPreset(folder: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"add.workflow.preset.in.test.mode",
					"Running CMakeTools in test mode. addWorkflowPreset is disabled.",
				),
			);
			return false;
		}
	...
	async selectConfigurePreset(
		...
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.config.preset.in.test.mode",
					"Running CMakeTools in test mode. selectConfigurePreset is disabled.",
				),
			);
			return false;
		}
		...
		if (!project.useCMakePresets) {
			log.info(
				localize(
					"skip.set.configure.preset",
					"Using kits, skip selecting configure preset",
				),
			);
			return false;
		}
		...
		const presetSelected =
			await project.presetsController.selectConfigurePreset();
		const configurePreset = project.configurePreset;
		this.statusBar.setConfigurePresetName(
			configurePreset?.displayName || configurePreset?.name || "",
		);
		...
		this.statusBar.setBuildPresetName(
			buildPreset?.displayName || buildPreset?.name || "",
		);
		const testPreset = project.testPreset;
		this.statusBar.setTestPresetName(
			testPreset?.displayName || testPreset?.name || "",
		);
	...
	viewConfigureSettings(): void {
		void vscode.commands.executeCommand(
			"workbench.action.openSettings",
			"@id:cmake.configureArgs, @id:cmake.configureEnvironment, @id:cmake.environment",
		);
	}
	...
	async selectBuildPreset(folder?: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.build.preset.in.test.mode",
					"Running CMakeTools in test mode. selectBuildPreset is disabled.",
				),
			);
			return false;
		}
		...
		if (!project.useCMakePresets) {
			log.info(
				localize(
					"skip.set.build.preset",
					"Using kits, skip selecting build preset",
				),
			);
			return false;
		}
		...
		const presetSelected =
			await project.presetsController.selectBuildPreset();
		const buildPreset = project.buildPreset;
		this.statusBar.setBuildPresetName(
			buildPreset?.displayName || buildPreset?.name || "",
		);
	...
	viewBuildSettings(): void {
		void vscode.commands.executeCommand(
			"workbench.action.openSettings",
			"@id:cmake.buildArgs, @id:cmake.buildToolArgs @id:cmake.buildEnvironment @id:cmake.environment",
		);
	}
	...
	async selectTestPreset(folder?: vscode.WorkspaceFolder): Promise<boolean> {
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.test.preset.in.test.mode",
					"Running CMakeTools in test mode. selectTestPreset is disabled.",
				),
			);
			return false;
		}
		...
		if (!project.useCMakePresets) {
			log.info(
				localize(
					"skip.set.test.preset",
					"Using kits, skip selecting test preset",
				),
			);
			return false;
		}
		...
		const presetSelected =
			await project.presetsController.selectTestPreset();
		const testPreset = project.testPreset;
		this.statusBar.setTestPresetName(
			testPreset?.displayName || testPreset?.name || "",
		);
	...
	viewTestSettings(): void {
		void vscode.commands.executeCommand(
			"workbench.action.openSettings",
			"@id:cmake.ctestArgs, @id:cmake.testEnvironment, @id:cmake.environment",
		);
	}
	...
	async selectPackagePreset(
		...
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.package.preset.in.test.mode",
					"Running CMakeTools in test mode. selectPackagePreset is disabled.",
				),
			);
			return false;
		}
		...
		if (!project.useCMakePresets) {
			log.info(
				localize(
					"skip.set.package.preset",
					"Using kits, skip selecting package preset",
				),
			);
			return false;
		}
		...
		const presetSelected =
			await project.presetsController.selectPackagePreset();
		const packagePreset = project.packagePreset;
		this.statusBar.setPackagePresetName(
			packagePreset?.displayName || packagePreset?.name || "",
		);
		return presetSelected;
	}
	...
	async selectWorkflowPreset(
		...
		if (util.isTestMode()) {
			log.trace(
				localize(
					"selecting.workflow.preset.in.test.mode",
					"Running CMakeTools in test mode. selectWorkflowPreset is disabled.",
				),
			);
			return false;
		}
		...
		if (!project.useCMakePresets) {
			log.info(
				localize(
					"skip.set.workflow.preset",
					"Using kits, skip selecting workflow preset",
				),
			);
			return false;
		}
		...
		const presetSelected =
			await project.presetsController.selectWorkflowPreset();
		const workflowPreset = project.workflowPreset;
		this.statusBar.setWorkflowPresetName(
			workflowPreset?.displayName || workflowPreset?.name || "",
		);
		return presetSelected;
	}
	...
	private readonly onBuildTargetChangedEmitter =
		new vscode.EventEmitter<string>();

	get onLaunchTargetChanged() {
		return this.onLaunchTargetChangedEmitter.event;
	}
	private readonly onLaunchTargetChangedEmitter =
		new vscode.EventEmitter<string>();

	get onActiveProjectChanged() {
		return this.onActiveProjectChangedEmitter.event;
	}
	private readonly onActiveProjectChangedEmitter = new vscode.EventEmitter<
...
async function setup(
...
): Promise<api.CMakeToolsExtensionExports> {
			...
			(...args: any[]) => {
				...
				const pr = (async () => {
					...
					try {
						...
						log.debug(
							localize(
								"cmake.finished.returned",
								"{0} finished (returned {1})",
								`[${id}] cmake.${name}`,
								JSON.stringify(ret),
							),
						);
						...
						log.debug(
							localize(
								"cmake.finished.returned.unserializable",
								"{0} finished (returned an unserializable value)",
								`[${id}] cmake.${name}`,
							),
						);
					}
	...
	reportProgress(
		localize("loading.extension.commands", "Loading extension commands"),
		progress,
	);
	for (const key of funs) {
		log.trace(
			localize(
				"register.command",
				"Register CMakeTools extension command {0}",
				`cmake.${key}`,
			),
		);
	...
	if (util.isTestMode()) {
		log.trace(
			localize(
				"register.command",
				"Register CMakeTools extension command cmake.getSettingsChangePromise",
			),
		);
		context.subscriptions.push(
			vscode.commands.registerCommand(
				"cmake.getSettingsChangePromise",
				() => getSettingsChangePromise(),
			),
		);
	}
	...
	context.subscriptions.push(
		...[
			...
			vscode.commands.registerCommand("cmake.outline.configureAll", () =>
				runCommand("configureAll"),
			),
			...
			vscode.commands.registerCommand(
				...
				() =>
					runCommand(
						"configureAllWithDebugger",
						ConfigureTrigger.projectOutlineConfigureAllWithDebugger,
					),
			),
			vscode.commands.registerCommand("cmake.outline.buildAll", () =>
				runCommand("buildAll"),
			),
			vscode.commands.registerCommand("cmake.outline.stopAll", () =>
				runCommand("stopAll"),
			),
			vscode.commands.registerCommand("cmake.outline.cleanAll", () =>
				runCommand("cleanAll"),
			),
			vscode.commands.registerCommand(
				"cmake.outline.cleanConfigureAll",
				() => runCommand("cleanConfigureAll"),
			),
			vscode.commands.registerCommand(
				...
				() =>
					runCommand(
						"cleanConfigureAllWithDebugger",
						ConfigureTrigger.projectOutlineCleanConfigureAllWithDebugger,
					),
			),
			vscode.commands.registerCommand("cmake.outline.editCacheUI", () =>
				runCommand("editCacheUI"),
			),
			vscode.commands.registerCommand(
				"cmake.outline.cleanRebuildAll",
				() => runCommand("cleanRebuildAll"),
			),
			// Commands for outline items
			vscode.commands.registerCommand(
				"cmake.outline.configure",
				async (what: ProjectNode | SourceFileNode) => {
					if (what instanceof ProjectNode) {
						await runCommand(
							"configure",
							what.folder,
							false,
							what.sourceDirectory,
						);
			...
			vscode.commands.registerCommand(
				...
				async (what: SourceFileNode) => {
					await runCommand(
						"configureWithDebugger",
						what.folder,
						what.sourcePath,
						ConfigureTrigger.projectOutlineConfigureWithDebugger,
					);
				},
			),
			vscode.commands.registerCommand(
			...
			vscode.commands.registerCommand(
				...
				(what: TargetNode) =>
					runCommand("build", what.folder, what.name, what.sourceDir),
			),
			vscode.commands.registerCommand(
			...
			vscode.commands.registerCommand(
				...
				(what: SourceFileNode) =>
					runCommand("compileFile", what.filePath),
			),
			...
			vscode.commands.registerCommand(
				...
				(what: WorkspaceFolderNode) =>
					runCommand("selectWorkspace", what.wsFolder),
			),
			// Notification of active project change (e.g. when cmake.sourceDirectory changes)
			vscode.commands.registerCommand("cmake.statusbar.update", () =>
				extensionManager?.updateStatusBarForActiveProjectChange(),
			),
		],
	);
...
class SchemaProvider implements vscode.TextDocumentContentProvider {
	public async provideTextDocumentContent(uri: vscode.Uri): Promise<string> {
		console.assert(
			uri.path[0] === "/",
			"A preceeding slash is expected on schema uri path",
		);
		const fileName: string = uri.path.substr(1);
		const locale: string = util.getLocaleId();
		let localizedFilePath: string = path.join(
			util.thisExtensionPath(),
			"dist/schema/",
			locale,
			"schemas",
			fileName,
...
export async function activate(
	...
	const oldCMakeToolsExtension = vscode.extensions.getExtension(
		"vector-of-bool.cmake-tools",
	);
	if (oldCMakeToolsExtension) {
		await vscode.window.showWarningMessage(
			localize(
				"uninstall.old.cmaketools",
				"Please uninstall any older versions of the CMake Tools extension. It is now published by Microsoft starting with version 1.2.0.",
			),
		);
	}
	...
	if (
		vscode.workspace
			...
			.get("showOptionsMovedNotification")
	) {
		void vscode.window
			.showInformationMessage(
				localize(
					"options.moved.notification.body",
				...
				localize(
					"options.moved.notification.configure.cmake.options",
					"Configure CMake Options Visibility",
				),
				localize(
					"options.moved.notification.do.not.show",
					"Do Not Show Again",
				),
			)
	...
	taskProvider = vscode.tasks.registerTaskProvider(
		CMakeTaskProvider.CMakeScriptType,
		cmakeTaskProvider,
	);
	...
	pinnedCommands = new PinnedCommands(
		extensionManager.getWorkspaceConfig(),
...
export function onExtensionActiveCommandsChanged(
	listener: () => any,
	thisObject: any | null,
) {
...
export async function updateFullFeatureSet() {
	if (extensionManager) {
		await enableFullFeatureSet(
			extensionManager.workspaceHasAtLeastOneProject(),
		);
	} else {
		...
		log.info(
			`We don't have an extension manager created yet. ` +
