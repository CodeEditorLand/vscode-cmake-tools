...
export class CMakeFileApiDriver extends CMakeDriver {
	...
	async doInit() {
		...
		if (
			cacheExists &&
			this.generator?.name ===
				(await this.getGeneratorFromCache(this.cachePath))
		) {
	...
	private async updateCodeModel(binaryDir?: string): Promise<boolean> {
		...
		const reply_path =
			process.platform === "linux"
				? initialReplyPath.replace("~", process.env.HOME || "./")
				: initialReplyPath;
		...
		if (indexFile) {
			...
			// load cache
			const cache_obj = indexFile.objects.find(
				(value: Index.ObjectKind) => value.kind === "cache",
			);
			...
			this._cache = await loadCacheContent(
				path.join(reply_path, cache_obj.jsonFile),
			);

			// load targets
			const codemodel_obj = indexFile.objects.find(
				(value: Index.ObjectKind) => value.kind === "codemodel",
			);
			...
			// load toolchains
			const toolchains_obj = indexFile.objects.find(
				(value: Index.ObjectKind) => value.kind === "toolchains",
			);
			...
			if (!toolchains_obj) {
				...
				if (this._codeModelContent) {
					this._codeModelContent.toolchains = await loadToolchains(
						path.join(reply_path, toolchains_obj.jsonFile),
					);
				}
			}

			// load cmake files if available
			const cmakefiles_obj = indexFile.objects.find(
				(value: Index.ObjectKind) => value.kind === "cmakeFiles",
			);

			if (cmakefiles_obj) {
				this._cmakeFiles = await loadCMakeFiles(
					path.join(reply_path, cmakefiles_obj.jsonFile),
				);
			} else {
				this._cmakeFiles = [];
			}
	...
	get targets(): Target[] {
		...
		if (targets) {
			...
			return [...metaTargets, ...targets].filter(
				(value, idx, self) =>
					self.findIndex((e) => value.name === e.name) === idx,
			);
		} else {
			return [];
		}
	...
	get executableTargets(): ExecutableTarget[] {
		const uniqueExecTargets = this.uniqueTargets.filter(
			(t) =>
				t.type === "rich" &&
				(t as RichTarget).targetType === "EXECUTABLE",
		);
	...
	private readonly _codeModelChanged =
		new vscode.EventEmitter<null | codeModel.CodeModelContent>();
