...
export interface GlobalSettingsReply
...
export interface SetGlobalSettingsReply
	extends ReplyMessage,
...
export interface CodeModelReply extends ReplyMessage, ServerCodeModelContent {
	inReplyTo: "codemodel";
}
...
export class CMakeServerClient {
	...
	private takePromiseForCookie(
		cookie: string,
	): MessageResolutionCallbacks | undefined {
	...
	private onMessage(some: SomeMessage): void {
		...
		switch (some.type) {
			case "hello": {
				const unlinkPromise = fs
					.exists(this.pipeFilePath)
					.then(async (exists) => {
	...
	private sendRequest(type: string, params: any = {}): Promise<any> {
		...
		const promise = new Promise((resolve, reject) =>
			this.promisesResolvers.set(cookie, { resolve, reject }),
		);
	...
	private constructor(params: ClientInitPrivate) {
		...
		const child = childProc.spawn(
			params.cmakePath,
			["-E", "server", "--experimental", `--pipe=${pipeFile}`],
			{
				env: finalEnv,
				...
				onHello: async (msg: HelloMessage) => {
					...
					try {
						...
						if (haveCache) {
							...
							const tempCache =
								await cache.CMakeCache.fromPath(cachePath);

							const srcDir = tempCache.get(
							...
							const extraMessage: string =
								hsparams.platform || hsparams.toolset
									? localize(
