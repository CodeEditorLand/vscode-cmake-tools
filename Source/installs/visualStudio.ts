...
export async function vsInstallations(): Promise<VSInstallation[]> {
	...
	if (
		...
		cachedVSInstallations.queryTime &&
		now - cachedVSInstallations.queryTime < 900000
	) {
	...
	const vswhereArgs = [
		"/c",
		`${sys32Path}\\chcp 65001>nul && "${vswhereExe}" -all -format json -utf8 -products * -legacy -prerelease`,
	];

	const vswhereResult = await proc.execute(
		`${sys32Path}\\cmd.exe`,
		vswhereArgs,
		null,
		{ silent: true, encoding: "utf8", shell: true },
	).result;
 ...
 */
export function getHostTargetArchString(
	hostArch: string,
	targetArch?: string,
	amd64Alias: boolean = false,
	...
	const version = parsedTargetArch
	...
	targetArch = parsedTargetArch[0].includes("=")
		? hostArch
		: parsedTargetArch[0];
	...
	const archValue =
		hostArch === targetArch ? hostArch : `${hostArch}_${targetArch}`;

	return includeVersion ? `${archValue} ${version}` : archValue;
}

// Gets the MSVC toolsets installed for a given VS install.
export async function enumerateMsvcToolsets(
	vsInstallRoot: string,
	vsVersion: string,
	...
	if (version < 15) {
		...
		if (await fs.exists(toolsetDir)) {
			const dirContents = await fs.readdir(toolsetDir, {
				"withFileTypes": true,
			});
			...
			return dirContents
				...
				.map((dir) => dir.name)
				.sort((a, b) => util.compareVersions(a, b))
				.reverse();
		}
	}
...
export function filterVSInstallationsByMsvcToolset(
 ...
 */
async function collectDevBatVars(
	hostArch: string,
	devBat: string,
	args: string[],
	majorVersion: number,
	...
	for (const envVar of msvcEnvVars) {
		bat.push(
			`if DEFINED ${envVar} echo ${envVar} := %${envVar}% >> ${envFileName}`,
		);
	}
	...
	if (
		...
		const newWinSdkBinPath = path.join(
			WindowsSdkDir,
			"bin",
			WindowsSDKVersion,
			hostArch,
		...
		if (
			newWinSdkBinPathExist &&
			existPath !== "" &&
			existPath.toLowerCase().indexOf(newWinSdkBinPath.toLowerCase()) < 0
		) {
			log.info(
				localize(
					"windows.sdk.path.patch",
					"Patch Windows SDK path from {0} to {1} for {2}",
					oldWinSdkBinPath,
					newWinSdkBinPath,
					devBat,
				),
 ...
 */
export async function getVcVarsBatScript(
	vsInstall: VSInstallation,
	hostArch: string,
	targetArch?: string,
): Promise<string | null> {
 ...
 */
export async function varsForVSInstallation(
	inst: VSInstallation,
	hostArch: string,
	targetArch?: string,
	toolsetVersion?: string,
	...
	log.trace(
		`varsForVSInstallation path:'${inst.installationPath}' version:${inst.installationVersion} host arch:${hostArch} - target arch:${targetArch}`,
	);
	...
	const devBatArgs = [
		getHostTargetArchString(hostArch, targetArch, majorVersion < 15, true),
	];
	...
	if (!variables) {
		...
		if (paths.ninjaPath) {
			...
			if (undefined !== envPATH) {
				...
				const ninjaPathBase = envPaths.find(
					(path) => path === ninjaPath,
				);
