...
export function cmakeify(
	value: string | boolean | number | string[] | CMakeValue,
): CMakeValue {
	...
	if (value === true || value === false) {
		...
		throw new Error(
			localize(
				"invalid.value",
				"Invalid value to convert to cmake value: {0}",
...
async function _killTree(pid: number) {
	if (process.platform !== "win32") {
		...
		const stdout = (
			await execute("pgrep", ["-P", pid.toString()], null, {
				silent: true,
			}).result
		).stdout.trim();
...
export function splitCommandLine(cmd: string): string[] {
	...
	return quoted_args!.map((arg) =>
		arg.replace(/\\(")/g, "$1").replace(/^"(.*)"$/g, "$1"),
	);
}
...
export function makeDebuggerEnvironmentVars(
	env?: Environment,
...
export function fromDebuggerEnvironmentVars(
	debug_env?: DebuggerEnvironmentVariable[],
): Environment {
...
export async function getExtensionLocalizedPackageJson(): Promise<{
	[key: string]: any;
}> {
	let localizedFilePath: string = path.join(
		thisExtensionPath(),
		`package.nls.${getLocaleId()}.json`,
	);
...
export function thisExtensionActiveCommands(context: {
	[key: string]: any;
}): string[] {
	const pkg = thisExtension().packageJSON;

	const allCommands = pkg.contributes.menus
...
export function versionGreater(
	lhs: Version | string,
	rhs: Version | string,
): boolean {
...
export function versionGreaterOrEquals(
	lhs: Version | string,
	rhs: Version | string,
): boolean {
...
export function versionLessOrEquals(
	lhs: Version | string,
	rhs: Version | string,
): boolean {
	const ordering = compareVersions(lhs, rhs);
...
export function lexicographicalCompare(
	a: Iterable<string>,
	b: Iterable<string>,
): number {
...
export function getLocaleId(): string {
	if (typeof process.env.VSCODE_NLS_CONFIG === "string") {
		const vscodeNlsConfigJson: any = JSON.parse(
...
export function chokidarOnAnyChange(
	watcher: chokidar.FSWatcher,
	listener: (path: string, stats?: fs.Stats | undefined) => void,
) {
	return watcher
		.on("add", listener)
		.on("change", listener)
...
export async function normalizeAndVerifySourceDir(
	sourceDir: string,
	...
	if (
		...
		result.length > 1 &&
		result.charCodeAt(0) > 97 &&
		result.charCodeAt(0) <= 122 &&
		result[1] === ":"
	) {
	...
	if (!(await checkDirectoryExists(result))) {
		rollbar.error(
			localize(
				"sourcedirectory.not.a.directory",
...
export async function getAllCMakeListsPaths(
	path: string,
...
export function getRelativePath(file: string, dir: string): string {
	...
	const relPathDir: string = lightNormalizePath(
		path.relative(dir, fullPathDir),
	);
...
export function isFileInsideFolder(
	uri: vscode.Uri,
	folderPath: string,
): boolean {
