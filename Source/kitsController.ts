...
export class KitsController {
	...
	private constructor(
		readonly project: CMakeProject,
		private readonly _kitsWatcher: chokidar.FSWatcher,
	) {}
	...
	static async init(project: CMakeProject) {
		...
		const folderKitsFiles: string[] = [
			KitsController._workspaceKitsPath(project.workspaceFolder),
		].concat(expandedAdditionalKitFiles);

		const kitsWatcher = chokidar.watch(folderKitsFiles, {
			ignoreInitial: true,
	...
	get availableKits() {
		console.assert(
			KitsController.length > 0,
			"readKits should have been called at least once before.",
		);

		if (this.project.workspaceContext.config.showSystemKits) {
			return KitsController.specialKits.concat(
				this.folderKits.concat(
					this.additionalKits.concat(KitsController.userKits),
				),
			);
		} else {
	...
	static async readUserKits(
		project: CMakeProject | undefined,
		...
		try {
			if (
				(await fs.exists(OLD_USER_KITS_FILEPATH)) &&
				!(await fs.exists(USER_KITS_FILEPATH))
			) {
				rollbar.info(
			...
			rollbar.exception(
				localize(
					"failed.to.migrate.kits.file",
					"Failed to migrate prior user-local kits file.",
				),
				e,
				{ from: OLD_USER_KITS_FILEPATH, to: USER_KITS_FILEPATH },
			);
		...
		KitsController.userKits = await readKitsFile(
			USER_KITS_FILEPATH,
			project.workspaceContext.folder.uri.fsPath,
			await project.getExpansionOptions(),
		);
	 ...
	 */
	async readKits(
		kitsReadMode = KitsReadMode.allAvailable,
		progress?: ProgressHandle,
	) {
		if (
			kitsReadMode === KitsReadMode.userKits ||
			kitsReadMode === KitsReadMode.allAvailable
		) {
		...
		if (
			kitsReadMode === KitsReadMode.folderKits ||
			kitsReadMode === KitsReadMode.allAvailable
		) {
			// Read default folder kits
			this.folderKits = await readKitsFile(
				KitsController._workspaceKitsPath(this.workspaceFolder),
				this.project.workspaceContext.folder.uri.fsPath,
				await this.project.getExpansionOptions(),
			);
		...
		if (current) {
			const searchKits: Kit[] =
				kitsReadMode === KitsReadMode.allAvailable
					? this.availableKits
					: kitsReadMode === KitsReadMode.userKits
						? KitsController.userKits
						: this.folderKits.concat(this.additionalKits);

			const already_active_kit = searchKits.find(
				(kit) => kit.name === current.name,
			);
	...
	async checkHaveKits(): Promise<boolean> {
		...
		if (!avail.find((kit) => kit.name === SpecialKits.Unspecified)) {
			...
			rollbar.error(
				localize(
					"invalid.only.kit",
					"Invalid only kit. Expected to find {0}",
		...
		if (!KitsController.checkingHaveKits) {
			...
			rollbar.error(
				localize(
					"already.checking.kits",
					"Already checking kits. Please try again later.",
				),
			);
	...
	private _pickKitCancellationTokenSource: vscode.CancellationTokenSource =
	...
	async selectKit(): Promise<boolean> {
		...
		log.debug(
			localize(
				...
				"Start selection of kits. Found {0} kits.",
				avail.length,
			),
		);
		...
		log.debug(
			localize(
				...
				"Opening kit selection QuickPick",
			),
		);
	...
	private static async _writeUserKitsFile(cmakePath: string, kits: Kit[]) {
		...
		const stripped_kits = kits.filter(
			(kit) =>
				kit.name !== SpecialKits.ScanForKits &&
		...
		try {
			...
			await fs.writeFile(
				USER_KITS_FILEPATH,
				JSON.stringify(sorted_kits, null, 2),
			);
		} catch (e: any) {
	...
	static async scanForKits(cmakePath: string) {
		...
		// Do the scan:
		const discovered_kits = await scanForKits(cmakePath, {
		...
		const new_definition_user_kits = KitsController.userKits.filter(
			(kit) => !!!kit.visualStudio,
		);

		// The VS kits saved so far in cmake-tools-kits.json
		const user_vs_kits = KitsController.userKits.filter(
			(kit) => !!kit.visualStudio,
		);
		...
		user_vs_kits.forEach((kit) => {
			if (
				kit.visualStudio &&
				(kit.visualStudio.startsWith("VisualStudio.15") ||
					kit.visualStudio.startsWith("VisualStudio.16"))
			) {
		...
		if (old_definition_vs_kits.length > 1) {
			log.info(
				localize(
					"found.duplicate.kits",
					"Found Visual Studio kits with the old ids saved in the cmake-tools-kits.json.",
				),
			);
			...
			const chosen =
				await vscode.window.showInformationMessage<vscode.MessageItem>(
					localize(
						"delete.duplicate.kits",
						"Would you like to delete the duplicate Visual Studio kits from cmake-tools-kits.json?",
					),
					{
		...
		const new_kits_by_name = discovered_kits.reduce(
			(acc, kit) =>
				KitsController.isBetterMatch(kit, acc[kit.name])
					? { ...acc, [kit.name]: kit }
					: acc,
			old_kits_by_name,
		);
	...
	static isBetterClangCLDefinition(newKit: Kit, existingKit?: Kit): boolean {
		if (newKit.name.indexOf("MSVC CLI") >= 0) {
			return (
				existingKit?.visualStudioArchitecture === "amd64" ||
				existingKit?.preferredGenerator?.platform === "amd64" ||
				(existingKit?.preferredGenerator?.toolset !== undefined &&
					existingKit.preferredGenerator.toolset.indexOf("amd64") >=
						0)
			);
		}
	...
	static isBetterCompilerMatch(
		newCompilers?: { [lang: string]: string },
		existingCompilers?: { [lang: string]: string },
	): boolean {
		...
		if (newCompilers) {
			...
			if (path && newLangs.length === existingLangs.length) {
				...
				for (const p of path) {
					const newScore = newLangs.reduce(
						(acc, lang) =>
							newCompilers[lang]?.startsWith(p) ? 1 + acc : acc,
						0,
					);

					const existingScore = existingLangs.reduce(
						(acc, lang) =>
							existingCompilers[lang]?.startsWith(p)
								? 1 + acc
								: acc,
						0,
					);
