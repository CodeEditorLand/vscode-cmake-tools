...
export function execute(
	...
	const encoding =
		options.outputEncoding && iconv.encodingExists(options.outputEncoding)
			...
			rollbar.invoke(
				...
				() => {
					...
					const lines = str
						.split("\n")
						.map((l) =>
							l.endsWith("\r") ? l.substr(0, l.length - 1) : l,
						);
		...
		child?.on("close", (retc) => {
			try {
				rollbar.invoke(
					...
					{ line_acc, stderr_line_acc, command, retc },
					() => {
