...
import {
	DebugOrigin,
	logCMakeDebuggerTelemetry,
} from "@cmt/debug/cmakeDebugger/cmakeDebuggerTelemetry";
import {
	DebuggerInformation,
	getDebuggerPipeName,
...
export class DebugAdapterNamedPipeServerDescriptorFactory
	implements vscode.DebugAdapterDescriptorFactory
{
	async createDebugAdapterDescriptor(
		...
		if (cmakeDebugType === "configure" || cmakeDebugType === "script") {
			...
			if (cmakeDebugType === "script") {
				...
				if (!fs.existsSync(script)) {
					throw new Error(
						localize(
							"cmake.debug.scriptPath.does.not.exist",
							'The script path, "{0}", could not be found.',
							script,
						),
					);
				}
				...
				if (
					...
					!Object.values(ConfigureTrigger).includes(
						session.configuration.trigger,
					)
				) {
				...
				if (session.configuration.clean) {
					...
					if (session.configuration.configureAll) {
						...
						void extensionManager?.configureWithDebuggerInternal(
							debuggerInformation,
							session.configuration.folder,
							session.configuration.sourceDir,
							session.configuration.trigger,
						);
					}
				}
		...
		logger.info(
			localize(
				"debugger.create.descriptor",
				'Connecting debugger on named pipe: "{0}"',
				pipeName,
			),
		);
