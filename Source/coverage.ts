...
export async function handleCoverageInfoFiles(
	run: vscode.TestRun,
	coverageInfoFiles: string[],
	coverageData: WeakMap<vscode.FileCoverage, vscode.FileCoverageDetail[]>,
) {
	for (const coverageInfoFile of coverageInfoFiles) {
		let contents: Uint8Array;
		try {
			contents = await vscode.workspace.fs.readFile(
				vscode.Uri.file(coverageInfoFile),
			);
		} catch (e) {
			log.warning(
				localize(
					"test.openCoverageInfoFile",
					"Could not open coverage info file: {0}. Skipping...",
					coverageInfoFile,
				),
			);
			return;
