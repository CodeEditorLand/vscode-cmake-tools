...
async function createGDBDebugConfiguration(
	debuggerPath: string,
...
async function createLLDBDebugConfiguration(
	debuggerPath: string,
	target: ExecutableTarget,
): Promise<VSCodeDebugConfiguration> {
	if (!(await checkDebugger(debuggerPath))) {
...
function createMsvcDebugConfiguration(
	target: ExecutableTarget,
...
export async function getDebugConfigurationFromCache(
	cache: CMakeCache,
	target: ExecutableTarget,
	platform: string,
	modeOverride?: MIModes,
	...
	if (debuggerPathOverride) {
		if (
			path.isAbsolute(debuggerPathOverride) &&
			(await fs.exists(debuggerPathOverride))
		) {
	...
	if (compilerPath === null) {
		...
		); // MSVC should be already found by CMAKE_LINKER
	}
	...
	if (modeOverride !== MIModes.gdb) {
		...
		if (modeOverride === MIModes.lldb || lldbMIReplaced) {
			...
			if (
				...
				return createLLDBDebugConfiguration(
					cpptoolsDebuggerPath,
					target,
				);
			}
	...
	if (
		...
		(await checkDebugger(miDebuggerPath))
	) {
