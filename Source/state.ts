...
export class StateManager {
	...
	private _get<T>(
		...
		return isMultiProject
			? this.extensionContext.globalState.get<T>(
					this.folder.uri.fsPath + `${folderName} ` + key,
				)
			: this.extensionContext.globalState.get<T>(
					this.folder.uri.fsPath + key,
				);
	}
	...
	async setIgnoreCMakeListsMissing(
		folderName: string,
		v: boolean,
		isMultiProject: boolean,
	) {
	...
	private getCachedConfigurePresets(
		folderName: string,
		isMultiProject: boolean,
	): string[] {
		return (
	...
	private async addCachedConfigurePreset(
		folderName: string,
		preset: string,
		isMultiProject: boolean,
	) {
		const configurePresets = this.getCachedConfigurePresets(
			folderName,
			isMultiProject,
		);
	...
	async setBuildPresetName(
		folderName: string,
		configurePreset: string,
		v: string | null,
		isMultiProject: boolean,
	) {
		await this.addCachedConfigurePreset(
			folderName,
			configurePreset,
			isMultiProject,
		);
	...
	getActiveKitName(
		...
		return (
			this._get<string>("activeKitName", folderName, isMultiProject) ||
			null
		);
	}

	async setActiveKitName(
		folderName: string,
		v: string | null,
		isMultiProject: boolean,
	) {
	...
	getLaunchTargetName(
		...
		return (
			this._get<string>("launchTargetName", folderName, isMultiProject) ||
			null
		);
	}

	async setLaunchTargetName(
		folderName: string,
		t: string | null,
		isMultiProject: boolean,
	) {
	...
	getActiveVariantSettings(
		...
		const pairs = this._get<[string, string][]>(
	...
	async setActiveVariantSettings(
		folderName: string,
