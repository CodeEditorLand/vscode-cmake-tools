...
import { errorToString, fixPaths, replaceAll } from "@cmt/util";
...
export interface PresetContextVars
...
export interface ExpansionOptions {
	...
	vars:
		...
		| PresetContextNotPresetSpecificVars;
	/**
 ...
 */
export async function expandString<T>(
	input: string | T,
	...
	try {
		...
		do {
			...
			const expansion = await expandStringHelper(
				result,
				opts,
				errorHandler,
			);
...
async function expandStringHelper(
	...
	if (opts.variantVars) {
		...
		const variantRegex = RegExp(
			`\\$\\{variant:(${varValueRegexp})\\}`,
			"g",
		);
...
export function getParentEnvSubstitutions(
	input: string,
	...
	for (const mat of matchAll(input, parentEnvRegex)) {
		...
		const replacementValue = penvOverride
			? penvOverride[varName]
			: process.env[varName];
...
export function errorHandlerHelper(
	presetName: string,
	errorHandler?: ExpansionErrorHandler,
) {
	if (errorHandler) {
