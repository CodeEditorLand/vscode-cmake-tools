...
export async function getCMakeExecutableInformation(
	path: string,
): Promise<CMakeExecutable> {
	...
	if (path && path.length !== 0) {
		...
		if (cmakeInfo.has(normalizedPath)) {
			...
			if (cmakeExe.isPresent) {
				...
				log.error(
					localize(
						"cmake.exe.not.found.in.cache",
						"CMake executable not found in cache. Checking again.",
					),
				);
			}
		}
		...
		try {
			...
			if (execVersion.retc === 0 && execVersion.stdout) {
				...
				// server bugs
				cmake.isServerModeSupported = util.versionGreater(
					cmake.version,
					cmake.minimalServerModeVersion,
				);

				// Support for new file based API, it replace the server mode
				cmake.isFileApiModeSupported = util.versionGreaterOrEquals(
					cmake.version,
				...
				cmake.isDefaultGeneratorSupported = util.versionGreaterOrEquals(
					cmake.version,
...
export async function setCMakeDebuggerAvailableContext(
	value: boolean,
): Promise<void> {
