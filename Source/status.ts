...
abstract class Button {
	...
	private _getVisibilitySetting():
		...
		if (this.settingsName) {
			let setting = Object(this.config.options.advanced)[
				this.settingsName
			]?.statusBarVisibility;

			if (setting === "inherit") {
				if (this.config.options.statusBarVisibility === "hidden") {
					setting = Object(this.config.options.advanced)[
						this.settingsName
					]?.inheritDefault;
				} else {
...
class VariantStatus extends Button {
	...
	protected getTooltipShort(): string | null {
		return this.prependCMake(
			`${this.bracketText} - ${this._statusMessage}\n${this.tooltip}`,
		);
	}
}

class KitSelection extends Button {
	private static readonly _noActiveKit = localize(
		"no.active.kit",
		"No active kit",
	);
...
class BuildTargetSelectionButton extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.buildTarget?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
class LaunchTargetSelectionButton extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.launchTarget?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class ConfigurePresetSelection extends Button {
	private static readonly _noPresetSelected = localize(
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.configurePreset?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class BuildPresetSelection extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.buildPreset?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class TestPresetSelection extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.testPreset?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class PackagePresetSelection extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.packagePreset?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class WorkflowPresetSelection extends Button {
	...
	protected getTextShort(): string {
		let len =
			this.config.options.advanced?.workflowPreset?.statusBarLength || 0;
		if (!Number.isInteger(len) || len <= 0) {
			len = 20;
		}
		let text = this.getTextNormal();
...
export class StatusBar implements vscode.Disposable {
	...
	private readonly _configurePresetButton = new ConfigurePresetSelection(
		this._config,
		3.55,
	);
	...
	private readonly _debugButton: DebugButton = new DebugButton(
		this._config,
		3.22,
	);
	...
	private readonly _testPresetButton = new TestPresetSelection(
		this._config,
		3.15,
	);
