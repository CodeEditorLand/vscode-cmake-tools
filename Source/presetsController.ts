...
export class PresetsController {
	...
	private async resetPresetsFile(
		...
		if (presetsFile) {
			...
			setOriginalPresetsFile(
				this.folderPath,
				await this.parsePresetsFile(presetsFileBuffer, file),
			);
		} else {
	...
	async getAllConfigurePresets(): Promise<preset.ConfigurePreset[]> {
		return preset
			.configurePresets(this.folderPath)
			.concat(preset.userConfigurePresets(this.folderPath));
	}

	async getAllBuildPresets(): Promise<preset.BuildPreset[]> {
		return preset
			.buildPresets(this.folderPath)
			.concat(preset.userBuildPresets(this.folderPath));
	}

	async getAllTestPresets(): Promise<preset.TestPreset[]> {
		return preset
			.testPresets(this.folderPath)
			.concat(preset.userTestPresets(this.folderPath));
	}

	async getAllPackagePresets(): Promise<preset.PackagePreset[]> {
		return preset
			.packagePresets(this.folderPath)
			.concat(preset.userPackagePresets(this.folderPath));
	}

	async getAllWorkflowPresets(): Promise<preset.WorkflowPreset[]> {
		return preset
			.workflowPresets(this.folderPath)
			.concat(preset.userWorkflowPresets(this.folderPath));
	}
	...
	private async mergeIncludeFiles(
		rootPresetsFile: preset.PresetsFile | undefined,
		presetsFile: preset.PresetsFile | undefined,
		file: string,
		referencedFiles: Set<string>,
	): Promise<void> {
		...
		for (let i = presetsFile.include.length - 1; i >= 0; i--) {
			...
			if (includeFile.cmakeMinimumRequired) {
				if (
					!rootPresetsFile.cmakeMinimumRequired ||
					util.versionLess(
						rootPresetsFile.cmakeMinimumRequired,
			...
			if (includeFile.configurePresets) {
				rootPresetsFile.configurePresets =
					includeFile.configurePresets.concat(
						rootPresetsFile.configurePresets || [],
					);
			}
			if (includeFile.buildPresets) {
				rootPresetsFile.buildPresets = includeFile.buildPresets.concat(
					rootPresetsFile.buildPresets || [],
				);
			}
			if (includeFile.testPresets) {
				rootPresetsFile.testPresets = includeFile.testPresets.concat(
					rootPresetsFile.testPresets || [],
				);
			}
			if (includeFile.packagePresets) {
				rootPresetsFile.packagePresets =
					includeFile.packagePresets.concat(
						rootPresetsFile.packagePresets || [],
					);
			}
			if (includeFile.workflowPresets) {
				rootPresetsFile.workflowPresets =
					includeFile.workflowPresets.concat(
						rootPresetsFile.workflowPresets || [],
					);
			}
	...
	private async expandPresetsFile(
		...
		const expandedConfigurePresets = (
			await Promise.all(
				(presetsFile?.configurePresets || []).map(
		...
		const expandedBuildPresets = (
			await Promise.all(
				(presetsFile?.buildPresets || []).map(async (buildPreset) =>
		...
		const expandedPackagePresets = (
			await Promise.all(
				(presetsFile?.packagePresets || []).map(async (packagePreset) =>
		...
		const expandedTestPresets = (
			await Promise.all(
				(presetsFile?.testPresets || []).map(async (testPreset) =>
		...
		const expandedWorkflowPresets = (
			await Promise.all(
				(presetsFile?.workflowPresets || []).map(
					async (workflowPreset) =>
	...
	private async watchPresetsChange() {
		...
		this._presetsWatcher = chokidar
			.watch(this._referencedFiles, { ignoreInitial: true })
			.on("add", handler)
			.on("change", handler)
			.on("unlink", handler);
	}
