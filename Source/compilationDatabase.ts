...
export class CompilationDatabase {
	...
	constructor(infos: CompileCommand[]) {
		this.infoByFilePath = infos.reduce(
			(acc, cur) =>
				acc.set(util.platformNormalizePath(cur.file), {
	...
	public static async fromFilePaths(
			...
			try {
				...
				log.warning(
					localize(
						...
						"Error parsing compilation database {0}: {1}",
						`"${path}"`,
	...
	public static toJson(database: CompilationDatabase | null): string {
		...
		return JSON.stringify(
			[...database.infoByFilePath.values()].map(
