...
export async function loadCacheContent(
	filename: string,
): Promise<Map<string, cache.CacheEntry>> {
	...
	if (
		detectedVersion.major !== expectedVersion.major ||
		detectedVersion.minor < expectedVersion.minor
	) {
		log.warning(
			localize(
...
function findPropertyValue(
	cacheElement: Cache.CMakeCacheEntry,
	name: string,
): string {
	const propertyElement = cacheElement.properties.find(
		(prop) => prop.name === name,
	);
...
function convertFileApiCacheToExtensionCache(
	cmakeCacheContent: Cache.CacheContent,
...
export async function loadCodeModelContent(
	filename: string,
): Promise<CodeModelKind.Content | null> {
	const fileContent = await tryReadFile(filename);
...
export async function loadTargetObject(
	filename: string,
): Promise<CodeModelKind.TargetObject | null> {
	const fileContent = await tryReadFile(filename);
...
async function convertTargetObjectFileToExtensionTarget(
	...
	if (targetObject.artifacts) {
		executablePath = targetObject.artifacts.find((artifact) =>
			artifact.path.endsWith(targetObject.nameOnDisk),
		);

		if (executablePath) {
			executablePath = convertToAbsolutePath(
				executablePath.path,
				buildDirectory,
			);
			...
			if (targetObject.install) {
				installPaths.push(
					...targetObject.install.destinations.map((dest) => ({
						path: convertToAbsolutePath(
							path.join(dest.path, targetObject.nameOnDisk),
							targetObject.install!.prefix.path,
						),
						subPath: dest.path,
					})),
				);
...
export async function loadAllTargetsForBuildTypeConfiguration(
	replyPath: string,
	buildDirectory: string,
	configuration: CodeModelKind.Configuration,
): Promise<{ name: string; targets: Target[] }> {
	const metaTargets = [];
	...
	const targetsList = await Promise.all(
		configuration.targets.map((t) =>
			convertTargetObjectFileToExtensionTarget(
				buildDirectory,
				path.join(replyPath, t.jsonFile),
			),
		),
	);
...
export async function loadConfigurationTargetMap(
	replyPath: string,
	codeModelFileName: string,
): Promise<Map<string, Target[]>> {
	const codeModelContent = await loadCodeModelContent(
		path.join(replyPath, codeModelFileName),
	);
	...
	const targets = await Promise.all(
		codeModelContent.configurations.map((config) =>
			loadAllTargetsForBuildTypeConfiguration(
				replyPath,
				buildDirectory,
				config,
			),
...
function convertToAbsolutePath(inputPath: string, basePath: string) {
	...
	const absolutePath = path.isAbsolute(inputPath)
		? inputPath
		: path.join(basePath, inputPath);

	return path.normalize(absolutePath);
}

function convertToExtCodeModelFileGroup(
	targetObject: CodeModelKind.TargetObject,
	rootPaths: CodeModelKind.PathInfo,
): CodeModelFileGroup[] {
	const fileGroup: CodeModelFileGroup[] = !targetObject.compileGroups
		? []
		: targetObject.compileGroups.map((group) => {
				const compileCommandFragments = group.compileCommandFragments
					? group.compileCommandFragments.map((frag) => frag.fragment)
					: [];
				...
				return {
					...
					defines: group.defines
						? group.defines.map((define) => define.define)
						: [],
	...
	const defaultIndex =
		fileGroup.push({
			sources: [],
			isGenerated: false,
		} as CodeModelFileGroup) - 1;

	const targetRootSource = convertToAbsolutePath(
	...
	targetObject.sources.forEach((sourceFile) => {
		...
		const fileRelativePath = path
			.relative(targetRootSource, fileAbsolutePath)
			.replace("\\", "/");

		if (sourceFile.compileGroupIndex !== undefined) {
			fileGroup[sourceFile.compileGroupIndex].sources.push(
				fileRelativePath,
			);
		} else {
...
async function loadCodeModelTarget(
	rootPaths: CodeModelKind.PathInfo,
	jsonFile: string,
): Promise<CodeModelTarget | null> {
	...
	if (targetObject.compileGroups) {
		const allSysroots = removeEmpty(
			targetObject.compileGroups.map((x) =>
	...
	return {
		...
		sourceDirectory: convertToAbsolutePath(
			targetObject.paths.source,
			rootPaths.source,
		),
		fullName: targetObject.nameOnDisk,
		artifacts: targetObject.artifacts
			? targetObject.artifacts.map((a) =>
					convertToAbsolutePath(
						path.join(targetObject.paths.build, a.path),
						rootPaths.build,
					),
				)
			: [],
...
export async function loadProject(
	...
	const projectPaths = {
		build: project.directoryIndexes
			? path.join(
					rootPaths.build,
					configuration.directories[project.directoryIndexes[0]]
						.build,
				)
			: rootPaths.build,
	...
	const targets = await Promise.all(
		(project.targetIndexes || []).map((targetIndex) =>
			loadCodeModelTarget(
				rootPaths,
				path.join(
					replyPath,
					configuration.targets[targetIndex].jsonFile,
				),
			),
		),
	);
	...
	return {
		name: project.name,
		targets,
		sourceDirectory: projectPaths.source,
	} as CodeModelProject;
}

export async function loadConfig(
	paths: CodeModelKind.PathInfo,
	replyPath: string,
	configuration: CodeModelKind.Configuration,
) {
	const projects = await Promise.all(
		configuration.projects.map((_, index) =>
			loadProject(paths, replyPath, index, configuration),
		),
	);
...
export async function loadExtCodeModelContent(
	replyPath: string,
	codeModelFileName: string,
): Promise<CodeModelContent | null> {
	const codeModelContent = await loadCodeModelContent(
		path.join(replyPath, codeModelFileName),
	);
...
export async function loadToolchains(
	filename: string,
): Promise<Map<string, CodeModelToolchain>> {
	const fileContent = await tryReadFile(filename);
