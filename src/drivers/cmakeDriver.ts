...
export type CMakePreconditionProblemSolver = (
...
export abstract class CMakeDriver implements vscode.Disposable {
		...
		vscode.window.onDidCloseTerminal((closed) => {
			for (const [key, term] of this._compileTerms) {
				if (term === closed) {
					log.debug(
						localize(
							"user.closed.file.compilation.terminal",
							"User closed a file compilation terminal",
						),
					);
	 ...
	 */
	async computeExpandedEnvironment(
		toExpand: Environment,
		...
		for (const entry of Object.entries(toExpand)) {
			env[entry[0]] = await expand.expandString(entry[1], {
				...opts,
				envOverride: expanded,
			});
		}
	...
	async getConfigureEnvironment(
		configurePreset?: preset.ConfigurePreset | null,
		extraEnvironmentVariables?: Environment,
	): Promise<Environment> {
		let envs;
		if (this.useCMakePresets) {
			envs = EnvironmentUtils.create(
				configurePreset
					? configurePreset.environment
					: this._configurePreset?.environment,
			);
			envs = EnvironmentUtils.merge([
				envs,
				await this.computeExpandedEnvironment(
			...
			envs = EnvironmentUtils.merge([
				envs,
				await this.computeExpandedEnvironment(
					this.config.configureEnvironment,
					envs,
				),
			]);
		} else {
			...
			envs = EnvironmentUtils.merge([
				envs,
				await this.computeExpandedEnvironment(this._variantEnv, envs),
			]);
		}
	 ...
	 */
	async getCMakeBuildCommandEnvironment(
		in_env?: Environment,
	): Promise<Environment> {
		if (this.useCMakePresets) {
			let envs = EnvironmentUtils.merge([
				in_env,
				this._buildPreset?.environment,
			]);
			envs = EnvironmentUtils.merge([
				envs,
				await this.computeExpandedEnvironment(
	...
	async getCTestCommandEnvironment(): Promise<Environment> {
		if (this.useCMakePresets) {
			...
			if (
				...
				this.testPreset !== null &&
				checkTestOverridesPresent(this.config)
			) {
				log.info(
					localize(
						"test.with.overrides",
						"NOTE: You are testing with preset {0}, but there are some overrides being applied from your VS Code settings.",
						this.testPreset.displayName ?? this.testPreset.name,
					),
				);
			}
	...
	async getCPackCommandEnvironment(): Promise<Environment> {
		if (this.useCMakePresets) {
			let envs = EnvironmentUtils.create(
				this._packagePreset?.environment,
			);
			envs = EnvironmentUtils.merge([
				envs,
				await this.computeExpandedEnvironment(
			...
			if (
				this.useCMakePresets &&
				this.packagePreset !== null &&
				checkPackageOverridesPresent(this.config)
			) {
				log.info(
					localize(
						"package.with.overrides",
						"NOTE: You are packaging with preset {0}, but there are some overrides being applied from your VS Code settings.",
						this.packagePreset.displayName ??
							this.packagePreset.name,
					),
				);
			}
	...
	get onProgress(): vscode.Event<ProgressMessage> {
		return (_cb: (ev: ProgressMessage) => any) =>
	...
	get expansionOptions(): expand.ExpansionOptions {
		...
		const target: Partial<TargetTriple> =
			parseTargetTriple(this._kitDetect?.triple ?? "") ?? {};
		...
		if (this.variantKeywordSettings) {
			...
			this.variantKeywordSettings.forEach(
				(value: string, key: string) => (variantVars[key] = value),
			);
		}
	...
	static sourceDirExpansionOptions(
	...
	getEffectiveSubprocessEnvironment(
		opts?: proc.ExecutionOptions,
	): Environment {
		const cur_env = process.env;
		const kit_env = this.config.ignoreKitEnv
	...
	async runCompileCommand(cmd: CompileCommand): Promise<vscode.Terminal> {
		...
		if (
			this.useCMakePresets &&
			this._buildPreset &&
			checkBuildOverridesPresent(this.config)
		) {
			log.info(
				localize(
	...
	protected async _cleanPriorConfiguration() {
		...
		const cmake_files = this.config.deleteBuildDirOnCleanConfigure
			? build_dir
	...
	async setConfigurePreset(
		...
		if (configurePreset) {
			log.info(
				localize(
					"switching.to.config.preset",
					"Switching to configure preset: {0}",
			...
			const needs_clean =
				this.binaryDir === newBinaryDir &&
				preset.configurePresetChangeNeedsClean(
					configurePreset,
					this._configurePreset,
				);
			...
			log.info(
				localize(
					"unsetting.config.preset",
					"Unsetting configure preset",
				),
			);
	...
	private async _setConfigurePreset(
		configurePreset: preset.ConfigurePreset | null,
	): Promise<void> {
		this._configurePreset = configurePreset;
		log.debug(
			localize(
				"cmakedriver.config.preset.set.to",
				"CMakeDriver configure preset set to {0}",
				configurePreset?.name || null,
			),
		);
	...
	async setBuildPreset(
		...
		if (buildPreset) {
			log.info(
				localize(
					"switching.to.build.preset",
					"Switching to build preset: {0}",
					buildPreset.name,
				),
			);
		} else {
			log.info(
				localize("unsetting.build.preset", "Unsetting build preset"),
			);
		}
	...
	private async _setBuildPreset(
		buildPreset: preset.BuildPreset | null,
	): Promise<void> {
		this._buildPreset = buildPreset;
		log.debug(
			localize(
				"cmakedriver.build.preset.set.to",
				"CMakeDriver build preset set to {0}",
				buildPreset?.name || null,
			),
		);
	}
		...
		if (testPreset) {
			log.info(
				localize(
					...
					"Switching to test preset: {0}",
					testPreset.name,
				),
			);
		} else {
			log.info(
				localize("unsetting.test.preset", "Unsetting test preset"),
			);
		}
	...
	private async _setTestPreset(
		testPreset: preset.TestPreset | null,
	): Promise<void> {
		this._testPreset = testPreset;
		log.debug(
			localize(
				"cmakedriver.test.preset.set.to",
				"CMakeDriver test preset set to {0}",
				testPreset?.name || null,
			),
		);
	}
	...
	async setPackagePreset(
		...
		if (packagePreset) {
			log.info(
				localize(
					"switching.to.package.preset",
					"Switching to package preset: {0}",
					packagePreset.name,
				),
			);
		} else {
	...
	private async _setPackagePreset(
		packagePreset: preset.PackagePreset | null,
	): Promise<void> {
		this._packagePreset = packagePreset;
		log.debug(
			localize(
				"cmakedriver.package.preset.set.to",
				"CMakeDriver package preset set to {0}",
				packagePreset?.name || null,
			),
		);
	}
	...
	async setWorkflowPreset(
		...
		if (workflowPreset) {
			log.info(
				localize(
					"switching.to.workflow.preset",
					"Switching to workflow preset: {0}",
					workflowPreset.name,
				),
			);
		} else {
	...
	private async _setWorkflowPreset(
		workflowPreset: preset.WorkflowPreset | null,
	): Promise<void> {
		this._workflowPreset = workflowPreset;
		log.debug(
			localize(
				"cmakedriver.workflow.preset.set.to",
				"CMakeDriver workflow preset set to {0}",
				workflowPreset?.name || null,
			),
		);
	}
	...
	async setKit(
		...
		if (this.useCMakePresets) {
			log.info(
				localize(
					"skip.set.kit",
					"Using preset, skip setting kit: {0}",
					kit.name,
				),
			);
			return;
		...
		log.info(
			localize("switching.to.kit", "Switching to kit: {0}", kit.name),
		);
		...
		await this.doSetKit(async () => {
			...
			await this._refreshExpansions();
			const scope = this.workspaceFolder
				? vscode.Uri.file(this.workspaceFolder)
				: undefined;
			const newBinaryDir = util.lightNormalizePath(
				await expand.expandString(
					this.config.buildDirectory(this.isMultiProject, scope),
	...
	private async _setKit(
		...
		log.debug(
			localize(
				"cmakedriver.kit.set.to",
				"CMakeDriver Kit set to {0}",
				kit.name,
			),
		);
		this._kitEnvironmentVariables = await effectiveKitEnvironment(
			kit,
			this.expansionOptions,
		);
		...
		if (
			preferredGenerators.length === 0 &&
			!(
				this.usingFileApi &&
				this.cmake.version &&
				util.versionGreaterOrEquals(
					this.cmake.version,
					this.cmake.minimalDefaultGeneratorVersion,
				) &&
				kit.name === "__unspec__"
			)
		) {
	...
	protected abstract doSetConfigurePreset(
		needsClean: boolean,
		cb: () => Promise<void>,
	): Promise<void>;
	...
	protected abstract doSetPackagePreset(
		cb: () => Promise<void>,
	): Promise<void>;
	protected abstract doSetWorkflowPreset(
		cb: () => Promise<void>,
	): Promise<void>;
	...
	async setVariant(
		opts: VariantOption,
		keywordSetting: Map<string, string> | null,
	) {
		log.debug(
			localize(
				"setting.new.variant",
				"Setting new variant {0}",
				opts.short || "(Unnamed)",
			),
		);
		this._variantBuildType = opts.buildType || this._variantBuildType;
		this._variantConfigureSettings =
			opts.settings || this._variantConfigureSettings;
	...
	private async _refreshExpansions(
		...
		return this.doRefreshExpansions(async () => {
			this.sourceDir = await util.normalizeAndVerifySourceDir(
				this.sourceDirUnexpanded,
				CMakeDriver.sourceDirExpansionOptions(this.workspaceFolder),
			);

			const opts = this.expansionOptions;
			opts.envOverride =
				await this.getConfigureEnvironment(configurePreset);

			if (!this.useCMakePresets) {
				const scope = this.workspaceFolder
					? vscode.Uri.file(this.workspaceFolder)
					: undefined;
				this._binaryDir = util.lightNormalizePath(
					await expand.expandString(
						this.config.buildDirectory(this.isMultiProject, scope),
						opts,
					),
				...
				if (installPrefix) {
					this._installDir = util.lightNormalizePath(
						await expand.expandString(installPrefix, opts),
					);
				}
			}
		});
	...
	get currentBuildType(): string {
		if (this.useCMakePresets) {
			if (
				(this.isMultiConfig || this.isMultiConfFast) &&
				this._buildPreset?.configuration
			) {
				return this._buildPreset.configuration;
			}
			const buildType =
				this._configurePreset?.cacheVariables?.["CMAKE_BUILD_TYPE"];
			if (util.isString(buildType)) {
				return buildType;
			} else if (
				buildType &&
				typeof buildType === "object" &&
				util.isString(buildType.value)
			) {
	...
	get allTargetName(): string {
		...
		if (
			gen &&
			(gen.includes("Visual Studio") ||
				gen.toLowerCase().includes("xcode"))
		) {
			return "ALL_BUILD";
		} else {
			return "all";
	...
	public async testHaveCommand(
		...
		try {
			...
			log.trace(
				localize(
					"command.version.test.return.code",
					"{0} returned code {1}",
					`"${program} ${args.join(" ")}"`,
					nullableValueToString(result.retc),
				),
			);
	...
	isCommonGenerator(genName: string): boolean {
		return (
			genName === "Ninja" ||
			genName === "Ninja Multi-Config" ||
			genName === "MinGW Makefiles" ||
			genName === "NMake Makefiles" ||
			genName === "Unix Makefiles" ||
			genName === "MSYS Makefiles"
		);
	}
	 ...
	 */
	async findBestGenerator(
		preferredGenerators: CMakeGenerator[],
	): Promise<CMakeGenerator | null> {
		log.debug(
			localize(
				"trying.to.detect.generator",
				"Trying to detect generator supported by system",
			),
		);
		...
		for (const gen of preferredGenerators) {
			...
			const generator_present = await (async (): Promise<boolean> => {
				if (gen_name === "Ninja" || gen_name === "Ninja Multi-Config") {
					return (
						...
						this.testHaveCommand("ninja-build")
					);
				}
			...
			if (!generator_present) {
				const vsMatch =
					/^(Visual Studio \d{2} \d{4})($|\sWin64$|\sARM$)/.exec(
						gen_name,
					);
				...
				if (
					gen_name.toLowerCase().startsWith("xcode") &&
					platform === "darwin"
				) {
					return gen;
				}
				...
				if (
					this.cmakeGenerators.indexOf(gen_name) >= 0 &&
					!this.isCommonGenerator(gen_name) &&
					!(
						gen_name.startsWith("Visual Studio") &&
						platform !== "win32"
					)
				) {
					return gen;
	...
	public configOrBuildInProgress(): boolean {
		return (
			this.configInProgress() || this.cmakeBuildRunner.isBuildInProgress()
		);
	}
	...
	public async cleanConfigure(
		...
		extra_args: string[],
		consumer?: proc.OutputConsumer,
		cancelInformation?: ConfigureCancelInformation,
		debuggerInformation?: DebuggerInformation,
	): Promise<ConfigureResult> {
		if (this.isConfigInProgress) {
			await this.preconditionHandler(
		...
		if (this.cmakeBuildRunner.isBuildInProgress()) {
			await this.preconditionHandler(
				CMakePreconditionProblems.BuildIsAlreadyRunning,
			);
			return {
				result: -1,
				resultType: ConfigureResultType.ConfigureInProgress,
			};
		}
	...
	async getCompilerVersion(compilerPath: string): Promise<CompilerInfo> {
		...
		const sortedCompilerAllowList = this.compilerAllowList.sort(
			(a, b) => b.name.length - a.name.length,
		);
		const compiler = sortedCompilerAllowList.find((comp) =>
			compilerName.includes(comp.name),
		);
		...
		if (compiler?.versionOutputRegexp) {
			version =
				(await this.testCompilerVersion(
					compilerName,
					compilerDir,
					compiler?.versionSwitch,
					RegExp(compiler.versionOutputRegexp, "mgi"),
					compiler.captureGroup,
				)) || "unknown";
		} else {
			version = "unknown";
		}
	...
	private getGeneratorNameForTelemetry(
		generator: string | null = this.generatorName,
	): string {
		if (generator) {
			return (
				this.cmakeGenerators.find((g) => generator.startsWith(g)) ??
				"other"
			);
		}
		return "other";
	...
	public shouldUseCachedConfiguration(trigger: ConfigureTrigger): boolean {
		return this.isCacheConfigSupported &&
			!this.isConfiguredAtLeastOnce &&
			trigger === ConfigureTrigger.configureWithCache &&
			!this.config.configureOnOpen
			? true
			: false;
	}

	public async generateConfigArgsFromPreset(
		configPreset: preset.ConfigurePreset,
	): Promise<string[]> {
		...
		return init_cache_flags.concat(
			preset.configureArgs(configPreset),
			await Promise.all(
				this.config.configureArgs.map(async (value) =>
					expand.expandString(value, {
						...this.expansionOptions,
						envOverride: await this.getConfigureEnvironment(),
					}),
				),
			),
		);
	...
	public async generateConfigArgsFromSettings(
		extra_args: string[] = [],
		withoutCmakeSettings: boolean = false,
	): Promise<string[]> {
		// Cache flags will construct the command line for cmake.
		const init_cache_flags = await this.generateInitCacheFlags();
		const initial_common_flags = extra_args.concat(
			this.config.configureArgs,
		);
		const common_flags = initial_common_flags.includes("--warn-unused-cli")
			? initial_common_flags.filter((f) => f !== "--warn-unused-cli")
			: initial_common_flags.concat("--no-warn-unused-cli");
		const define_flags = withoutCmakeSettings
			? []
			: this.generateCMakeSettingsFlags();
		...
		// Expand all flags
		const opts = this.expansionOptions;
		const expanded_flags_promises = final_flags.map(async (value: string) =>
			expand.expandString(value, {
				...opts,
				envOverride: expanded_configure_env,
			}),
		);
	...
	async configure(
		...
		withoutCmakeSettings: boolean = false,
		showCommandOnly?: boolean,
		presetOverride?: preset.ConfigurePreset,
		options?: proc.ExecutionOptions,
	): Promise<ConfigureResult> {
		// Check if the configuration is using cache in the first configuration and adjust the logging messages based on that.
		const shouldUseCachedConfiguration: boolean =
			this.shouldUseCachedConfiguration(trigger);
		...
		if (
			...
			log.debug(
				localize(
					"no.cached.config",
					"No cached config could be used for IntelliSense",
				),
			);
		...
		if (this.cmakeBuildRunner.isBuildInProgress()) {
			...
			return {
				result: -1,
				resultType: ConfigureResultType.BuildInProgress,
			};
		}
		this.isConfigInProgress = true;
		try {
			...
			if (!showCommandOnly) {
				if (!shouldUseCachedConfiguration) {
					log.debug(
						localize("start.configure", "Start configure"),
						extra_args,
					);
				} else {
					log.debug(
			...
			const pre_check_ok =
				await this._beforeConfigureOrBuild(showCommandOnly);
			if (!pre_check_ok) {
			...
			if (this.useCMakePresets) {
				...
				const configurePreset:
					...
					| undefined
					| null =
					trigger === ConfigureTrigger.taskProvider
						? presetOverride
						: this._configurePreset;
				if (!configurePreset) {
					log.debug(
					...
					return {
						result: -3,
						resultType: ConfigureResultType.NoConfigurePreset,
					};
				}
				// For now, fields in presets are expanded when the preset is selected
				expanded_flags =
					await this.generateConfigArgsFromPreset(configurePreset);

				if (
					!showCommandOnly &&
					!shouldUseCachedConfiguration &&
					checkConfigureOverridesPresent(this.config)
				) {
					log.info(
						localize(
							"configure.with.overrides",
							"NOTE: You are configuring with preset {0}, but there are some overrides being applied from your VS Code settings.",
							configurePreset.displayName ?? configurePreset.name,
						),
					);
				}
			} else {
				expanded_flags = await this.generateConfigArgsFromSettings(
					extra_args,
			...
			if (this.useCMakePresets) {
				telemetryProperties = {
					CMakeExecutableVersion: cmakeVersion
						? util.versionToString(cmakeVersion)
						: "",
					CMakeGenerator: this.getGeneratorNameForTelemetry(
						presetOverride?.generator || this.generatorName,
					),
				...
				telemetryProperties = {
					CMakeExecutableVersion: cmakeVersion
						...
						: "",
					CMakeGenerator: this.getGeneratorNameForTelemetry(),
					ConfigType: this.isMultiConfFast
						? "MultiConf"
			...
			if (this._kit?.compilers) {
				...
				if (this._kit.compilers["C"]) {
					cCompilerVersion = await this.getCompilerVersion(
						this._kit.compilers["C"],
					);
				}

				if (this._kit.compilers["CXX"]) {
					cppCompilerVersion = await this.getCompilerVersion(
						this._kit.compilers["CXX"],
					);
				}

				if (cCompilerVersion) {
					telemetryProperties.CCompilerName = cCompilerVersion.name;
					telemetryProperties.CCompilerVersion =
						cCompilerVersion.version;
				}

				if (cppCompilerVersion) {
					telemetryProperties.CppCompilerName =
						cppCompilerVersion.name;
					telemetryProperties.CppCompilerVersion =
						cppCompilerVersion.version;
				}
			} else if (
				this._kit?.visualStudio &&
				...
				if (compilerPath) {
					const compiler =
						await this.getCompilerVersion(compilerPath);
			...
			if (this.useCMakePresets) {
				const arch = presetOverride
					? presetOverride.__developerEnvironmentArchitecture
					: this._configurePreset
						? this._configurePreset
			...
			if (this.useCMakePresets && this.workspaceFolder) {
				const configurePresets = preset.configurePresets(
					this.workspaceFolder,
				);
				const userConfigurePresets = lodash.differenceWith(
					preset.userConfigurePresets(this.workspaceFolder),
					configurePresets,
					(a, b) => a.name === b.name,
				);
				const buildPresets = preset.buildPresets(this.workspaceFolder);
				const userBuildPresets = lodash.differenceWith(
					preset.userBuildPresets(this.workspaceFolder),
					buildPresets,
					(a, b) => a.name === b.name,
				);
				const testPresets = preset.testPresets(this.workspaceFolder);
				const userTestPresets = lodash.differenceWith(
					preset.userTestPresets(this.workspaceFolder),
					testPresets,
					(a, b) => a.name === b.name,
				);
				const packagePresets = preset.packagePresets(
					this.workspaceFolder,
				);
				const userPackagePresets = lodash.differenceWith(
					preset.userPackagePresets(this.workspaceFolder),
					packagePresets,
					(a, b) => a.name === b.name,
				);
				const workflowPresets = preset.workflowPresets(
					this.workspaceFolder,
				);
				const userWorkflowPresets = lodash.differenceWith(
					preset.userWorkflowPresets(this.workspaceFolder),
					workflowPresets,
					(a, b) => a.name === b.name,
				);
				telemetryMeasures["ConfigurePresets"] = configurePresets.length;
				telemetryMeasures["HiddenConfigurePresets"] =
					this.countHiddenPresets(configurePresets);
				telemetryMeasures["UserConfigurePresets"] =
					userConfigurePresets.length;
				telemetryMeasures["HiddenUserConfigurePresets"] =
					this.countHiddenPresets(userConfigurePresets);
				telemetryMeasures["BuildPresets"] = buildPresets.length;
				telemetryMeasures["HiddenBuildPresets"] =
					this.countHiddenPresets(buildPresets);
				telemetryMeasures["UserBuildPresets"] = userBuildPresets.length;
				telemetryMeasures["HiddenUserBuildPresets"] =
					this.countHiddenPresets(userBuildPresets);
				telemetryMeasures["TestPresets"] = testPresets.length;
				telemetryMeasures["HiddenTestPresets"] =
					this.countHiddenPresets(testPresets);
				telemetryMeasures["UserTestPresets"] = userTestPresets.length;
				telemetryMeasures["HiddenUserTestPresets"] =
					this.countHiddenPresets(userTestPresets);
				telemetryMeasures["PackagePresets"] = packagePresets.length;
				telemetryMeasures["HiddenPackagePresets"] =
					this.countHiddenPresets(packagePresets);
				telemetryMeasures["UserPackagePresets"] =
					userPackagePresets.length;
				telemetryMeasures["HiddenUserPackagePresets"] =
					this.countHiddenPresets(userPackagePresets);
				telemetryMeasures["WorkflowPresets"] = workflowPresets.length;
				telemetryMeasures["HiddenWorkflowPresets"] =
					this.countHiddenPresets(workflowPresets);
				telemetryMeasures["UserWorkflowPresets"] =
					userWorkflowPresets.length;
				telemetryMeasures["HiddenUserWorkflowPresets"] =
					this.countHiddenPresets(userWorkflowPresets);
			}
			if (consumer) {
			...
			telemetryProperties.Canceled = cancelInformation?.canceled
				? "true"
				: "false";
			telemetry.logEvent(
				"configure",
			...
			log.info(
				localize("configure.failed", "Failed to configure project"),
			);
			return {
				result: -1,
				resultType: ConfigureResultType.NormalOperation,
	...
	private async generateInitCacheFlags(): Promise<string[]> {
		...
		for (let init of cache_init) {
			init = await expand.expandString(init, {
				...this.expansionOptions,
				envOverride,
			});
	...
	private generateCMakeSettingsFlags(): string[] {
		...
		if (this._variantLinkage !== null) {
			settingMap.BUILD_SHARED_LIBS = util.cmakeify(
				this._variantLinkage === "shared",
			);
		}

		const configurationScope = this.workspaceFolder
			? vscode.Uri.file(this.workspaceFolder)
			: null;
		...
		const allowBuildTypeOnMultiConfig =
			config.get<boolean>("setBuildTypeOnMultiConfig") || false;

		if (
			(!this.isMultiConfFast ||
				(this.isMultiConfFast && allowBuildTypeOnMultiConfig)) &&
			this.currentBuildType !== "Unspecified"
		) {
		...
		try {
			util.objectPairs(this.config.configureSettings).forEach(
				([key, value]) => (settingMap[key] = util.cmakeify(value)),
			);
		} catch (e: any) {
			log.error(e.message);
			throw e;
		}
		util.objectPairs(this._variantConfigureSettings).forEach(
			([key, value]) =>
				(settingMap[key] = util.cmakeify(value as string)),
		);

		// Export compile_commands.json
		const exportCompileCommandsSetting = config.get<boolean>(
			"exportCompileCommandsFile",
		);
		const exportCompileCommandsFile: boolean =
			exportCompileCommandsSetting === undefined
				? true
				: exportCompileCommandsSetting || false;
		settingMap.CMAKE_EXPORT_COMPILE_COMMANDS = util.cmakeify(
			exportCompileCommandsFile,
		);
		...
		if (this._kit.compilers) {
			log.debug(
				localize(
					"using.compilers.in.for.configure",
					"Using compilers in {0} for configure",
					this._kit.name,
				),
			);
			for (const lang in this._kit.compilers) {
				const compiler = this._kit.compilers[lang];
				settingMap[`CMAKE_${lang}_COMPILER`] = {
					type: "FILEPATH",
					value: compiler,
				};
			}
		...
		if (this._kit.toolchainFile) {
			log.debug(
				localize(
					"using.cmake.toolchain.for.configure",
					"Using CMake toolchain {0} for configuring",
					this._kit.name,
				),
			);
			settingMap.CMAKE_TOOLCHAIN_FILE = {
				type: "FILEPATH",
				value: this._kit.toolchainFile,
			};
		}
		if (this._kit.cmakeSettings) {
			util.objectPairs(this._kit.cmakeSettings).forEach(
				([key, value]) =>
					(settingMap[key] = util.cmakeify(value as string)),
			);
		}
	...
	async build(
		...
		log.debug(
			localize("start.build", "Start build"),
			targets?.join(", ") || "",
		);
		if (this.isConfigInProgress) {
			await this.preconditionHandler(
		...
		const timeStart: number = new Date().getTime();
		const child = await this._doCMakeBuild(
			targets,
			consumer,
			isBuildCommand,
		);
		...
		const telemetryProperties: telemetry.Properties | undefined = this
			.useCMakePresets
			? undefined
			: {
					ConfigType: this.isMultiConfFast
		...
		if (child) {
			if (consumer) {
				if (
					...
					// Wrong type: shouldn't get here, just in case
					rollbar.error("Wrong build result type.");
					telemetryMeasures["ErrorCount"] = (await child.result).retc
						? 1
						: 0;
				}
			}
	 ...
	 */
	private async _beforeConfigureOrBuild(
		showCommandOnly?: boolean,
	): Promise<boolean> {
		if (!showCommandOnly) {
			log.debug(
				localize(
					"running.pre-configure.checks",
					"Running pre-configure checks and steps",
				),
			);
		}
		...
		if (!this.sourceDir) {
			...
			await this.preconditionHandler(
				CMakePreconditionProblems.NoSourceDirectoryFound,
			);
			return false;
		}
	 ...
	 */
	private readonly _settingsSub = this.config.onChange(
		"configureSettings",
		async () => this.doConfigureSettingsChange(),
	);
	private readonly _argsSub = this.config.onChange(
		"configureArgs",
		async () => this.doConfigureSettingsChange(),
	);
	private readonly _envSub = this.config.onChange(
		"configureEnvironment",
		async () => this.doConfigureSettingsChange(),
	);
	private readonly _buildArgsSub = this.config.onChange(
		"buildArgs",
		async () => {
			await onBuildSettingsChange();
		},
	);
	private readonly _buildEnvSub = this.config.onChange(
		"buildEnvironment",
		async () => {
			await onBuildSettingsChange();
		},
	);
	private readonly _testArgsSub = this.config.onChange(
		"ctestArgs",
		async () => {
			await onTestSettingsChange();
		},
	);
	private readonly _testEnvSub = this.config.onChange(
		"testEnvironment",
		async () => {
			await onTestSettingsChange();
		},
	);
	private readonly _packEnvSub = this.config.onChange(
		"cpackEnvironment",
		async () => {
			await onPackageSettingsChange();
		},
	);
	private readonly _packArgsSub = this.config.onChange(
		"cpackArgs",
		async () => {
			await onPackageSettingsChange();
		},
	);
	private readonly _generalEnvSub = this.config.onChange(
		"environment",
		async () => {
	...
	async generateBuildCommandFromPreset(
		buildPreset: preset.BuildPreset,
		targets?: string[],
	): Promise<proc.BuildCommand | null> {
		...
		const args = preset.buildArgs(
			...
			this.config.buildToolArgs,
		);
		const initialEnvironment = EnvironmentUtils.create(
			buildPreset.environment,
		);
		const build_env =
			await this.getCMakeBuildCommandEnvironment(initialEnvironment);
		const expanded_args_promises = args.map(async (value: string) =>
			expand.expandString(value, {
				...this.expansionOptions,
				envOverride: build_env,
			}),
		);
		const expanded_args = (await Promise.all(
			expanded_args_promises,
		)) as string[];
		log.trace(
			localize(
				"cmake.build.args.are",
				"CMake build args are: {0}",
				JSON.stringify(args),
			),
		);
		...
		if (checkBuildOverridesPresent(this.config)) {
			log.info(
				localize(
					...
					"NOTE: You are building with preset {0}, but there are some overrides being applied from your VS Code settings.",
	...
	async generateBuildCommandFromSettings(
		targets?: string[],
	): Promise<proc.BuildCommand | null> {
		if (!targets || targets.length === 0) {
			return null;
		...
		const buildToolArgs: string[] = ["--"].concat(
			this.config.buildToolArgs,
		);
		...
		const configurationScope = this.workspaceFolder
			? vscode.Uri.file(this.workspaceFolder)
			: null;
		const parallelJobsSetting = vscode.workspace
			.getConfiguration("cmake", configurationScope)
			.inspect<number | undefined>("parallelJobs");
		let numJobs: number | undefined =
			parallelJobsSetting?.workspaceFolderLanguageValue ||
			parallelJobsSetting?.workspaceFolderValue ||
			parallelJobsSetting?.globalValue;
		...
		if (numJobs && ((gen && !/Visual Studio/.test(gen)) || numJobs > 1)) {
			...
			if (
				...
				if (gen) {
					if (
						/(Unix|MinGW) Makefiles|Ninja/.test(gen) &&
						targets.length === 1 &&
						targets[0] !== "clean"
					) {
						buildToolArgs.push("-j", numJobs.toString());
					} else if (
						/Visual Studio/.test(gen) &&
						targets.length === 1 &&
						targets[0] !== "clean"
					) {
						buildToolArgs.push(
							"/maxcpucount:" + numJobs.toString(),
						);
					}
				}
			}
		...
		const opts = this.expansionOptions;
		const expanded_args_promises = args.map(async (value: string) =>
			expand.expandString(value, { ...opts, envOverride: build_env }),
		);
		const expanded_args = (await Promise.all(
			expanded_args_promises,
		)) as string[];
	...
	async getCMakeBuildCommand(
		targets?: string[],
	): Promise<proc.BuildCommand | null> {
		if (this.useCMakePresets) {
			if (!this._buildPreset) {
				log.debug(
					localize("no.build.preset", "No build preset selected"),
				);
				return null;
			}
			return this.generateBuildCommandFromPreset(
				this._buildPreset,
				targets,
			);
		} else {
	...
	private async _doCMakeBuild(
		targets?: string[],
		consumer?: proc.OutputConsumer,
		isBuildCommand?: boolean,
	): Promise<proc.Subprocess | null> {
		...
		if (buildcmd) {
			...
			const useBuildTask: boolean =
				this.config.buildTask && isBuildCommand === true;
			if (useBuildTask) {
				const task: CMakeTask | undefined =
					await CMakeTaskProvider.findBuildTask(
						this._buildPreset?.name,
						targets,
						this.expansionOptions,
					);
				if (task) {
					const resolvedTask: CMakeTask | undefined =
						await CMakeTaskProvider.resolveInternalTask(task);
					if (resolvedTask) {
						await this.cmakeBuildRunner.setBuildProcessForTask(
				...
				const exeOpt: proc.ExecutionOptions = {
					environment: buildcmd.build_env,
					outputEncoding: outputEnc,
				};
				this.cmakeBuildRunner.setBuildProcess(
					this.executeCommand(
						buildcmd.command,
						buildcmd.args,
						consumer,
	...
	private async _baseInit(
		...
		const initBaseDriverWithPresetLoc = localize(
			"init.driver.using.preset",
			"Initializing base driver using preset",
		);
		const initBaseDriverWithKitLoc = localize(
			"init.driver.using.kit",
			"Initializing base driver using kit",
		);
		log.debug(
			`${useCMakePresets ? initBaseDriverWithPresetLoc : initBaseDriverWithKitLoc}`,
		);
	 ...
	 */
	static async createDerived<T extends CMakeDriver>(
		inst: T,
		useCMakePresets: boolean,
		kit: Kit | null,
		configurePreset: preset.ConfigurePreset | null,
	...
	public getDiagnostics(): DiagnosticsConfiguration {
		return {
			...
			cmakeVersion: this.cmake.version
				? util.versionToString(this.cmake.version)
				: "",
