...
export function cleanStack(stack?: string): string {
	...
	stack = stack.replace(
		...
		(
			...
			fileInfo: string,
			lineColumn: string,
		) => match.replace(fileInfo, `main.js${lineColumn}`),
	);
...
class RollbarController {
	...
	exception(what: string, exception: Error, additional: object = {}): void {
		log.fatal(
			localize("unhandled.exception", "Unhandled exception: {0}", what),
			exception,
			JSON.stringify(additional, (key, value) =>
