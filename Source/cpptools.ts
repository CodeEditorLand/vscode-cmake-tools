...
function parseTargetArch(target: string): Architecture {
	...
	if (
		target.indexOf("aarch64") >= 0 ||
		target.indexOf("arm64") >= 0 ||
		target.indexOf("armv8-a") >= 0 ||
		target.indexOf("armv8.") >= 0
	) {
		return "arm64";
...
export function parseCompileFlags(
	cptVersion: cpptools.Version,
	args: string[],
	lang?: string,
): CompileFlagInformation {
	...
	while (1) {
		...
		if (requireStandardTarget && (lower === "-m32" || lower === "-m64")) {
			...
			if (lang === "CXX" || lang === "OBJCXX" || lang === "CUDA") {
				...
				log.warning(
					localize("unknown language", "Unknown language: {0}", lang),
				);
			}
		}
	}
...
export class CppConfigurationProvider
	implements cpptools.CustomConfigurationProvider
{
	...
	private getConfiguration(
		uri: vscode.Uri,
	): cpptools.SourceFileConfigurationItem | undefined {
	...
	async provideFolderBrowseConfiguration(
		...
		return (
			this.workspaceBrowseConfigurations.get(
				util.platformNormalizePath(uri.fsPath),
			) ?? null
		);
	}
	...
	private readonly fileIndex = new Map<
		string,
		Map<string, cpptools.SourceFileConfigurationItem>
	>();
	...
	private buildConfigurationData(
		...
		const useFragments: boolean =
			this.cpptoolsVersion >= cpptools.Version.v6;
		// If the file didn't have a language, default to C++
		const lang =
			fileGroup.language === "RC" ? undefined : fileGroup.language;
		...
		if ("toolchains" in opts.codeModelContent) {
			compilerToolchains =
				opts.codeModelContent.toolchains?.get(lang ?? "") ||
		...
		const compilerCache =
			opts.cache.get(`CMAKE_${lang}_COMPILER`) ||
			opts.cache.get("CMAKE_CXX_COMPILER") ||
			opts.cache.get("CMAKE_C_COMPILER");
		// Try to get the path to the compiler we want to use
		const compilerPath = compilerToolchains
			? compilerToolchains.path
			: compilerCache
				? compilerCache.as<string>()
				: opts.clCompilerPath;
		if (!compilerPath) {
		...
		const targetArchFromToolchains = targetFromToolchains
			? parseTargetArch(targetFromToolchains)
			: undefined;

		const normalizedCompilerPath = util.platformNormalizePath(compilerPath);
		let compileCommandFragments = useFragments
			? fileGroup.compileCommandFragments ||
				target.compileCommandFragments
			: [];
		...
		if (!useFragments) {
			...
			({ extraDefinitions, standard, targetArch } = parseCompileFlags(
		...
		const includePath = (
			fileGroup.includePath
				? fileGroup.includePath.map((p) => p.path)
				: target.includePath || []
		).concat(frameworkPaths);
		const normalizedIncludePath = includePath.map((p) =>
			util.platformNormalizePath(p),
		);
		...
		this.workspaceBrowseConfiguration = {
			...
			compilerArgs: flags,
			compilerFragments: useFragments
		...
		this.workspaceBrowseConfigurations.set(
			util.platformNormalizePath(opts.folder),
			this.workspaceBrowseConfiguration,
		);
	 ...
	 */
	private updateFileGroup(
		sourceDir: string,
		fileGroup: CodeModelFileGroup,
		options: CodeModelParams,
		target: TargetDefaults,
		sysroot?: string,
	) {
		const configuration = this.buildConfigurationData(
			fileGroup,
			options,
			target,
			sysroot,
		...
		for (const src of fileGroup.sources) {
			...
			const normalizedAbsolutePath =
				util.platformNormalizePath(absolutePath);
	...
	updateConfigurationData(opts: CodeModelParams) {
		...
		for (const config of opts.codeModelContent.configurations) {
			...
			if (
				config.name === opts.activeBuildTypeVariant ||
				(!opts.activeBuildTypeVariant && config.name === "")
			) {
				for (const project of config.projects) {
					for (const target of project.targets) {
						...
						const compileCommandFragments = [
							...util.first(
								grps,
								(grp) => grp.compileCommandFragments || [],
							),
						];
						const defines = [
							...new Set(
								util.flatMap(grps, (grp) => grp.defines || []),
							),
						];
						...
						const filteredGroups =
							target.fileGroups &&
							target.fileGroups.filter((fg) => !fg.isGenerated)
								.length > 0
								? target.fileGroups.filter(
										(fg) => !fg.isGenerated,
									)
								: target.fileGroups;
	...
	getDiagnostics(): DiagnosticsCpptools {
		...
		return {
			...
			executablesCount: this.targets.reduce<number>(
				(acc, target) => (target.type === "EXECUTABLE" ? acc + 1 : acc),
				0,
			),
			librariesCount: this.targets.reduce<number>(
				(acc, target) =>
					target.type.endsWith("LIBRARY") ? acc + 1 : acc,
				0,
			),
