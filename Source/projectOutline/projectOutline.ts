...
export class DirectoryNode<Node extends BaseNode> extends BaseNode {
	...
	update<InputItem extends NamedItem>(opts: {
		...
		for (const new_subdir of opts.tree.children) {
			...
			if (!existing) {
				existing = new DirectoryNode<Node>(
					this.id,
					this.fsPath,
					new_subdir.pathPart,
				);
...
export class SourceFileNode extends BaseNode {
	constructor(
		...
		super(
			`${prefix}::${path.basename(filePath)}:${path.relative(sourcePath, path.dirname(filePath))}`,
		);
	}
	...
	update(cm: codeModel.CodeModelTarget, ctx: TreeUpdateContext) {
		...
		addToTree(
			...
			new SourceFileNode(
				...
				this.folder,
				this.sourceDir,
				path.join(this.sourceDir, "CMakeLists.txt"),
			),
		);
...
export class ProjectNode extends BaseNode {
	...
	getChildren() {
		const children: BaseNode[] = this.sortProjectChildren(
			this._rootDir.getChildren(),
		);

		const cmakelists = new SourceFileNode(
			this.id,
			this.folder,
		...
		const possibleUserPreset = path.join(
			this.sourceDirectory,
...
export class WorkspaceFolderNode extends BaseNode {
	...
	private getNode(cmakeProject: CMakeProject, modelProjectName: string) {
		return this._projects
			.get(cmakeProject.folderPath)
			?.get(modelProjectName);
	}
	...
	updateCodeModel(
		cmakeProject: CMakeProject,
		model: codeModel.CodeModelContent | null,
		ctx: TreeUpdateContext,
	) {
...
export class ProjectOutline implements vscode.TreeDataProvider<BaseNode> {
	...
	updateCodeModel(
		...
		if (!existing) {
			rollbar.error(
				localize(
					"error.update.code.model.on.nonexist.folder",
					"Updating code model on folder that has not yet been loaded.",
				),
			);
	...
	getChildren(node?: BaseNode): BaseNode[] {
		try {
			...
			rollbar.error(
				localize(
					"error.rendering.children.nodes",
					"Error while rendering children nodes",
				),
			);
