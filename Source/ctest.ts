		...
		Testing: {
			...
			Test: {
				...
				Results: {
					...
					Measurement: {
						Value: [EncodedMeasurementValue | string];
					}[];
				}[];
			}[];
		}[];
...
function decodeOutputMeasurement(
	node: EncodedMeasurementValue | string,
): string {
	if (typeof node === "string") {
		return node;
	}
	let buffer = !!node.$.encoding
		? Buffer.from(node._, node.$.encoding)
		: Buffer.from(node._, "utf-8");
	if (!!node.$.compression) {
...
function cleanupResultsXml(messy: MessyResults): CTestResults {
	...
	if (
		testingHead.TestList.length === 1 &&
		(testingHead.TestList[0] as any as string) === ""
	) {
		...
		site: {
			...
			testing: {
				...
				test: testingHead.Test.map((test): Test => {
					...
					for (const namedMeasurement of test.Results[0]
						.NamedMeasurement) {
						measurements.set(namedMeasurement.$.name, {
							type: namedMeasurement.$.type,
							name: namedMeasurement.$.name,
							value: decodeOutputMeasurement(
								namedMeasurement.Value[0],
							),
						});
					}
					return {
						...
						status: test.$.Status,
						measurements,
						output: decodeOutputMeasurement(
							test.Results[0].Measurement[0].Value[0],
						),
					};
				}),
			},
...
export async function readTestResultsFile(
	testXml: string,
): Promise<CTestResults | undefined> {
	try {
...
export class CTestDriver implements vscode.Disposable {
	...
	private coverageData = new WeakMap<
		vscode.FileCoverage,
	...
	private readonly testsChangedEmitter = new vscode.EventEmitter<
		CTestInfo | undefined
	>();
	readonly onTestsChanged = this.testsChangedEmitter.event;

	private testItemCollectionToArray(
	...
	private async getCTestArgs(
		driver: CMakeDriver,
		customizedTask: boolean = false,
		testPreset?: TestPreset,
	): Promise<string[] | undefined> {
		let ctestArgs: string[];
		const opts = driver.expansionOptions;
		const initialArgs = await Promise.all(
			this.ws.config.ctestDefaultArgs.map(async (value) =>
				expandString(value, driver.expansionOptions),
			),
		);
		const additionalArgs = await Promise.all(
			this.ws.config.ctestArgs.map(async (value) =>
				expandString(value, driver.expansionOptions),
			),
		);
	...
	public async runCTest(
		...
		if (this.ws.config.testExplorerIntegrationEnabled) {
			...
			const ctestArgs = await this.getCTestArgs(
				driver,
				customizedTask,
				testPreset,
			);
			const returnCode = await this.runCTestHelper(
				tests,
				run,
				run.token,
				driver,
			...
			return vscode.window.withProgress(
				...
				async (progress, cancel) => {
					progress.report({
						message: localize("running.tests", "Running tests"),
					});
					return this.runCTestDirectly(
						driver,
						customizedTask,
						cancel,
						testPreset,
	...
	private async runCTestDirectly(
		driver: CMakeDriver,
		customizedTask: boolean = false,
		...
		const ctestpath = await this.ws.getCTestPath(
		...
		const ctestArgs =
			(await this.getCTestArgs(driver, customizedTask, testPreset)) || [];
		...
		const testResults = await this.runCTestImpl(
			driver,
			ctestpath,
			ctestArgs,
			cancellationToken,
	 ...
	 * @returns : the driver
	 */
	private async getProjectDriver(
		test: vscode.TestItem,
	): Promise<CMakeDriver> {
		const folder = this.getTestRootFolder(test);
		const project =
			await this.projectController?.getProjectForFolder(folder);
		if (!project) {
			throw new Error(
				localize(
					"no.project.found",
	 ...
	 *
	 */
	private async fillDriverMap(
		tests: vscode.TestItem[],
		run: vscode.TestRun,
		cancellation: vscode.CancellationToken,
		driverMap?: DriverMapT,
		driver?: CMakeDriver,
		ctestPath?: string,
		ctestArgs?: string[],
		customizedTask: boolean = false,
	): Promise<void> {
		let _driverMap = new Map<
			string,
			{
				driver: CMakeDriver;
				ctestPath: string;
		...
		for (const test of tests) {
			...
			if (ctestPath) {
				...
				_ctestPath = await this.ws.getCTestPath(
					_driver.cmakePathFromPreset,
				);
				if (_ctestPath === null) {
					this.ctestErrored(test, run, {
						message: localize(
							"ctest.path.not.set",
			...
			if (test.children.size > 0) {
				...
				if (!_driverMap.has(_driver.sourceDir)) {
					_driverMap.set(_driver.sourceDir, {
						driver: _driver,
						ctestPath: _ctestPath,
						ctestArgs: _ctestArgs,
						tests: [test],
					});
				} else {
					...
					_driverMap.set(_driver.sourceDir, {
						driver: d!.driver,
						ctestPath: d!.ctestPath,
						ctestArgs: d!.ctestArgs,
						tests: d!.tests.concat(test),
					});
				}
			}
		}
	...
	private async runCTestHelper(
		...
		customizedTask: boolean = false,
		consumer?: proc.OutputConsumer,
		entryPoint: RunCTestHelperEntryPoint = RunCTestHelperEntryPoint.RunTests,
	): Promise<number> {
		let returnCode: number = 0;
		...
		if (!this.ws.config.ctestAllowParallelJobs) {
			for (const driver of driverMap.values()) {
				for (const test of driver.tests) {
					...
					const _ctestArgs = driver.ctestArgs.concat(
						"-R",
						`^${util.escapeStringForRegex(test.id)}\$`,
					);
					...
					if (testResults) {
						const testResult = testResults.site.testing.test.find(
							(t) => t.name === test.id,
						);
						if (testResult) {
							returnCode = this.testResultsAnalysis(
								testResult,
								test,
								returnCode,
								run,
			...
			for (const driver of driverMap.values()) {
				...
				if (
					uniqueCtestArgs.filter((arg) => arg.startsWith("-j"))
						.length === 0
				) {
				...
				if (
					entryPoint === RunCTestHelperEntryPoint.TestExplorer &&
					testExplorer &&
					this._tests &&
					this._tests.tests.length !== driver.tests.length
				) {
				...
				if (testResults) {
					for (
						let i = 0;
						i < testResults.site.testing.test.length;
						i++
					) {
						const _test = driver.tests.find(
							(t) =>
								t.id === testResults.site.testing.test[i].name,
						);
						if (_test === undefined) {
						...
						returnCode = this.testResultsAnalysis(
							testResults.site.testing.test[i],
							_test,
							returnCode,
							run,
						);
	...
	private testResultsAnalysis(
		...
		if (testName === test.id) {
			...
			const durationStr =
				testResult.measurements.get("Execution Time")?.value;
		...
		if (test.uri && test.range) {
			run.appendOutput(
				output,
				new vscode.Location(test.uri, test.range.end),
				test,
			);
		} else {
		...
		if (testResult.status !== "passed" && !havefailures) {
			const failureDurationStr =
				testResult.measurements.get("Execution Time")?.value;
			const failureDuration = failureDurationStr
				? parseFloat(failureDurationStr) * 1000
				: undefined;
			const exitCode = testResult.measurements.get("Exit Value")?.value;
			const completionStatus =
				testResult.measurements.get("Completion Status")?.value;
	...
	private async runCTestImpl(
		...
		const cancellationHandler = cancellationToken.onCancellationRequested(
			async () => {
				...
				log.info(
					localize("ctest.run.cancelled", "CTest run was cancelled"),
				);
			},
		);
		...
		if (res.retc === null) {
			log.info(
				localize("ctest.run.terminated", "CTest run was terminated"),
			);
		} else {
			log.info(
				localize(
	...
	private createTestItemAndSuiteTree(
		testName: string,
		testExplorerRoot: vscode.TestItem,
		initializedTestExplorer: vscode.TestController,
		uri?: vscode.Uri,
	): TestAndParentSuite {
		...
		if (this.ws.config.testSuiteDelimiter) {
			const delimiterRegExp = new RegExp(
				this.ws.config.testSuiteDelimiter,
			);
			...
			for (const suiteId of parts) {
				...
				if (!suiteItem) {
					suiteItem = initializedTestExplorer.createTestItem(
						suiteId,
						suiteId,
					);
		...
		const testItem = initializedTestExplorer.createTestItem(
			testName,
			testLabel,
			uri,
		);
	...
	async refreshTests(driver: CMakeDriver): Promise<number> {
		...
		if (
			!driver.cmake.version ||
			util.versionLess(driver.cmake.version, {
				major: 3,
				minor: 14,
				patch: 0,
			})
			...
			for (const test of tests) {
				testExplorerRoot.children.add(
					initializedTestExplorer.createTestItem(
						test.name,
						test.name,
					),
				);
			...
			if (this.tests && this.tests.kind === "ctestInfo") {
				this.tests.tests.forEach((test) => {
					...
					if (
						test.backtrace !== undefined &&
						this.tests!.backtraceGraph.nodes[test.backtrace] !==
							undefined
					) {
						...
						const defSourceLineProperty = test.properties.filter(
							(property) => property.name === "DEF_SOURCE_LINE",
						)[0];
						if (
							defSourceLineProperty &&
							defSourceLineProperty.value &&
							typeof defSourceLineProperty.value === "string"
						) {
							// Use RegEx to match the format "file_path:line" in value[0]
							const match =
								defSourceLineProperty.value.match(/(.*):(\d+)/);
						...
						if (!testDefFile) {
							...
							testDefFile =
								this.tests!.backtraceGraph.files[
									this.tests!.backtraceGraph.nodes[
										test.backtrace
									].file
								];
							testDefLine =
					...
					const testAndParentSuite = this.createTestItemAndSuiteTree(
						test.name,
						testExplorerRoot,
						initializedTestExplorer,
						testDefFile ? vscode.Uri.file(testDefFile) : undefined,
					);
					...
					if (testDefLine !== undefined) {
						testItem.range = new vscode.Range(
							new vscode.Position(testDefLine - 1, 0),
							new vscode.Position(testDefLine - 1, 0),
						);
					}
						...
						for (const property of test.properties) {
							if (property.name === "LABELS") {
								if (util.isString(property.value)) {
									testTags.push(
										new vscode.TestTag(property.value),
									);
								} else {
									testTags.push(
										...property.value.map(
											(v) => new vscode.TestTag(v),
										),
									);
								}
							}
	...
	private async handleCoverageOnProjects(
		run: vscode.TestRun,
		projectCoverageConfigs: ProjectCoverageConfig[],
	) {
			...
			if (projectCoverageConfig.coverageInfoFiles.length === 0) {
				log.warning(
					localize(
						...
						projectCoverageConfig.project.sourceDir,
					),
				);
				continue;
			}
			const expandedCoverageInfoFiles = await Promise.all(
				projectCoverageConfig.coverageInfoFiles.map(
					async (coverageInfoFile: string) =>
						expandString(
							coverageInfoFile,
							projectCoverageConfig.driver.expansionOptions,
						),
				),
			);
			await handleCoverageInfoFiles(
				run,
				expandedCoverageInfoFiles,
				this.coverageData,
			);
		}
	}

	private async coverageCTestHelper(
		tests: vscode.TestItem[],
		run: vscode.TestRun,
		cancellation: vscode.CancellationToken,
	): Promise<number> {
			...
			if (project) {
				...
				if (driver) {
					projectCoverageConfigs.push({
						project: project,
						driver: driver,
						preRunCoverageTarget:
							driver.config.preRunCoverageTarget,
						postRunCoverageTarget:
							driver.config.postRunCoverageTarget,
						coverageInfoFiles: driver.config.coverageInfoFiles,
					});
				}
			}
		}
		...
		for (const projectCoverageConfig of projectCoverageConfigs) {
			if (projectCoverageConfig.preRunCoverageTarget) {
				...
				const rc = await projectCoverageConfig.project.build([
					projectCoverageConfig.preRunCoverageTarget,
				]);
				if (rc !== 0) {
					log.error(
						localize(
		...
		const runResult = await this.runCTestHelper(
			tests,
			run,
			cancellation,
			undefined,
			...
			false,
			undefined,
			RunCTestHelperEntryPoint.TestExplorer,
		);
		for (const projectCoverageConfig of projectCoverageConfigs) {
			if (projectCoverageConfig.postRunCoverageTarget) {
				log.info(
					localize(
						...
						projectCoverageConfig.postRunCoverageTarget,
						projectCoverageConfig.project.sourceDir,
					),
				);
				const rc = await projectCoverageConfig.project.build([
	...
	private async runTestHandler(
		...
		const requestedTests =
			request.include ||
			this.testItemCollectionToArray(testExplorer.items);
	...
	private async debugCTestHelper(
		tests: vscode.TestItem[],
		run: vscode.TestRun,
		cancellation: vscode.CancellationToken,
	): Promise<number> {
	...
	private async debugCTestImpl(
		...
		const workspaceLaunchConfig = vscode.workspace.workspaceFile
		...
		const workspaceConfigs =
			workspaceLaunchConfig?.get<vscode.DebugConfiguration[]>(
				"configurations",
			) ?? [];
		if (configs.length === 0 && workspaceConfigs.length === 0) {
			log.error(
				localize("no.launch.config", "No launch configurations found."),
			);
			return;
		}
		...
		let allConfigItems: ConfigItem[] = configs.map((config) => ({
			label: config.name,
			config,
			folder: workspaceFolder,
			detail: workspaceFolder.uri.fsPath,
		}));
		allConfigItems = allConfigItems.concat(
			workspaceConfigs.map((config) => ({
				label: config.name,
				config,
				detail: vscode.workspace.workspaceFile!.fsPath,
			})),
		);
		...
		if (allConfigItems.length === 1) {
			...
			const chosen = await vscode.window.showQuickPick(allConfigItems, {
		...
		chosenConfig.config =
			this.replaceAllInObject<vscode.DebugConfiguration>(
				chosenConfig.config,
				"${cmake.testProgram}",
				this.testProgram(testName),
			);
		chosenConfig.config =
			this.replaceAllInObject<vscode.DebugConfiguration>(
				chosenConfig.config,
				"${cmake.testWorkingDirectory}",
				this.testWorkingDirectory(testName),
			);
		...
		chosenConfig.config = this.replaceArrayItems(
			chosenConfig.config,
			"${cmake.testArgs}",
			this.testArgs(testName),
		...
		const started = new Promise<vscode.DebugSession>((resolve) => {
			onDidStartDebugSession = vscode.debug.onDidStartDebugSession(
	...
	private testWorkingDirectory(testName: string): string {
		const property = this.tests?.tests
			...
			?.properties.find((prop) => prop.name === "WORKING_DIRECTORY");
		...
		if (util.isArray(obj) && obj.length !== 0) {
			...
			for (let i = 0; i < obj.length; i++) {
				if (util.isArray(obj[i]) || typeof obj[i] === "object") {
					...
					const replacedItem = this.replaceArrayItemsHelper(
						obj[i] as string,
						str,
						replace,
					);
	...
	private replaceArrayItemsHelper(
		orig: string,
		str: string,
		replace: string[],
	): string | string[] {
	...
	private async debugTestHandler(
		request: vscode.TestRunRequest,
		cancellation: vscode.CancellationToken,
	) {
		// NOTE: We expect the testExplorer to be undefined when the cmake.ctest.testExplorerIntegrationEnabled is disabled.
	...
	private async buildTests(
		...
		for (const test of tests) {
			...
			if (status === 1) {
				...
				this.ctestErrored(test, run, {
					message: localize("build.failed", "Build failed"),
				});
			}
		}

		return (
			Array.from(builtFolder.values()).filter((v) => v !== 0).length === 0
		);
	}
	...
	private ensureTestExplorerInitialized(): vscode.TestController {
		// NOTE: We expect the testExplorer to be undefined when the cmake.ctest.testExplorerIntegrationEnabled is disabled.
		if (!testExplorer) {
			testExplorer = vscode.tests.createTestController(
				"cmake-tools.CTest",
				"CTest",
			);

			// Cast to any since this is not supported yet in the API we use.
			(testExplorer as any).refreshHandler = () =>
				vscode.commands.executeCommand("cmake.refreshTestsAll");

			if (this.projectController) {
				for (const project of this.projectController.getAllCMakeProjects()) {
					const folderPath = util.platformNormalizePath(
						project.sourceDir,
					);
					const folderName = path.basename(project.sourceDir);
					const testItem = testExplorer.createTestItem(
						folderPath,
						folderName,
					);
			...
			testExplorer.createRunProfile(
				...
				(
					request: vscode.TestRunRequest,
					cancellation: vscode.CancellationToken,
				) => this.runTestHandler(request, cancellation),
				true,
			);
			testExplorer.createRunProfile(
				...
				(
					request: vscode.TestRunRequest,
					cancellation: vscode.CancellationToken,
				) => this.runTestHandler(request, cancellation, true),
				true,
			).loadDetailedCoverage = async (_, fileCoverage) =>
				this.coverageData.get(fileCoverage) ?? [];
			testExplorer.createRunProfile(
				"Debug Tests",
				...
				(
					...
					try {
						const testProject =
							this.projectController!.getAllCMakeProjects().filter(
								(project) =>
									request.include![0].uri!.fsPath.includes(
										project.folderPath,
									),
							);
						return testProject![0].cTestController.debugTestHandler(
							request,
							cancellation,
						);
					} catch (e) {
	...
	addTestExplorerRoot(folder: string) {
		...
		testExplorer.items.add(
			testExplorer.createTestItem(normalizedFolder, normalizedFolder),
		);
	}
