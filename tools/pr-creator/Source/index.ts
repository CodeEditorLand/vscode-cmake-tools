...
class Session {
	...
	private async activePullRequestExists(): Promise<boolean> {
		...
		return pullRequests.some((pr) =>
			pr.title?.startsWith(this.settings.title.split(" ")[0]),
		);
	}
	...
	public async run(): Promise<void> {
		try {
			...
			if (await this.activePullRequestExists()) {
				console.warn("An active pull request already exists, exiting.");
				return;
			}
...
export function copyFiles(
	...
	const absoluteSourceLocation = path.resolve(
		parentRoot,
		`${sourceRepo}/${sourceLocation}/*`,
	);
	const absoluteTargetLocation = path.resolve(
		parentRoot,
...
program
	.requiredOption(
		...
		"The source file location. Relative path from the root of the repository.",
	)
	.requiredOption("--target-repo <target-repo>", "The target repository.")
	.requiredOption(
		"--target-file-location <target-file-location>",
		"The target file location. Relative path from the root of the repository.",
	)
	.parse(process.argv);
...
if (
	options.sourceFileLocation &&
	options.targetRepo &&
	options.targetFileLocation
) {
	copyFiles(
		options.sourceFileLocation,
		options.targetRepo,
