...
export class ConfigurationReader implements vscode.Disposable {
		...
		reader.updateSubscription = vscode.workspace.onDidChangeConfiguration(
			(e) => {
				if (e.affectsConfiguration("cmake", folder?.uri)) {
					const newConfigData =
						ConfigurationReader.loadConfig(folder);
	...
	static loadConfig(
		...
		const configData = vscode.workspace.getConfiguration(
			"cmake",
			folder?.uri,
		) as any as ExtensionConfigurationSettings;
		...
		const forPlatform = (configData as any)[platform] as
			| ExtensionConfigurationSettings
			| undefined;
	...
	updatePartial(
		newConfigData: Partial<ExtensionConfigurationSettings>,
		...
		for (const keyObject of Object.getOwnPropertyNames(newConfigData)) {
			...
			if (util.compare(newValue, oldValue) !== util.Ordering.Equivalent) {
				if (fireEvent) {
					const em: vscode.EventEmitter<
						ExtensionConfigurationSettings[typeof key]
					> = this.emitters[key];
	...
	buildDirectory(
		...
		if (
			multiProject &&
			this.isDefaultValue("buildDirectory", workspaceFolder)
		) {
		...
		if (communicationMode === "automatic" && this.useCMakeServer) {
			log.warning(
				localize(
					...
					"The setting {0} is replaced by {1}. Please upgrade your configuration.",
					'"useCMakeServer"',
	...
	get additionalCompilerSearchDirs(): string[] {
		...
		if (
			this.configData.additionalCompilerSearchDirs.length === 0 &&
			this.configData.mingwSearchDirs.length > 0
		) {
			log.warning(
				localize(
					"please.upgrade.configuration",
	...
	private readonly emitters: EmittersOf<ExtensionConfigurationSettings> = {
		...
		cmakeCommunicationMode:
			new vscode.EventEmitter<CMakeCommunicationMode>(),
		...
		useVsDeveloperEnvironment:
			new vscode.EventEmitter<UseVsDeveloperEnvironment>(),
	...
	onChange<K extends keyof ExtensionConfigurationSettings>(
		setting: K,
		cb: (value: ExtensionConfigurationSettings[K]) => any,
	): vscode.Disposable {
	...
	isDefaultValue<K extends keyof ExtensionConfigurationSettings>(
		setting: K,
...
class PromiseTracker {
	...
	public scheduleAndTrackTask(cb: () => any): void {
		const selfDestructWrapper = util
			.scheduleTask(() => {
				const result = cb();
...
export function checkConfigureOverridesPresent(
	config: ConfigurationReader,
): boolean {
	if (
		config.configureArgs.length > 0 ||
		Object.values(config.configureEnvironment).length > 0 ||
		checkGeneralEnvironmentOverridesPresent(config)
	) {
		return true;
	}
...
export function checkBuildOverridesPresent(
	config: ConfigurationReader,
): boolean {
	if (
		config.buildArgs.length > 0 ||
		config.buildToolArgs.length > 0 ||
		Object.values(config.buildEnvironment).length > 0 ||
		checkGeneralEnvironmentOverridesPresent(config)
	) {
		return true;
	}
...
export function checkTestOverridesPresent(
	config: ConfigurationReader,
): boolean {
	if (
		Object.values(config.testEnvironment).length > 0 ||
		config.ctestArgs.length > 0 ||
		checkGeneralEnvironmentOverridesPresent(config)
	) {
		return true;
	}
...
export function checkPackageOverridesPresent(
	config: ConfigurationReader,
): boolean {
	if (
		Object.values(config.cpackEnvironment).length > 0 ||
		config.cpackArgs.length > 0 ||
		checkGeneralEnvironmentOverridesPresent(config)
	) {
		return true;
	}
...
export function checkGeneralEnvironmentOverridesPresent(
	config: ConfigurationReader,
): boolean {
