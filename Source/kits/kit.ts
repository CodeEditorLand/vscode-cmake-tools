 ...
 */
export const OLD_USER_KITS_FILEPATH = path.join(
	process.platform === "win32" ? paths.roamingDataDir : paths.dataDir,
	"cmake-tools.json",
);
...
export async function getCompilerVersion(
	...
	const exec = await proc.execute(binPath, ["-v"], undefined, {
		overrideLocale: true,
		timeout: 60000,
	}).result;
	...
	if (vendor === "Clang") {
		version_re = /^(?:Apple LLVM|.*clang) version ([^\s-]+)(?:[\s-]|$)/gim;

		version_match_index = 1;
	} else {
		version_re = /^gcc(-| )version (.*?) .*/gim;
...
function isMingw(bin: string): boolean {
	...
	const isMingw =
		isWin32 && (bin.toLowerCase().includes("mingw") || isMsys(bin));
...
async function asMingwKit(bin: string, kit: Kit): Promise<Kit> {
	...
	if (mingwMakeExists) {
		...
		const execMake = await proc.execute(mingwMakePath, ["-v"], null, {
			environment: { PATH: kit.environmentVariables["CMT_MINGW_PATH"] },
			timeout: 30000,
		}).result;
 ...
 */
export async function kitIfCompiler(
	bin: string,
	...
	if (gcc_res) {
		const version = isTrusted
			? await getCompilerVersion("GCC", bin, pr)
			: null;
		...
		if (await fs.exists(gxx_bin1)) {
			...
			const version2 =
				isTrusted && (await fs.exists(bin2))
					? await getCompilerVersion("GCC", bin2, pr)
					: null;

			const version_is_match =
				version2 === null
					? false
					: version === null
						? false
						: version2.fullVersion === version.fullVersion;
		...
		const version = isTrusted
			? await getCompilerVersion("Clang", bin, pr)
			: null;
		...
		if (isTrusted) {
			...
			if (
				version.target &&
				version.target.triple.includes("msvc") &&
				version.installedDir &&
				version.installedDir.includes("Microsoft Visual Studio")
			) {
		...
		const clangxx_fname = clang_cl_res
			? fname
			: fname.replace(/^clang/, "clang++");
		...
		if (await fs.exists(clangxx_bin1)) {
			...
			const fname2 = clang_cl_res
				? fname.replace(/clang\-cl(-\d+(\.\d+(\.\d+)?)?)/, "clang-cl")
				: fname.replace(/clang(-\d+(\.\d+(\.\d+)?)?)/, "clang");

			const bin2 = path.join(path.dirname(bin), fname2);

			const clangxx_fname2 = clang_cl_res
				? fname
				: fname2.replace(/clang/, "clang++");

			const clangxx_bin2 = path.join(path.dirname(bin), clangxx_fname2);
			// Ensure the version is match
			const version2 =
				isTrusted && (await fs.exists(bin2))
					? await getCompilerVersion("Clang", bin2, pr)
					: null;
		...
		const clangKit: Kit = {
			name:
				(clang_cl_res
					? version?.detectedName.replace(/^Clang/, "Clang-cl")
					: version?.detectedName) ??
				localize(
...
async function scanDirectory<Ret>(
	dir: string,
	mapper: (filePath: string) => Promise<Ret | null>,
): Promise<Ret[]> {
	if (
		util.isTestMode() &&
		process.platform === "win32" &&
		dir.indexOf("AppData") > 0 &&
		dir.indexOf("Local") > 0
	) {
 ...
 */
export async function scanDirForCompilerKits(
	dir: string,
	isTrusted: boolean = true,
	pr?: ProgressReporter,
): Promise<Kit[]> {
	const kits = await scanDirectory(dir, async (bin) => {
		...
		try {
			...
			rollbar.exception(
				localize("failed.to.scan.kit", "Failed to scan a kit file"),
				e,
				{ bin, exception: e.code, stat },
			);
...
export function vsVersionName(inst: VSInstallation): string {
	...
	return end < 0
		? inst.catalog.productDisplayVersion
		: inst.catalog.productDisplayVersion.substring(0, end - 1);
}
 ...
 */
export async function getShellScriptEnvironment(
	kit: Kit,
	...
	if (!tmpDir) {
		console.log(
			`TEMP dir is not set. Shell script "${script_filename}" will not run.`,
		);
	...
	if (opts) {
		environmentSetupScript = await expand.expandString(
	...
	const res = await proc.execute(run_command, [], null, {
		shell: true,
		silent: true,
	}).result; // run script
	...
	if (!env || env === "") {
		console.log(
			`Error running ${kit.environmentSetupScript} with:`,
			output,
		);
	...
	const vars = env
		.split("\n")
		.map((line) => line.trim())
		.filter((line) => line.length !== 0 && !line.match(filter))
 ...
 */
async function tryCreateNewVCEnvironment(
	inst: VSInstallation,
	hostArch: string,
	targetArch: string,
	pr?: ProgressReporter,
	...
	log.debug(
		localize("detected.kit.for.version", "Detected VsKit for version"),
	);
	...
	if (version) {
		...
		if (generatorName) {
			log.debug(
				` ${localize("generator.present", "Generator Present: {0}", generatorName)}`,
			);

			kit.preferredGenerator = {
				name: generatorName,
				platform:
					(generatorPlatformFromVSArch[targetArch] as string) ||
					targetArch,
		...
		log.debug(
			` ${localize("selected.preferred.generator.name", "Selected Preferred Generator Name: {0} {1}", generatorName, JSON.stringify(kit.preferredGenerator))}`,
		);
	}
...
export async function scanForVSKits(pr?: ProgressReporter): Promise<Kit[]> {
	...
	const hostArches: MsvcHostArches[] =
		util.getHostArchitecture() === "arm64"
	...
	const prs = installs.map(async (inst): Promise<Kit[]> => {
		...
		const vsVersionSupportsArm64 =
			(version?.major === 17 && version.minor >= 4) ||
			(version && version.major > 17);
		...
		hostArches.forEach((hostArch) => {
			...
			targetArches.forEach((targetArch) => {
				sub_prs.push(
					tryCreateNewVCEnvironment(inst, hostArch, targetArch, pr),
				);
			});
		});
...
async function scanDirForClangForMSVCKits(
	dir: PathWithTrust,
	vsInstalls: VSInstallation[],
	cmakePath?: string,
): Promise<Kit[]> {
	const kits = await scanDirectory(
		dir.path,
		async (binPath): Promise<Kit[] | null> => {
			const isClangGnuCli = path.basename(binPath, ".exe") === "clang";

			const isClangMsvcCli =
				path.basename(binPath, ".exe") === "clang-cl";

			if (!isClangGnuCli && !isClangMsvcCli) {
				return null;
			}

			const version = dir.isTrusted
				? await getCompilerVersion("Clang", binPath)
				: null;
			...
			for (const vs of vsInstalls) {
				...
				const vsArch =
					version?.target && version.target.triple.includes("i686-pc")
						? "x86"
						: "x64";
				...
				const clangExists = async () => {
					const exists =
						binPath.startsWith(
							`${vs.installationPath}\\VC\\Tools\\Llvm\\${clangArchPath}bin`,
						) &&
						(await util.checkFileExists(
							util.lightNormalizePath(binPath),
						));
				...
				if (isClangGnuCli) {
					...
					const installationVersion = /^(\d+)+./.exec(
						vs.installationVersion,
					);

					const generatorName: string | undefined =
						installationVersion
							? VsGenerators[installationVersion[1]]
							: undefined;
...
export async function scanForClangForMSVCKits(
	searchPaths: PathWithTrust[],
	cmakePath?: string,
): Promise<Promise<Kit[]>[]> {
	const vs_installs = await vsInstallations();

	const results = searchPaths.map((p) =>
		scanDirForClangForMSVCKits(p, vs_installs, cmakePath),
	);

	return results;
}

async function getVSInstallForKit(
	kit: Kit,
): Promise<VSInstallation | undefined> {
	if (process.platform !== "win32") {
		return undefined;
	}
	...
	const match = (inst: VSInstallation) =>
		...
		(!!kit.compilers &&
			kit.name.indexOf("Clang") >= 0 &&
			kit.name.indexOf(vsDisplayName(inst)) >= 0);
...
export async function getVSKitEnvironment(
	kit: Kit,
 ...
 * @returns `Environment`
 */
export async function effectiveKitEnvironment(
	kit: Kit,
	opts?: expand.ExpansionOptions,
): Promise<Environment> {
	...
	if (!getVSKitEnv) {
		...
		for (const env_var of Object.keys(kit_env)) {
			env[env_var] = await expand.expandString(
				kit_env[env_var],
				opts ?? expandOptions,
			);
		}
		...
		const expandOptions: expand.ExpansionOptions = opts
			? { ...opts, envOverride: env, penvOverride: env }
			: {
					vars: {} as expand.KitContextVars,
					envOverride: env,
					penvOverride: env,
...
export async function findCLCompilerPath(
	env?: Environment,
): Promise<string | null> {
	if (!env) {
		return null;
...
export async function scanForKits(cmakePath?: string, opt?: KitScanOptions) {
	log.debug(
		localize("scanning.for.kits.on.system", "Scanning for Kits on system"),
	);
	...
	const result = await vscode.window.withProgress(prog, async (pr) => {
		...
		paths.windows.defaultCompilerPaths.LLVM.forEach((p) =>
			addScanPath(p.path, p.isTrusted),
		);

		paths.windows.defaultCompilerPaths.MSYS2.forEach((p) =>
			addScanPath(p.path, p.isTrusted),
		);

		const compiler_kits = Array.from(scan_paths).map((path_el) =>
			scanDirForCompilerKits(path_el[0], path_el[1], pr),
		);
		...
		if (isWin32) {
			...
			if (process.env.hasOwnProperty("LLVM_ROOT")) {
				const llvm_root = path.normalize(
					(process.env.LLVM_ROOT as string) + "\\bin",
				);
			...
			scan_paths.forEach((isTrusted, path) =>
				addScanPath(path, isTrusted, clang_paths),
			);
			...
			vs_installs.forEach((vs_install) => {
				bundled_clang_paths.push(
					vs_install.installationPath + "\\VC\\Tools\\Llvm\\bin",
				);

				bundled_clang_paths.push(
					vs_install.installationPath + "\\VC\\Tools\\Llvm\\x64\\bin",
				);
			});

			bundled_clang_paths.forEach((path_el) =>
				addScanPath(path_el, true, clang_paths),
			);
		...
		kits.map((k) =>
			log.info(
				localize(
					...
					"Found Kit ({0}): {1}",
					k.isTrusted
						? localize("trusted", "trusted")
						: localize("untrusted", "untrusted"),
					k.name,
				),
			),
		);
		...
		scan_paths.forEach((isTrusted, path) =>
			!isTrusted ? untrusted_paths.add(path) : undefined,
		);
		...
		void vscode.window
			...
			.then(async (action) => {
				if (action?.action === "yes") {
					...
					const additionalCompilerSearchDirs = settings.get<string[]>(
						"additionalCompilerSearchDirs",
						[],
					);

					additionalCompilerSearchDirs.push(
						...Array.from(untrusted_paths),
					);
...
export async function scanForKitsIfNeeded(
	project: CMakeProject,
): Promise<boolean> {
	const kitsVersionSaved =
		project.workspaceContext.state.extensionContext.globalState.get<number>(
			"kitsVersionSaved",
		);
	...
	if (
		(!kitsVersionSaved || kitsVersionSaved !== kitsVersionCurrent) &&
		!util.isTestMode() &&
		...
		await project.workspaceContext.state.extensionContext.globalState.update(
			"kitsVersionSaved",
			kitsVersionCurrent,
		);
 ...
 */
export async function descriptionForKit(
	kit: Kit,
	shortVsName: boolean = false,
): Promise<string> {
	if (kit.description) {
	...
	if (kit.visualStudio) {
		if (kit.compilers) {
			// Clang for MSVC
			const compilers = Object.keys(kit.compilers).map(
				(k) => `${k} = ${kit.compilers![k]}`,
			);
...
export async function readKitsFile(
	filePath: string,
	workspaceFolder?: string,
	expansionOptions?: expand.ExpansionOptions,
): Promise<Kit[]> {
		...
		expansionOptions = {
			vars: {
				...
				workspaceRoot: workspaceFolder
					? workspaceFolder
					: "${workspaceRoot}",
				workspaceRootFolderName: workspaceFolder
					? path.basename(workspaceFolder)
	...
	for (const kit of dropNulls(kits)) {
		...
		if (kit.toolchainFile) {
			kit.toolchainFile = await expand.expandString(
				kit.toolchainFile,
				expansionOptions,
			);
		}
		...
		if (kit.compilers) {
			for (const lang in kit.compilers) {
				kit.compilers[lang] = await expand.expandString(
					kit.compilers[lang],
					expansionOptions,
				);
			}
 ...
 */
export function kitsForWorkspaceDirectory(
	workspaceFolder: string,
): Promise<Kit[]> {
...
export async function getAdditionalKits(project: CMakeProject): Promise<Kit[]> {
	...
	const expandedAdditionalKitFiles: string[] =
		await project.getExpandedAdditionalKitFiles();

	let additionalKits: Kit[] = [];

	for (const kitFile of expandedAdditionalKitFiles) {
		additionalKits = additionalKits.concat(
			await readKitsFile(
				kitFile,
				project.workspaceContext.folder.uri.fsPath,
				opts,
			),
		);
	}
...
export function kitChangeNeedsClean(newKit: Kit, oldKit: Kit | null): boolean {
	...
	const important_params = (k: Kit) => ({
		...
		tc: k.toolchainFile,
		preferredGeneratorName: k.preferredGenerator
			? k.preferredGenerator.name
			: null,
		preferredGeneratorPlatform:
			k.preferredGenerator && k.preferredGenerator.platform
				? k.preferredGenerator.platform
				: null,
		preferredGeneratorToolset:
			k.preferredGenerator && k.preferredGenerator.toolset
				? k.preferredGenerator.toolset
				: null,
	});
