...
import {
	CMakeExecutable,
	getCMakeExecutableInformation,
} from "@cmt/cmakeExecutable";
import { CompilationDatabase } from "@cmt/compilationDatabase";
import {
	checkConfigureOverridesPresent,
...
import {
	...
	DebugTrackerFactory,
	getDebuggerPipeName,
} from "@cmt/debug/cmakeDebugger/debuggerConfigureDriver";
...
import { expandString, expandStrings, ExpansionOptions } from "@cmt/expand";
import {
	enableFullFeatureSet,
	extensionManager,
	setContextAndStore,
	updateFullFeatureSet,
} from "@cmt/extension";
...
export class CMakeProject {
	...
	private readonly onUseCMakePresetsChangedEmitter =
	 ...
	 */
	private constructor(
		readonly workspaceContext: DirectoryContext,
		projectController?: ProjectController,
		readonly isMultiProjectFolder: boolean = false,
	) {
		// Handle the active kit changing. We want to do some updates and teardown
		log.debug(
			localize(
				"constructing.cmakeproject",
				"Constructing new CMakeProject instance",
			),
		);
		this.projectController = projectController;
		this.cTestController = new CTestDriver(
			workspaceContext,
			projectController,
		);
		this.cPackageController = new CPackDriver(workspaceContext);
		this.workflowController = new WorkflowDriver(
			workspaceContext,
			projectController,
		);
		this.onCodeModelChanged(FireLate, (_) =>
			this._codeModelChangedApiEventEmitter.fire(),
		);
	}
	...
	get folderPath(): string {
		return this.isMultiProjectFolder
			? this.sourceDir
			: this.workspaceContext.folder.uri.fsPath;
	}
	...
	async setUseCMakePresets(useCMakePresets: boolean) {
		...
		if (
			!useCMakePresets &&
			this.targetName.value === this.targetsInPresetName
		) {
			this.targetName.set("all");
		}
		...
		if (oldValue !== useCMakePresets) {
			...
			if (drv) {
				log.debug(
					localize("disposing.driver", "Disposing CMake driver"),
				);
	...
	private readonly statusMessage = new Property<string>(
		localize("initializing", "Initializing"),
	);
	...
	private readonly _configurePreset =
		new Property<preset.ConfigurePreset | null>(null);
	...
	private async resetPresets(driver: CMakeDriver | null) {
		...
		await this.workspaceContext.state.setConfigurePresetName(
			this.folderName,
			null,
	...
	async expandConfigPresetbyName(
		configurePreset: string | null | undefined,
	): Promise<preset.ConfigurePreset | undefined> {
		if (!configurePreset) {
			return undefined;
		}
		log.debug(
			localize(
				"resolving.config.preset",
				"Resolving the selected configure preset",
			),
		);
		...
		if (
			!preset.getPresetByName(
				preset.configurePresets(this.folderPath),
				configurePreset,
			) &&
			!preset.getPresetByName(
				preset.userConfigurePresets(this.folderPath),
				configurePreset,
			)
		) {
			return undefined;
		...
		const presetInherits = await preset.getConfigurePresetInherits(
			this.folderPath,
			configurePreset,
			true,
			true,
		...
		if (!expandedConfigurePreset) {
			log.error(
				localize(
					"failed.resolve.config.preset",
					"Failed to resolve configure preset: {0}",
					configurePreset,
				),
			);
			return undefined;
		...
		if (
			expandedConfigurePreset.__file &&
			expandedConfigurePreset.__file.version <= 2
		) {
			if (!expandedConfigurePreset.binaryDir) {
				log.error(
					localize(
						"binaryDir.not.set.config.preset",
						"{0} is not set in configure preset: {1}",
						'"binaryDir"',
						configurePreset,
					),
				);
			...
			if (!expandedConfigurePreset.generator) {
				log.error(
					localize(
						"generator.not.set.config.preset",
						"{0} is not set in configure preset: {1}",
						'"generator"',
						configurePreset,
					),
		...
		preset.updateCachedExpandedPreset(
			this.folderPath,
			expandedConfigurePreset,
			"configurePresets",
		);
		...
		expandedConfigurePreset.environment =
			EnvironmentUtils.mergePreserveNull([
	...
	async setConfigurePreset(configurePreset: string | null) {
		...
		if (configurePreset) {
			const expandedConfigurePreset: preset.ConfigurePreset | undefined =
				await this.expandConfigPresetbyName(configurePreset);
			...
			if (
				previousGenerator &&
				previousGenerator !== expandedConfigurePreset?.generator
			) {
				await this.shutDownCMakeDriver();
			}
			log.debug(
				localize(
					"loading.new.config.preset",
					"Loading new configure preset into CMake driver",
				),
			);
			if (drv) {
				try {
					this.statusMessage.set(
						localize("reloading.status", "Reloading..."),
					);
					...
					void vscode.window.showErrorMessage(
						localize(
							"unable.to.set.config.preset",
							"Unable to set configure preset {0}.",
							`"${error}"`,
						),
					);
					this.statusMessage.set(
						localize(
							"error.on.switch.config.preset",
							"Error on switch of configure preset ({0})",
							error.message,
						),
					);
					this.cmakeDriver = Promise.resolve(null);
	...
	private readonly _buildPreset = new Property<preset.BuildPreset | null>(
		null,
	);

	async expandBuildPresetbyName(
		buildPreset: string | null,
	): Promise<preset.BuildPreset | undefined> {
		if (!buildPreset) {
			return undefined;
		}
		log.debug(
			localize(
				...
				"Resolving the selected build preset",
			),
		);
		...
		if (!expandedBuildPreset) {
			log.error(
				localize(
					"failed.resolve.build.preset",
					"Failed to resolve build preset: {0}",
					buildPreset,
				),
			);
			return undefined;
		...
		// Make sure we pass CMakeDriver the preset defined env as well as the parent env
		expandedBuildPreset.environment = EnvironmentUtils.mergePreserveNull([
			expandedBuildPreset.__parentEnvironment,
			expandedBuildPreset.environment,
		]);
	...
	async setBuildPreset(buildPreset: string | null) {
		const drv = await this.cmakeDriver; // Use only an existing driver, do not create one
		if (buildPreset) {
			const expandedBuildPreset =
				await this.expandBuildPresetbyName(buildPreset);
			if (!expandedBuildPreset) {
				this._buildPreset.set(null);
				return;
			}
			...
			if (!expandedBuildPreset.configurePreset) {
				log.error(
					localize(
						"configurePreset.not.set.build.preset",
						"{0} is not set in build preset: {1}",
						'"configurePreset"',
						buildPreset,
					),
				);
			...
			log.debug(
				localize(
					"loading.new.build.preset",
					"Loading new build preset into CMake driver",
				),
			);
			if (drv) {
				try {
					...
					await this.workspaceContext.state.setBuildPresetName(
						this.folderName,
						expandedBuildPreset.configurePreset,
						buildPreset,
						this.isMultiProjectFolder,
					);
					this.statusMessage.set(localize("ready.status", "Ready"));
					...
					void vscode.window.showErrorMessage(
						localize(
							"unable.to.set.build.preset",
							"Unable to set build preset {0}.",
							`"${error}"`,
						),
					);
					this.statusMessage.set(
						localize(
							"error.on.switch.build.preset",
							"Error on switch of build preset ({0})",
							error.message,
						),
					);
					this.cmakeDriver = Promise.resolve(null);
	...
	async expandTestPresetbyName(
		testPreset: string | null,
	): Promise<preset.TestPreset | undefined> {
		if (!testPreset) {
			return undefined;
		}
		log.debug(
			localize(
				"resolving.test.preset",
				"Resolving the selected test preset",
			),
		);
		...
		if (!expandedTestPreset) {
			log.error(
				localize(
					"failed.resolve.test.preset",
					"Failed to resolve test preset: {0}",
					testPreset,
				),
			);
			return undefined;
		...
		if (!expandedTestPreset.configurePreset) {
			log.error(
				localize(
					"configurePreset.not.set.test.preset",
					"{0} is not set in test preset: {1}",
					'"configurePreset"',
					testPreset,
				),
			);
		...
		// Make sure we pass CMakeDriver the preset defined env as well as the parent env
		expandedTestPreset.environment = EnvironmentUtils.mergePreserveNull([
			expandedTestPreset.__parentEnvironment,
			expandedTestPreset.environment,
		]);
	...
	async setTestPreset(testPreset: string | null) {
		...
		if (testPreset) {
			...
			const expandedTestPreset =
				await this.expandTestPresetbyName(testPreset);
			if (!expandedTestPreset) {
				this._testPreset.set(null);
				return;
			}
			...
			log.debug(
				localize(
					"loading.new.test.preset",
					"Loading new test preset into CMake driver",
				),
			);
			if (drv) {
				try {
					...
					if (expandedTestPreset.configurePreset) {
						await this.workspaceContext.state.setTestPresetName(
							this.folderName,
							expandedTestPreset.configurePreset,
							testPreset,
							this.isMultiProjectFolder,
						);
					}
					...
					void vscode.window.showErrorMessage(
						localize(
							"unable.to.set.test.preset",
							"Unable to set test preset {0}.",
							`"${error}"`,
						),
					);
					this.statusMessage.set(
						localize(
							"error.on.switch.test.preset",
							"Error on switch of test preset ({0})",
							error.message,
						),
					);
					this.cmakeDriver = Promise.resolve(null);
	...
	private readonly _packagePreset = new Property<preset.PackagePreset | null>(
		null,
	);

	async expandPackagePresetbyName(
		packagePreset: string | null,
	): Promise<preset.PackagePreset | undefined> {
		if (!packagePreset) {
			return undefined;
		}
		log.debug(
			localize(
				"resolving.package.preset",
				"Resolving the selected package preset",
			),
		);
		...
		if (!expandedPackagePreset) {
			log.error(
				localize(
					"failed.resolve.package.preset",
					"Failed to resolve package preset: {0}",
					packagePreset,
				),
			);
			return undefined;
		...
		if (!expandedPackagePreset.configurePreset) {
			log.error(
				localize(
					"configurePreset.not.set.package.preset",
					"{0} is not set in package preset: {1}",
					'"configurePreset"',
					packagePreset,
				),
			);
		...
		if (expandedPackagePreset.name !== preset.defaultPackagePreset.name) {
			preset.updateCachedExpandedPreset(
				this.folderPath,
				expandedPackagePreset,
				"packagePresets",
			);
		}

		// Make sure we pass CMakeDriver the preset defined env as well as the parent env
		expandedPackagePreset.environment = EnvironmentUtils.mergePreserveNull([
			expandedPackagePreset.__parentEnvironment,
			expandedPackagePreset.environment,
		]);
	...
	async setPackagePreset(packagePreset: string | null) {
		...
		if (packagePreset) {
			...
			const expandedPackagePreset =
				await this.expandPackagePresetbyName(packagePreset);
			if (!expandedPackagePreset) {
				this._packagePreset.set(null);
				return;
			}
			...
			log.debug(
				localize(
					"loading.new.package.preset",
					"Loading new package preset into CMake driver",
				),
			);
			if (drv) {
				try {
					...
					if (expandedPackagePreset.configurePreset) {
						await this.workspaceContext.state.setPackagePresetName(
							this.folderName,
							expandedPackagePreset.configurePreset,
							packagePreset,
							this.isMultiProjectFolder,
						);
					}
					...
					void vscode.window.showErrorMessage(
						localize(
							"unable.to.set.package.preset",
							"Unable to set package preset {0}.",
							`"${error}"`,
						),
					);
					this.statusMessage.set(
						localize(
							"error.on.switch.package.preset",
							"Error on switch of package preset ({0})",
							error.message,
						),
					);
					this.cmakeDriver = Promise.resolve(null);
	...
	private readonly _workflowPreset =
		new Property<preset.WorkflowPreset | null>(null);

	async expandWorkflowPresetbyName(
		workflowPreset: string | null,
	): Promise<preset.WorkflowPreset | undefined> {
		if (!workflowPreset) {
			return undefined;
		}
		log.debug(
			localize(
				"resolving.workflow.preset",
				"Resolving the selected workflow preset",
			),
		);
		const expandedWorkflowPreset = await preset.getWorkflowPresetInherits(
			this.folderPath,
			workflowPreset,
			lightNormalizePath(this.folderPath || "."),
			this.sourceDir,
		...
		if (!expandedWorkflowPreset) {
			log.error(
				localize(
					"failed.resolve.workflow.preset",
					"Failed to resolve workflow preset: {0}",
					workflowPreset,
				),
			);
			return undefined;
		...
		if (!expandedWorkflowPreset.steps[0].name) {
			log.error(
				localize(
					"configurePreset.not.set.workflow.preset",
					"{0} is not set in workflow preset: {1}",
					'"configurePreset"',
					workflowPreset,
				),
			);
	...
	async setWorkflowPreset(workflowPreset: string | null) {
		...
		if (workflowPreset) {
			...
			const expandedWorkflowPreset =
				await this.expandWorkflowPresetbyName(workflowPreset);
			if (!expandedWorkflowPreset) {
				this._workflowPreset.set(null);
				return;
			}
			...
			log.debug(
				localize(
					"loading.new.workflow.preset",
					"Loading new workflow preset into CMake driver",
				),
			);
			if (drv) {
				try {
					...
					if (this.configurePreset) {
						await this.workspaceContext.state.setWorkflowPresetName(
							this.folderName,
							this.configurePreset.name,
							workflowPreset,
							this.isMultiProjectFolder,
					...
					void vscode.window.showErrorMessage(
						localize(
							"unable.to.set.workflow.preset",
							"Unable to set workflow preset {0}.",
							`"${error}"`,
						),
					);
					this.statusMessage.set(
						localize(
							"error.on.switch.workflow.preset",
							"Error on switch of workflow preset ({0})",
							error.message,
						),
					);
					this.cmakeDriver = Promise.resolve(null);
	...
	private readonly _codeModelContent = new Property<CodeModelContent | null>(
		null,
	);
	...
	private readonly _codeModelChangedApiEventEmitter =
		new vscode.EventEmitter<void>();

	public notifyOnSelectedConfigurationChanged(
	...
	private readonly _onSelectedConfigurationChangedApiEvent =
		new vscode.EventEmitter<ConfigurationType>();

	private readonly communicationModeSub =
		this.workspaceContext.config.onChange("cmakeCommunicationMode", () => {
			log.info(
				localize(
					"communication.changed.restart.driver",
					"Restarting the CMake driver after a communication mode change.",
				),
			);
			return this.shutDownCMakeDriver();
		});

	private readonly generatorSub = this.workspaceContext.config.onChange(
		"generator",
		async () => {
			log.info(
				localize(
					"generator.changed.restart.driver",
					"Restarting the CMake driver after a generator change.",
				),
			);
	...
	private readonly preferredGeneratorsSub =
		this.workspaceContext.config.onChange(
			"preferredGenerators",
			async () => {
				log.info(
					localize(
						"preferredGenerator.changed.restart.driver",
						"Restarting the CMake driver after a preferredGenerators change.",
					),
				);
				await this.reloadCMakeDriver();
			},
	...
	private readonly cmakePathSub = this.workspaceContext.config.onChange(
		"cmakePath",
		async () => {
			...
			if (!cmakeInfo.isPresent) {
				void vscode.window.showErrorMessage(
					localize(
						"bad.executable",
						"Bad CMake executable: {0}. Check to make sure it is installed or the value of the {1} setting contains the correct path",
						`"${cmakeInfo.path}"`,
						'"cmake.cmakePath"',
					),
				);
	 ...
	 */
	private readonly variantManager = new VariantManager(
	...
	dispose() {
		log.debug(
			localize("disposing.extension", "Disposing CMake Tools extension"),
		);
		...
		rollbar.invokeAsync(
			localize("extension.dispose", "Extension dispose"),
			() => this.asyncDispose(),
		);
	...
	async asyncDispose() {
		...
		for (const disp of [
			...
			this.isBusy,
			this.variantManager,
			this.cTestController,
			this.cPackageController,
		]) {
			disp.dispose();
		}
	}
	...
	private getPreferredGenerators(): CMakeGenerator[] {
		const userPreferred: CMakeGenerator[] =
			this.workspaceContext.config.preferredGenerators.map((g) => ({
				name: g,
			}));
	...
	public async cmakePreConditionProblemHandler(
		...
		switch (e) {
			...
			case CMakePreconditionProblems.MissingCMakeListsFile:
				...
				const ignoreCMakeListsMissing: boolean =
					this.workspaceContext.state.getIgnoreCMakeListsMissing(
						this.folderName,
						this.isMultiProjectFolder,
					) || this.workspaceContext.config.ignoreCMakeListsMissing;
				telemetryProperties["ignoreCMakeListsMissing"] =
					ignoreCMakeListsMissing.toString();

				if (!ignoreCMakeListsMissing && !this.isMultiProjectFolder) {
					const existingCmakeListsFiles: string[] | undefined =
						await util.getAllCMakeListsPaths(this.folderPath);

					if (
						existingCmakeListsFiles !== undefined &&
						existingCmakeListsFiles.length > 0
					) {
					...
					const items: FileItem[] = existingCmakeListsFiles
						? existingCmakeListsFiles.map<FileItem>((file) => ({
								label:
					...
					const browse: string = localize(
						"browse.for.cmakelists",
						"[Browse for CMakeLists.txt]",
					);
					...
					items.push({
						...
						description: localize(
							...
							"Search for CMakeLists.txt on this computer",
						),
					});
					items.push({
						label: dontAskAgain,
						fullPath: "",
						description: localize(
							"do.not.ask.again.description",
							"Do not ask for CMakeLists.txt again in this folder. This will enable the cmake.ignoreCMakeListsMissing setting.",
						),
					});
					const selection: FileItem | undefined =
						await vscode.window.showQuickPick(items, {
							placeHolder:
								items.length === 1
									? localize(
											"cmakelists.not.found",
					...
					telemetryProperties["missingCMakeListsUserAction"] =
						selection === undefined
							? "cancel"
							: selection.label === browse
								? "browse"
								: selection.label === dontAskAgain
									? "dontAskAgain"
									: "pick";
					...
					if (!selection) {
						...
						const cmakeListsFile =
							await vscode.window.showOpenDialog(openOpts);
						if (cmakeListsFile) {
						...
						await vscode.workspace
							.getConfiguration("cmake", this.workspaceFolder)
							.update(
								"ignoreCMakeListsMissing",
								true,
					...
					if (selectedFile) {
						...
						await this.setSourceDir(
							await util.normalizeAndVerifySourceDir(
								newSourceDirectory,
								CMakeDriver.sourceDirExpansionOptions(
									this.workspaceContext.folder.uri.fsPath,
								),
							),
						);
						void vscode.workspace
							...
							.update("sourceDirectory", this._sourceDir);
						if (config) {
	 ...
	 */
	private async startNewCMakeDriver(
		...
		const preConditionHandler = async (
			e: CMakePreconditionProblems,
			config?: ConfigurationReader,
		) => this.cmakePreConditionProblemHandler(e, true, config);
		let communicationMode =
			this.workspaceContext.config.cmakeCommunicationMode.toLowerCase();
		...
		try {
			...
			switch (communicationMode) {
				case fileApi:
					drv = await CMakeFileApiDriver.create(
						cmake,
						this.workspaceContext.config,
						this.sourceDir,
						this.isMultiProjectFolder,
					...
					break;
				case serverApi:
					drv = await CMakeServerDriver.create(
						cmake,
						this.workspaceContext.config,
						this.sourceDir,
						this.isMultiProjectFolder,
					...
					break;
				default:
					drv = await CMakeLegacyDriver.create(
						cmake,
						this.workspaceContext.config,
						this.sourceDir,
						this.isMultiProjectFolder,
		...
		if (!this.useCMakePresets) {
			...
			await drv.setVariant(
				this.variantManager.activeVariantOptions,
				this.variantManager.activeKeywordSetting,
			);
		}

		const newTargetName =
			this.defaultBuildTarget ||
			(this.useCMakePresets
				? this.targetsInPresetName
				: drv.allTargetName);
	...
	async executeCMakeCommand(
		args: string[],
		options?: ExecutionOptions,
	): Promise<ExecutionResult> {
		const drv = await this.getCMakeDriverInstance();
		if (drv) {
			return drv.executeCommand(drv.cmake.path, args, undefined, options)
				.result;
		} else {
			throw new Error(
				localize(
	...
	private async shutDownCMakeDriver() {
		...
		if (drv) {
			log.debug(
				localize("shutting.down.driver", "Shutting down CMake driver"),
			);
			await drv.asyncDispose();
			this.cmakeDriver = Promise.resolve(null);
		}
	...
	private async reloadCMakeDriver() {
		try {
			...
			if (drv) {
				log.debug(
					localize("reloading.driver", "Reloading CMake driver"),
				);
				await drv?.asyncDispose();
				return (this.cmakeDriver = this.startNewCMakeDriver(
	...
	private async init(sourceDirectory: string) {
		...
		this._launchTargetName.set(
			this.workspaceContext.state.getLaunchTargetName(
				this.folderName,
				this.isMultiProjectFolder,
			) || "",
		);
		...
		this.cTestController.onTestingEnabledChanged((enabled) =>
			this._ctestEnabled.set(enabled),
		);
		this.cPackageController.onPackagingEnabledChanged((enabled) =>
			this._cpackEnabled.set(enabled),
		);

		this.statusMessage.set(localize("ready.status", "Ready"));

		this.kitsController = await KitsController.init(this);
		this.presetsController = await PresetsController.init(
			this,
			this.kitsController,
			this.isMultiProjectFolder,
		);
		...
		if (!this.useCMakePresets) {
			...
			this.activeVariant.set(
				this.variantManager.activeVariantOptions.short,
			);
		}
		...
		this.variantManager.onActiveVariantChanged(() => {
			...
			rollbar.invokeAsync(
				localize("changing.build.variant", "Changing build variant"),
				async () => {
					const drv = await this.getCMakeDriverInstance();
					if (drv) {
						await drv.setVariant(
		...
		this.disposables.push(
			this.onPresetsChanged(() => this.doUseCMakePresetsChange()),
		);
		this.disposables.push(
			this.onUserPresetsChanged(() => this.doUseCMakePresetsChange()),
		);
	}

	public async hasPresetsFiles(): Promise<boolean> {
		if (
			(await fs.exists(this.presetsController.presetsPath)) ||
			(await fs.exists(this.presetsController.userPresetsPath))
		) {
			return true;
		}
		return false;
	...
	async doUseCMakePresetsChange(useCMakePresets?: string) {
		...
		if (usingCMakePresets !== this.wasUsingCMakePresets) {
			...
			if (usingCMakePresets) {
				const setPresetsFileLanguageMode = (
					...
					if (
						...
						if (
							config.allowCommentsInPresetsFile &&
				...
				this.onDidOpenTextDocumentListener =
	...
	async initializeKitOrPresets() {
		if (this.useCMakePresets) {
			const latestConfigPresetName =
				this.workspaceContext.state.getConfigurePresetName(
					this.folderName,
					this.isMultiProjectFolder,
				);
			if (latestConfigPresetName) {
				...
				const presets =
					await this.presetsController.getAllConfigurePresets();
				const latestConfigPreset: preset.ConfigurePreset | undefined =
					presets.find(
						(preset) => preset.name === latestConfigPresetName,
					);
				if (latestConfigPreset && !latestConfigPreset.hidden) {
					await this.presetsController.setConfigurePreset(
						latestConfigPresetName,
					);
				}
			}
		} else {
			// Check if the CMakeProject remembers what kit it was last using in this dir:
			const kitName = this.workspaceContext.state.getActiveKitName(
				this.folderName,
				this.isMultiProjectFolder,
			);
			if (kitName) {
				...
				const kit =
					this.kitsController.availableKits.find(
						(k) => k.name === kitName,
					) || null;
	...
	async isNinjaInstalled(): Promise<boolean> {
		...
		if (drv) {
			return (
				(await drv.testHaveCommand("ninja")) ||
				drv.testHaveCommand("ninja-build")
			);
		}
		...
		if (kit) {
			...
			if (drv) {
				try {
					...
					await this.workspaceContext.state.setActiveKitName(
						this.folderName,
						kit.name,
						this.isMultiProjectFolder,
					);
	...
	async getCMakePathofProject(): Promise<string> {
		const overWriteCMakePathSetting = this.useCMakePresets
			...
			: undefined;
		return (
			(await this.workspaceContext.getCMakePath(
				overWriteCMakePathSetting,
			)) || ""
		);
	}
	...
	async getCMakeExecutable() {
		...
		if (
			cmakeExe.version &&
			this.minCMakeVersion &&
			versionLess(cmakeExe.version, this.minCMakeVersion)
		) {
			rollbar.error(
				localize(
	...
	async getCMakeDriverInstance(): Promise<CMakeDriver | null> {
		return this.driverStrand.execute(async () => {
			if (!this.useCMakePresets && !this.activeKit) {
				log.debug(
					localize(
						...
						"Not starting CMake driver: no kit selected",
					),
				);
				return null;
			}
			...
			if ((await this.cmakeDriver) === null) {
				...
				console.assert(
					drv !== null,
					"Null driver immediately after creation?",
				);
				if (drv && !(drv instanceof CMakeLegacyDriver)) {
					this.codeModelDriverSub = drv.onCodeModelChanged((cm) =>
						this._codeModelContent.set(cm),
					);
				}
			}
	...
	static async create(
		...
		projectController?: ProjectController,
		isMultiProjectFolder?: boolean,
	): Promise<CMakeProject> {
		log.debug(
			localize(
				"safely.constructing.cmakeproject",
				"Safe constructing new CMakeProject instance",
			),
		);
		const inst = new CMakeProject(
			workspaceContext,
			projectController,
			isMultiProjectFolder,
		);
	...
	private async refreshCompileDatabase(
		opts: ExpansionOptions,
	): Promise<void> {
		const compdbPaths: string[] = [];
		if (
			this.workspaceContext.config.mergedCompileCommands &&
			this.workspaceContext.config.copyCompileCommands
		) {
			log.warning(
				localize(
					"merge.and.copy.compile.commands",
					"The {0} setting is ignored when {1} is defined.",
		...
		if (this.workspaceContext.config.mergedCompileCommands) {
			...
			const compdbPath = util.platformNormalizePath(
				path.join(await this.binaryDir, "compile_commands.json"),
			);
			if (await fs.exists(compdbPath)) {
				...
				if (this.workspaceContext.config.copyCompileCommands) {
					...
					const copyDest = util.lightNormalizePath(
						this.workspaceContext.config.copyCompileCommands,
					);
					const expandedDest = util.platformNormalizePath(
						await expandString(copyDest, opts),
					);
					if (compdbPath !== expandedDest) {
						...
						try {
							...
							void vscode.window.showErrorMessage(
								localize(
									"failed.to.create.parent.directory.1",
				...
				log.debug(
					localize(
						"cannot.copy.compile.commands",
						"Cannot copy {1} because it does not exist at {0}",
						compdbPath,
						"compile_commands.json",
					),
				);
		...
		if (!this.workspaceContext.config.loadCompileCommands) {
			...
			try {
				...
				const newDB =
					await CompilationDatabase.fromFilePaths(compdbPaths);
				...
				try {
					...
					void vscode.window.showErrorMessage(
						localize(
							"failed.to.create.parent.directory.2",
				...
				if (
					(await fs.exists(expandedDest)) &&
					(await fs.stat(expandedDest)).isDirectory()
				) {
				...
				try {
					await fs.writeFile(
						expandedDest,
						CompilationDatabase.toJson(newDB),
					);
				} catch (e: any) {
					// Just display the error. It's the best we can do.
					void vscode.window.showErrorMessage(
						localize(
							"failed.to.merge",
							"Failed to write merged compilation database to {0}: {1}",
							`"${expandedDest}"`,
							e.toString(),
						),
					);
				...
				void vscode.window.showErrorMessage(
					localize(
						...
						"Failed while trying to ingest the compile_commands.json: {0}",
						e.toString(),
					),
				);
				return;
	...
	async configureInternal(
		trigger: ConfigureTrigger = ConfigureTrigger.api,
		extraArgs: string[] = [],
		type: ConfigureType = ConfigureType.Normal,
		debuggerInformation?: DebuggerInformation,
		...
		if (trigger === ConfigureTrigger.configureWithCache) {
			log.debug(
				localize(
					"no.cache.available",
					"Unable to configure with existing cache",
				),
			);
						...
						async (consumer) => {
							...
							if (drv) {
								...
								try {
									...
									const showDebuggerConfigurationString =
										"showConfigureWithDebuggerNotification";

									if (result.result === 0) {
										await enableFullFeatureSet(true);
										await this.refreshCompileDatabase(
											drv.expansionOptions,
										);
									} else if (
										result.result !== 0 &&
										(await this.getCMakeExecutable())
											.isDebuggerSupported &&
										cmakeConfiguration.get(
											showDebuggerConfigurationString,
										) &&
										!forciblyCanceled &&
										!cancelInformation.canceled &&
										result.resultType ===
										...
										const doNotShowAgainTitle = localize(
											"options.configureWithDebuggerOnFail.do.not.show",
											"Don't Show Again",
										);
										void vscode.window
											.showErrorMessage<MessageItem>(
												localize(
													"configure.failed.tryWithDebugger",
													"Configure failed. Would you like to attempt to configure with the CMake Debugger?",
												),
												{ title: yesButtonTitle },
												{
													title: localize(
														"no.configureWithDebugger.button",
														"Cancel",
													),
												},
											...
											.then(async (chosen) => {
												if (chosen) {
													if (
														...
														await vscode.debug.startDebugging(
															undefined,
															{
																...
																pipeName:
																	getDebuggerPipeName(),
																trigger:
																	ConfigureTrigger.configureFailedConfigureWithDebuggerButton,
																fromCommand:
																	true,
															},
														);
														...
														await cmakeConfiguration.update(
															showDebuggerConfigurationString,
															false,
															vscode
																.ConfigurationTarget
									...
									await this.cTestController.refreshTests(
										drv,
									);
									this.onReconfiguredEmitter.fire();
									return result;
	...
	cleanConfigureWithDebugger(
		...
		return this.configureInternal(
			trigger,
			[],
			ConfigureType.CleanWithDebugger,
			debuggerInformation,
		);
	}
	...
	async maybeAutoSaveAll(showCommandOnly?: boolean): Promise<boolean> {
		...
		if (this.workspaceContext.config.saveBeforeBuild) {
			...
			const cmakeConfiguration =
				vscode.workspace.getConfiguration("cmake");
			const showSaveFailedNotificationString =
				"showNotAllDocumentsSavedQuestion";

			const saveGood = await vscode.workspace.saveAll();
			if (
				!saveGood &&
				cmakeConfiguration.get(showSaveFailedNotificationString)
			) {
				log.debug(
					localize(
				...
				const yesAndDoNotShowAgain: string = localize(
					"do.not.show.not.saved.again",
				...
				if (chosen?.title === yesAndDoNotShowAgain) {
					await cmakeConfiguration.update(
						showSaveFailedNotificationString,
						false,
				...
				return (
					chosen !== undefined &&
					(chosen.title === yesButtonTitle ||
						chosen.title === yesAndDoNotShowAgain)
				);
			}
		}
		return true;
	...
	private async doConfigure(
		...
		cb: (consumer: CMakeOutputConsumer) => Promise<ConfigureResult>,
	): Promise<ConfigureResult> {
		progress.report({
			message: localize("saving.open.files", "Saving open files"),
		});
		if (
			!(await this.maybeAutoSaveAll(
				type === ConfigureType.ShowCommandOnly,
			))
		) {
		...
		if (!this.useCMakePresets) {
			if (!this.activeKit) {
				...
				log.debug(
					localize(
						"no.kit.abort",
						"No kit selected. Abort configure",
					),
				);
				return { result: -1, resultType: ConfigureResultType.Other };
			}
			...
			if (!this.variantManager.haveVariant) {
				...
				if (!this.variantManager.haveVariant) {
					...
					return {
						result: -1,
						resultType: ConfigureResultType.Other,
					};
				}
			}
			...
			await vscode.window.showErrorMessage(
				localize(
					"cannot.configure.no.config.preset",
			...
			log.debug(
				localize(
					"no.preset.abort",
					"No preset selected. Abort configure",
				),
			);
			return { result: -1, resultType: ConfigureResultType.Other };
		}
	...
	private async needsReconfigure(): Promise<boolean> {
		...
		if (!needsReconfigure && !(await fs.exists(drv.binaryDir))) {
			...
			log.info(
				localize(
					"cmake.cache.dir.missing",
					"The folder containing the CMake cache is missing. The cache will be regenerated.",
				),
			);
		}

		const skipConfigureIfCachePresent =
			this.workspaceContext.config.skipConfigureIfCachePresent;
		if (
			skipConfigureIfCachePresent &&
			needsReconfigure &&
			(await fs.exists(drv.cachePath))
		) {
			log.info(
				localize(
	...
	async doCMakeFileChangeReconfigure(uri: vscode.Uri) {
		...
		if (
			...
			driver &&
			filePath === util.platformNormalizePath(driver.cachePath)
		) {
		...
		if (driver && driver.cmakeFiles.length > 0) {
			...
			isCmakeFile = driver.cmakeFiles.some(
				(f) =>
					filePath ===
					util.platformNormalizePath(path.resolve(this.sourceDir, f)),
			);
		} else {
			// Otherwise, fallback to a simple check (does not cover CMake include files)
			isCmakeFile = false;
			if (filePath.endsWith("cmakelists.txt")) {
				const allcmakelists: string[] | undefined =
					await util.getAllCMakeListsPaths(this.folderPath);
				// Look for the CMakeLists.txt files that are in the sourceDirectory root.
				isCmakeFile =
					filePath === path.join(sourceDirectory, "cmakelists.txt") ||
					allcmakelists?.find(
						(file) => filePath === util.platformNormalizePath(file),
					) !== undefined;
			}
		}
		...
		if (isCmakeFile) {
			...
			if (driver && !driver.configOrBuildInProgress()) {
				if (driver.config.configureOnEdit) {
					log.debug(
						localize(
							"cmakelists.save.trigger.reconfigure",
							"Detected saving of CMakeLists.txt, attempting automatic reconfigure...",
						),
					);
					...
					await this.configureInternal(
						ConfigureTrigger.cmakeListsChange,
						[],
						ConfigureType.Normal,
					);
				}
				...
				log.warning(
					localize(
						"cmakelists.save.could.not.reconfigure",
				...
				log.debug(
					localize(
						"needs.reconfigure",
						"The project needs to be reconfigured so that the changes saved in CMakeLists.txt have effect.",
					),
				);
			}
		}
	...
	async tasksBuildCommandDrv(drv: CMakeDriver): Promise<string | null> {
		...
		return buildargs
			? buildCmdStr(buildargs.command, buildargs.args)
			: null;
	}
	...
	async runBuild(
		...
		taskConsumer?: proc.OutputConsumer,
		...
		if (!showCommandOnly) {
			log.info(
				localize(
					"run.build",
					"Building folder: {0}",
					(await this.binaryDir) || this.folderName,
				),
		...
		if (showCommandOnly) {
			...
			if (!drv) {
				throw new Error(
					localize(
						"failed.to.get.cmake.driver",
						"Failed to get CMake driver",
					),
				);
			}
			const buildCmd = await drv.getCMakeBuildCommand(
				targets || (await this.getDefaultBuildTargets()),
			);
			if (buildCmd) {
				log.showChannel();
			...
			if (taskConsumer) {
				...
				return await vscode.window.withProgress(
					...
					async (progress, cancel) => {
						...
						log.showChannel();
						buildLogger.info(
							localize("starting.build", "Starting build"),
						);
						await setContextAndStore(isBuildingKey, true);
						const rc = await drv!.build(
							newTargets,
							consumer,
							isBuildCommand,
						);
						...
						const fileDiags: FileDiagnostic[] | undefined = drv!
							.config.parseBuildDiagnostics
							? await consumer!.compileConsumer.resolveDiagnostics(
									drv!.binaryDir,
									drv!.sourceDir,
								)
							: [];
	...
	async editCache(): Promise<void> {
		...
		void vscode.workspace
			.openTextDocument(vscode.Uri.file(drv.cachePath))
		...
		if (!this.cacheEditorWebview) {
			...
			if (!drv) {
				void vscode.window.showErrorMessage(
					localize(
						"cache.load.failed",
						"No CMakeCache.txt file has been found. Please configure project first!",
					),
				);
				return 1;
			}

			this.cacheEditorWebview = new ConfigurationWebview(
				drv.cachePath,
				() => {
					void this.configureInternal(
						ConfigureTrigger.commandEditCacheUI,
						[],
						ConfigureType.Cache,
					);
				},
	...
	private readonly targetsInPresetName = localize(
		"targests.in.preset",
		"[Targets In Preset]",
	);
	...
	async showTargetSelector(): Promise<string | null> {
		...
		if (this.useCMakePresets && this.buildPreset?.targets) {
			...
			targets.push(
				...(util.isString(this.buildPreset.targets)
					? [this.buildPreset.targets]
					: this.buildPreset.targets),
			);
			const sel = await vscode.window.showQuickPick(targets, {
				placeHolder: localize(
					"select.active.target.tooltip",
	...
	public async runCTestCustomized(
		driver: CMakeDriver,
		testPreset?: preset.TestPreset,
		consumer?: proc.OutputConsumer,
	) {
		return this.cTestController.runCTest(
			driver,
			true,
			testPreset,
			consumer,
	...
	async workflow(): Promise<number> {
		...
		if (!drv) {
			throw new Error(
				localize(
					"driver.died.before.workflow",
					"CMake driver died before starting workflow.",
				),
			);
		}

		// We shouldn't set isBusy here, because the workflow is composed of phases that each set this flag.
		return (await this.workflowController.runWorkflow(
			drv,
			this.workflowPreset,
			this.configurePreset,
			this.buildPreset,
	...
	public async runCPack(
		driver: CMakeDriver,
		packagePreset?: preset.PackagePreset,
		consumer?: proc.OutputConsumer,
	) {
		return this.cPackageController.runCPack(
			driver,
			packagePreset,
			consumer,
		);
	...
	public async runWorkflow(
		driver: CMakeDriver,
		workflowPreset?: preset.WorkflowPreset,
		consumer?: proc.OutputConsumer,
	) {
		return this.workflowController.runWorkflow(
			driver,
			workflowPreset,
			this.configurePreset,
			this.buildPreset,
	...
	public get defaultBuildTarget(): string | null {
		return this.workspaceContext.state.getDefaultBuildTarget(
			this.folderName,
			this.isMultiProjectFolder,
		);
	}
	private async setDefaultBuildTarget(v: string) {
		await this.workspaceContext.state.setDefaultBuildTarget(
			this.folderName,
			v,
			this.isMultiProjectFolder,
		);
	...
	public async getDefaultBuildTargets(): Promise<string[] | undefined> {
		...
		if (
			this.useCMakePresets &&
			(!defaultTarget || defaultTarget === this.targetsInPresetName)
		) {
			targets = this.buildPreset?.targets;
		}
	...
	async setLaunchTargetByName(name?: string | null) {
		if (await this.needsReconfigure()) {
			const rc = await this.configureInternal(
				ConfigureTrigger.launch,
				[],
				ConfigureType.Normal,
			);
			if (rc.result !== 0) {
		...
		if (executableTargets.length === 1) {
			...
			await this.workspaceContext.state.setLaunchTargetName(
				this.folderName,
				target.name,
				this.isMultiProjectFolder,
			);
	...
	async getCurrentLaunchTarget(): Promise<ExecutableTarget | null> {
		const targetName = this.workspaceContext.state.getLaunchTargetName(
			this.folderName,
			this.isMultiProjectFolder,
		);
		const target = (await this.executableTargets).find(
			(e) => e.name === targetName,
		);
	...
	async launchTargetPath(): Promise<string | null> {
		...
		if (!executable) {
			...
			log.warning(
				` - ${localize("have.you.called.add_executable", "Have you called add_executable() in your CMake project?")}`,
			);
			log.warning(
				` - ${localize("have.you.configured", "Have you executed a successful CMake configure?")}`,
			);
			log.warning(
				localize(
	...
	async currentBuildType(): Promise<string | null> {
		...
		if (this.useCMakePresets) {
			if (this.buildPreset) {
				if (this.buildPreset.configuration) {
					...
					try {
						...
						const cache: CMakeCache = await CMakeCache.fromPath(
							await this.cachePath,
						);
						const buildTypes: string[] | undefined = cache
			...
			if (
				!buildType &&
				this.configurePreset &&
				this.configurePreset.cacheVariables
			) {
				// Single config generators set the build type in config preset.
				buildType = preset.getStringValueFromCacheVar(
					this.configurePreset.cacheVariables["CMAKE_BUILD_TYPE"],
				);
			}
		} else {
			// Make sure the variant manager is initialized first
			...
			buildType =
				this.variantManager.activeVariantOptions.buildType || null;
		}
		return buildType;
	}
	...
	async appendBuildDirectoryToWorkspace() {
		...
		if (binaryDir) {
			...
			void vscode.window.showErrorMessage(
				localize(
					"unable.to.get.build.directory",
					"Unable to get the build directory.",
				),
			);
		}
	}
	...
	async prepareLaunchTargetExecutable(
		name?: string,
	): Promise<ExecutableTarget | null> {
		...
		if (name) {
			const found = (await this.executableTargets).find(
				(e) => e.name === name,
			);
			if (!found) {
				return null;
		...
		if (buildOnLaunch || isReconfigurationNeeded) {
			if (this.isInstallTarget(chosen.name)) {
				...
				const buildTargets =
					(await this.getDefaultBuildTargets()) || [];
				const allTargetName = await this.allTargetName;
				if (
					!buildTargets.includes(allTargetName) &&
					!buildTargets.includes(chosen.name)
				) {
	 ...
	 */
	async getTargetLaunchEnvironment(
		drv: CMakeDriver | null,
		debugEnv?: DebuggerEnvironmentVariable[],
	): Promise<Environment> {
		...
		if (
			(drv?.useCMakePresets ?? false) &&
			(checkConfigureOverridesPresent(this.workspaceContext.config) ??
				false)
		) {
			log.info(
				localize(
					"launch.with.overrides",
					`NOTE: You are launching a target and there are some environment overrides being applied from your VS Code settings.`,
				),
			);
		}
		...
		if (debugEnv) {
			const options = {
				...(await this.getExpansionOptions()),
				envOverride: env,
				penvOverride: configureEnv,
			};
	...
	async debugTarget(name?: string): Promise<vscode.DebugSession | null> {
		...
		try {
			...
			debugConfig = await debuggerModule.getDebugConfigurationFromCache(
				cache,
				targetExecutable,
				process.platform,
		...
		const launchEnv = await this.getTargetLaunchEnvironment(
			drv,
		...
		const cfg = vscode.workspace
			.getConfiguration("cmake", this.workspaceFolder.uri)
			.inspect<object>("debugConfig");
		const customSetting =
			cfg?.globalValue !== undefined ||
			cfg?.workspaceValue !== undefined ||
			cfg?.workspaceFolderValue !== undefined;
	...
	private launchTerminalTargetName =
		"_CMAKE_TOOLS_LAUNCH_TERMINAL_TARGET_NAME";
	private launchTerminalPath = "_CMAKE_TOOLS_LAUNCH_TERMINAL_PATH";
	// Watch for the user closing our terminal
	private readonly termCloseSub = vscode.window.onDidCloseTerminal(
		async (term) => {
	...
	private async createTerminal(
		...
		const options: vscode.TerminalOptions = {
			...
			env: launchEnv,
			cwd:
				(userConfig && userConfig.cwd) || path.dirname(executable.path),
		};
		...
		const launchBehavior =
			this.workspaceContext.config.launchBehavior.toLowerCase();
		...
		if (launchBehavior !== "newterminal") {
			for (const [, terminal] of this.launchTerminals) {
				...
				if (
					JSON.stringify(creationOptions.env) !==
	...
	async launchTarget(name?: string) {
		...
		if (executablePath.startsWith('"')) {
			let launchTerminalPath = (
				terminal.creationOptions as vscode.TerminalOptions
			).env![this.launchTerminalPath];
			if (process.platform === "win32") {
				executablePath = executablePath.replace(/\\/g, "/");
				launchTerminalPath = launchTerminalPath?.toLocaleLowerCase();
				if (
					launchTerminalPath?.includes("pwsh.exe") ||
					launchTerminalPath?.includes("powershell")
				) {
		...
		if (
			userConfig?.args?.length !== undefined &&
			userConfig.args.length > 0
		) {
			const args = await expandStrings(
				userConfig.args,
				await this.getExpansionOptions(),
			);
			args.forEach((arg) =>
				terminal.sendText(` ${shlex.quote(arg)}`, false),
			);
		}
	...
	public async quickStart(
		...
		if (!(await fs.exists(mainListFile))) {
			...
			log.info(
				localize(
					"cmakelists.created",
					"[Quick Start] CmakeLists.txt created successfully.",
				),
			);
		} else {
			log.info(
				localize(
					"cmakelists.exists",
					"[Quick Start] A CmakeLists.txt already exists, continuing...",
				),
			);
		}
		...
		if (await fs.exists(mainPresetsFile)) {
			log.info(
				localize(
					"cmakepresets.already.configured",
					"[Quick Start] A CMakePresets.json is already configured.",
				),
			);
		} else {
			if (!(await this.presetsController.selectConfigurePreset(true))) {
				...
				log.info(
					localize(
						"cmakepresets.not.created",
						"[Quick Start] CMakePresets.json not created.",
					),
				);
			} else {
				log.info(
					localize(
						"cmakepresets.created",
						"[Quick Start] CMakePresets.json created successfully.",
					),
				);
			}
		}
	...
	private async createCMakeListsFile(mainListFile: string): Promise<Number> {
		...
		if (cppFiles.length > 0) {
			selectedFiles =
				(await vscode.window.showQuickPick(cppFiles, {
					canPickMany: true,
					placeHolder: localize(
						"select.cpp.files",
						"Select targets to include in the CMakeLists.txt file",
					),
				})) || [];
		}
		...
		if (selectedFiles.length === 0) {
			if ((await this.createCppFile(projectName, langExt, type)) !== 0) {
				void vscode.window.showErrorMessage(
					localize(
						"create.file.failed",
						"Failed to create a new {0} file. Add target to CMakeLists.txt.",
						langExt,
					),
				);
				failedToCreate = true;
		...
		if (addlOptions?.some((option) => option.label === "CTest")) {
			init += ["include(CTest)", "enable_testing()", "\n"].join("\n");
		}
	...
	private async createCppFile(
		...
		if (projType === "Library") {
			if (
				!(await fs.exists(
					path.join(this.sourceDir, `${fileName}.${langExt}`),
				))
			) {
				await fs.writeFile(
					path.join(this.sourceDir, `${fileName}.${langExt}`),
			...
			if (
				!(await fs.exists(path.join(this.sourceDir, `main.${langExt}`)))
			) {
				await fs.writeFile(
					path.join(this.sourceDir, `main.${langExt}`),
					(langExt === "cpp"
						? [
								"#include <iostream>",
								"",
	...
	async setSourceDir(sourceDir: string) {
		...
		this.cmakeListsExists = await fs.exists(
			path.join(this._sourceDir, "CMakeLists.txt"),
		);
	}
	...
	async sendFileTypeTelemetry(uri: vscode.Uri): Promise<void> {
		...
		if (filePath.endsWith("cmakelists.txt")) {
			...
			fileType = "CMakeCache";
			const binaryDirectory = util.platformNormalizePath(
				await this.binaryDir,
			);
			...
			if (
				(!this.isMultiProjectFolder &&
					filePath.startsWith(
						util.platformNormalizePath(this.folderPath),
					)) ||
		...
		if (fileType) {
			telemetry.logEvent("cmakeFileWrite", {
				filetype: fileType,
				outsideActiveFolder: outside.toString(),
			});
		}
	}
	...
	async getDiagnostics(): Promise<DiagnosticsConfiguration> {
		...
		return {
			folder: this.isMultiProjectFolder
				? this.sourceDir
