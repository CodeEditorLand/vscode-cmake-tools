...
class WindowsEnvironment {
	readonly defaultCompilerPaths: WindowsDefaultCompilerPaths =
		new WindowsDefaultCompilerPaths(this);
...
class Paths {
	...
	async getCTestPath(
		...
		overWriteCMakePathSetting?: string,
	): Promise<string | null> {
		const ctestPath = await this.expandStringPath(
			wsc.config.rawCTestPath,
			wsc,
		);
		if (!ctestPath || ctestPath === "auto" || overWriteCMakePathSetting) {
			const cmake = await this.getCMakePath(
				wsc,
	...
	async getCPackPath(
		...
		const cpackPath = await this.expandStringPath(
			wsc.config.rawCPackPath,
			wsc,
		);
		if (!cpackPath || cpackPath === "auto" || overWriteCMakePathSetting) {
			...
			if (cmake === null) {
				...
				try {
					...
					const cpackName =
						process.platform === "win32" ? "cpack.exe" : "cpack";
					const cpackSibling = path.join(
						path.dirname(cmake),
						cpackName,
					);
