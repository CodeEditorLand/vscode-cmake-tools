	...
	if (cmakeProject && cmakePath) {
		...
		if (cmakeExe.isDebuggerSupported) {
			...
			cmakeLogger.info(
				localize(
					"run.script",
					'Executing CMake script: "{0}"',
					scriptPath,
				),
			);

			const env = EnvironmentUtils.merge([
				process.env,
			...
			if (result.retc === 0) {
				cmakeLogger.info(
					localize(
						...
						'CMake script: "{0}" completed successfully.',
						scriptPath,
					),
				);
			} else {
				cmakeLogger.info(
					localize(
						"run.script.failed",
						'CMake script: "{0}" completed unsuccessfully.',
						scriptPath,
					),
				);
			...
			cmakeLogger.error(
				localize(
					"run.script.cmakeDebugger.not.supported",
					"Cannot debug a script with this version of CMake, ensure you have CMake version 3.27 or later.",
				),
			);
		}
	}
