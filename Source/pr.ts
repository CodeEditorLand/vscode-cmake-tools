...
function limitify<T extends (...args: any[]) => any>(fn: T): T {
	const wrapper = (...args: Parameters<T>) =>
		fsAccessLimiter(fn, ...args) as ReturnType<T>;
	return wrapper as T;
}
		...
		if (!(await exists(parent))) {
			...
			if (!(await stat(parent)).isDirectory()) {
				throw new Error(
					localize(
						"cannot.create.path",
						"Cannot create {0}: {1} is a non-directory",
						fspath,
						parent,
					),
				);
			...
			if (!(await stat(fspath)).isDirectory()) {
				throw new Error(
					localize(
						...
						"Cannot create directory {0}. It exists, and is not a directory!",
						fspath,
					),
				);
			}
