...
export function processVariantFileData(root: VarFileRoot): VariantCollection {
	const settings = util
		.objectPairs(root)
		.map(([setting_name, setting_def]): VariantSetting => {
			const choices = util.objectPairs(setting_def!.choices).map(
				([opt_key, opt_def]): VariantOption => ({
					...opt_def!,
					key: opt_key,
				}),
			);
...
export class VariantManager implements vscode.Disposable {
	 ...
	 */
	private readonly _variantFileWatcher = chokidar.watch([], {
		ignoreInitial: true,
	...
	constructor(
		...
		log.debug(
			localize("constructing", "Constructing {0}", "VariantManager"),
		);
		...
		util.chokidarOnAnyChange(this._variantFileWatcher, (filePath) =>
			rollbar.invokeAsync(
				...
				() => this._reloadVariantsFile(filePath),
			),
		);
	...
	get haveVariant(): boolean {
		return !!this.stateManager.getActiveVariantSettings(
			this.folderName,
			this.isMultiProject,
		);
	}
	...
	variantConfigurationOptionsForKWs(
		keywordSetting: Map<string, string>,
	): VariantOption[] | string {
		const vars = this._variants;

		let error: string | undefined;

		const data = Array.from(keywordSetting.entries()).map(
			([setting_key, opt_key]): VariantOption => {
				const unknown_choice: VariantOption = {
					short: "Unknown",
					key: "__unknown__",
				};

				const found_setting = vars.settings.find(
					(s) => s.name === setting_key,
				);
	...
	get activeVariantOptions(): VariantOption {
		...
		const kws = this.stateManager.getActiveVariantSettings(
			this.folderName,
			this.isMultiProject,
		);
	...
	async selectVariant(name?: string) {
		const variants = this._variants.settings.map((setting) =>
			setting.choices.map((opt) => ({
		...
		const items: VariantCombination[] = product.map((optionset) => ({
			...
			keywordSettings:
				this.transformChoiceCombinationToKeywordSettings(optionset),
		...
		if (name) {
			...
			await this.publishActiveKeywordSettings(
				this.activeKeywordSetting ?? items[0].keywordSettings,
			);
			...
			const cfg = vscode.workspace
				...
				.inspect<object>("defaultVariants");
	...
	async publishActiveKeywordSettings(keywordSettings: Map<string, string>) {
		await this.stateManager.setActiveVariantSettings(
			this.folderName,
			keywordSettings,
			this.isMultiProject,
		);
	...
	findDefaultChoiceCombination(): Map<string, string> {
		...
		return this.transformChoiceCombinationToKeywordSettings(
			Array.from(defaults),
		);
	}
	...
	async initialize(folderName: string) {
		...
		if (
			this.stateManager.getActiveVariantSettings(
				this.folderName,
				this.isMultiProject,
			) === null
		) {
