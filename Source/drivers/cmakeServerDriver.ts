...
export class CMakeServerDriver extends CMakeDriver {
	...
	private readonly _progressEmitter =
		new vscode.EventEmitter<ProgressMessage>();
	...
	private convertServerCodeModel(
		...
		if (serverCodeModel) {
			...
			for (const config of serverCodeModel.configurations) {
				const newConfig: CodeModelConfiguration = {
					name: config.name,
					projects: [],
				};
				...
				for (const project of config.projects) {
					...
					for (const target of project.targets) {
						...
						const linkLanguageFlags: string | undefined =
							target.linkLanguageFlags;
						...
						if (target.fileGroups) {
							...
							for (const group of target.fileGroups) {
								const newGroup: CodeModelFileGroup = {
									...
									compileCommandFragments: group.compileFlags
										? [group.compileFlags]
										: linkLanguageFlags
											? [linkLanguageFlags]
											: [],
	...
	private readonly _codeModelChanged =
		new vscode.EventEmitter<null | CodeModelContent>();
	...
	async doRefreshExpansions(cb: () => Promise<void>): Promise<void> {
		...
		if (
			bindir_before !== this.binaryDir ||
			srcdir_before !== this.sourceDir ||
			new_env !== this._prevConfigureEnv
		) {
	...
	get targets(): RichTarget[] {
		...
		if (
			build_config.projects.some((project) =>
				project.hasInstallRule ? project.hasInstallRule : false,
			)
		) {
			metaTargets.push({
				type: "rich" as "rich",
			...
			(acc, project) =>
				acc.concat(
					project.targets.map((t) => ({
						...
						name: t.name,
						filepath:
							t.artifacts && t.artifacts.length
	...
	get executableTargets(): ExecutableTarget[] {
		return this.targets
			.filter((t) => t.targetType === "EXECUTABLE")
	...
	private async _setKitAndRestart(
		need_clean: boolean,
		cb: () => Promise<void>,
	) {
	...
	): Promise<CMakeServerDriver> {
		return this.createDerived(
			new CMakeServerDriver(
				cmake,
				config,
				sourceDir,
				isMultiProject,
...
function targetReducer(set: RichTarget[], t: RichTarget): RichTarget[] {
	if (
		!set.find(
			(t2) =>
				t.name === t2.name &&
				t.filepath === t2.filepath &&
				t.targetType === t2.targetType,
		)
	) {
		set.push(t);
	}
