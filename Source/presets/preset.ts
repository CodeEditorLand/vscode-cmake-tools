...
import {
	compareVersions,
	enumerateMsvcToolsets,
	getVcVarsBatScript,
	varsForVSInstallation,
	VSInstallation,
	vsInstallations,
} from "@cmt/installs/visualStudio";
...
function evaluateInheritedPresetConditions(
	preset: Preset,
	allPresets: Preset[],
	...
	const evaluateParent = (parentName: string) => {
		...
		if (parent && !preset.isUserPreset && parent.isUserPreset === true) {
			log.error(
				localize(
					"invalid.user.inherits",
					"Preset {0} in CMakePresets.json can't inherit from preset {1} in CMakeUserPresets.json",
					preset.name,
					parentName,
				),
			);
...
export function evaluatePresetCondition(
	...
	if (
		condition === undefined &&
		!evaluateInheritedPresetConditions(
			preset,
			allPresets,
			...
			if (e instanceof MissingConditionPropertyError) {
				log.error(
					localize(
						...
						"Preset {0}: Missing required property {1} on condition object",
						preset.name,
						`"${e.propertyName}"`,
					),
				);
				...
				log.error(
					localize(
						...
						"Preset {0}: Invalid condition type {1}",
						preset.name,
						`"${e.type}"`,
					),
				);
	...
	log.error(
		localize(
			"invalid.condition",
			"Preset {0}: Condition must be null, boolean, or an object.",
			preset.name,
		),
	);
	return undefined;
...
export const defaultBuildPreset: BuildPreset = {
	...
	description: localize(
		"default.build.preset.description",
		"An empty build preset that does not add any arguments",
	),
};
...
export const defaultTestPreset: TestPreset = {
	...
	description: localize(
		"default.test.preset.description",
...
export const defaultPackagePreset: PackagePreset = {
	...
	description: localize(
		"default.package.preset.description",
...
export const defaultWorkflowPreset: WorkflowPreset = {
	...
	description: localize(
		"default.workflow.preset.description",
		"An empty workflow preset that does not add any arguments",
	),
};
...
export function setOriginalPresetsFile(
	folder: string,
...
export function setOriginalUserPresetsFile(
	folder: string,
	presets: PresetsFile | undefined,
) {
...
export function setPresetsPlusIncluded(
	folder: string,
	presets: PresetsFile | undefined,
) {
	...
	if (presets) {
		...
		if (presets.configurePresets) {
			for (const configPreset of presets.configurePresets) {
				configPreset.isUserPreset =
					configPreset.__file?.__path?.endsWith(
		...
		if (presets.buildPresets) {
			for (const buildPreset of presets.buildPresets) {
				buildPreset.isUserPreset =
					buildPreset.__file?.__path?.endsWith(
						"CMakeUserPresets.json",
					) ?? true;
			}
		}
		if (presets.testPresets) {
			for (const testPreset of presets.testPresets) {
				testPreset.isUserPreset =
					testPreset.__file?.__path?.endsWith(
						"CMakeUserPresets.json",
					) ?? true;
			}
		}
		if (presets.packagePresets) {
			for (const packagePreset of presets.packagePresets) {
				packagePreset.isUserPreset =
					packagePreset.__file?.__path?.endsWith(
						"CMakeUserPresets.json",
					) ?? true;
			}
		}
		if (presets.workflowPresets) {
			for (const workflowPreset of presets.workflowPresets) {
				workflowPreset.isUserPreset =
					workflowPreset.__file?.__path?.endsWith(
						"CMakeUserPresets.json",
					) ?? true;
			}
		}
...
export function setUserPresetsPlusIncluded(
	folder: string,
	presets: PresetsFile | undefined,
) {
...
export function setExpandedPresets(
	folder: string,
	presets: PresetsFile | undefined,
) {
 ...
 */
export function updateCachedExpandedPreset(
	folder: string,
	preset: Preset,
	presetType:
		| "configurePresets"
 ...
 * @returns void
 */
function updateCachedExpandedPresethelper(
	cache: PresetsFile | undefined,
	preset: Preset,
	presetType:
		| "configurePresets"
		| "buildPresets"
	...
	if (presetType === "configurePresets") {
		cache.configurePresets = [
			...cache.configurePresets!.slice(0, index),
			preset as ConfigurePreset,
			...cache.configurePresets!.slice(index + 1),
		];
	} else if (presetType === "buildPresets") {
		cache.buildPresets = [
			...cache.buildPresets!.slice(0, index),
			preset as BuildPreset,
			...cache.buildPresets!.slice(index + 1),
		];
	} else if (presetType === "testPresets") {
		cache.testPresets = [
			...cache.testPresets!.slice(0, index),
			preset as TestPreset,
			...cache.testPresets!.slice(index + 1),
		];
	} else if (presetType === "packagePresets") {
		cache.packagePresets = [
			...cache.packagePresets!.slice(0, index),
			preset as PackagePreset,
			...cache.packagePresets!.slice(index + 1),
		];
	} else if (presetType === "workflowPresets") {
		cache.workflowPresets = [
			...cache.workflowPresets!.slice(0, index),
			preset as WorkflowPreset,
			...cache.workflowPresets!.slice(index + 1),
		];
	}
}

export function setExpandedUserPresetsFile(
	folder: string,
	presets: PresetsFile | undefined,
) {
	setUserPresetsHelper(presets);
...
export function configurePresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
...
export function userConfigurePresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	if (usePresetsPlusIncluded) {
 ...
 */
export function allConfigurePresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	return lodash.unionWith(
		configurePresets(folder, usePresetsPlusIncluded).concat(
...
export function userBuildPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	if (usePresetsPlusIncluded) {
 ...
 * Don't use this function if you need to keep any changes in the presets
 */
export function allBuildPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	return lodash.unionWith(
		buildPresets(folder, usePresetsPlusIncluded).concat(
...
export function userTestPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	if (usePresetsPlusIncluded) {
 ...
 * Don't use this function if you need to keep any changes in the presets
 */
export function allTestPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	return lodash.unionWith(
		testPresets(folder, usePresetsPlusIncluded).concat(
...
export function userPackagePresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	if (usePresetsPlusIncluded) {
 ...
 * Don't use this function if you need to keep any changes in the presets
 */
export function allPackagePresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	return lodash.unionWith(
		packagePresets(folder, usePresetsPlusIncluded).concat(
...
export function userWorkflowPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	if (usePresetsPlusIncluded) {
 ...
 * Don't use this function if you need to keep any changes in the presets
 */
export function allWorkflowPresets(
	folder: string,
	usePresetsPlusIncluded: boolean = false,
) {
	return lodash.unionWith(
		workflowPresets(folder, usePresetsPlusIncluded).concat(
...
export function getPresetByName<T extends Preset>(
	presets: T[],
	name: string,
): T | null {
...
function isInheritable(
	key:
		| keyof ConfigurePreset
		| keyof BuildPreset
		| keyof TestPreset
		| keyof PackagePreset
		| keyof WorkflowPreset,
) {
	return (
		key !== "name" &&
		key !== "hidden" &&
...
export function inheritsFromUserPreset(
	preset:
		| ConfigurePreset
		| BuildPreset
		| TestPreset
	...
	if (
		presetType !== "workflowPresets" &&
		(preset as Preset).inherits &&
		presetInherits(
			originalUserPresetsFile[presetType],
			(preset as Preset).inherits,
		)
	) {
		return true;
	...
	const inheritedConfigurePreset =
		presetType === "workflowPresets"
			? (preset as WorkflowPreset).steps[0]?.name
			: presetType !== "configurePresets"
				? (preset as InheritsConfigurePreset).configurePreset
				: undefined;

	return inheritedConfigurePreset
		? !!originalUserPresetsFile.configurePresets?.find(
				(p) => p.name === inheritedConfigurePreset,
			)
		: false;
}
...
async function getVendorForConfigurePreset(
	folder: string,
	name: string,
	sourceDir: string,
...
async function getVendorForConfigurePresetImpl(
	folder: string,
	name: string,
	sourceDir: string,
	workspaceFolder: string,
...
async function getVendorForConfigurePresetHelper(
	folder: string,
	...
	if (refs.has(preset.name)) {
		...
		log.error(
			localize(
				...
				"Circular inherits in configure preset {0}",
				preset.name,
			),
		);
		errorHandler?.errorList.push([
			localize(
				"circular.inherits.in.config.preset",
				"Circular inherits in configure preset",
			),
			preset.name,
		]);
		return null;
...
async function getExpansionOptions(
	workspaceFolder: string,
	sourceDir: string,
	preset: ConfigurePreset | BuildPreset | TestPreset | PackagePreset,
	envOverride?: EnvironmentWithNull,
	penvOverride?: EnvironmentWithNull,
	includeGenerator: boolean = true,
) {
	const generator = includeGenerator
		? "generator" in preset
			? preset.generator
			: "__generator" in preset
				? preset.__generator
				: undefined
		: undefined;
...
async function expandCondition(
	condition: boolean | Condition | null | undefined,
	expansionOpts: ExpansionOptions,
	errorHandler?: ExpansionErrorHandler,
): Promise<boolean | Condition | undefined> {
...
export function getArchitecture(preset: ConfigurePreset) {
	...
	log.warning(
		localize(
			"no.cl.arch",
			"Configure preset {0}: No architecture specified for cl.exe, using {1} by default",
			preset.name,
...
export function getToolset(preset: ConfigurePreset): Toolset {
	...
	if (result) {
		if (result.name === "x86" || result.name === "x64") {
			log.warning(
				localize(
					"invalid.cl.toolset.arch",
					"Configure preset {0}: Unexpected toolset architecture specified {1}, did you mean {2}?",
					preset.name,
					`"${result.name}"`,
					`"host=${result.name}"`,
				),
		...
		if (!result.version && result.name !== latestToolsetName) {
			log.warning(
				localize(
					"no.cl.toolset.version",
					"Configure preset {0}: No toolset version specified for cl.exe, using latest by default",
					preset.name,
				),
			);
		}
		...
		if (option.indexOf("=") < 0) {
			...
			switch (keyValue[0].toLowerCase()) {
				...
				default:
					log.warning(
						localize(
							"unknown.toolset.option",
							"Unrecognized toolset option will be ignored: {0}",
							option,
						),
					);
					break;
...
async function getVsDevEnv(
	...
	if (opts.preset.cacheVariables && matches && matches.groups?.version) {
		...
		if (useCMakeGeneratorInstance && cmakeGeneratorInstance) {
			...
			vsInstall = vsInstalls.find(
				(vs) =>
					vs.installationPath &&
					path.normalize(vs.installationPath) ===
						cmakeGeneratorInstanceNormalized,
			);

			if (!vsInstall) {
				log.warning(
					localize(
						"specified.vs.not.found",
						"Configure preset {0}: Visual Studio instance specified by {1} was not found, falling back on default instance lookup behavior.",
						opts.preset.name,
						`CMAKE_GENERATOR_INSTANCE="${cmakeGeneratorInstance}"`,
					),
				);
			}
		}
	...
	if (!vsInstall) {
		...
		for (const vs of vsInstalls) {
			...
			if (await getVcVarsBatScript(vs, toolset.host!, arch)) {
				...
				if (toolset.version) {
					const availableToolsets = await enumerateMsvcToolsets(
						vs.installationPath,
						vs.installationVersion,
					);
					// forcing non-null due to false positive (toolset.version is checked in conditional)
					if (
						availableToolsets?.find((t) =>
							t.startsWith(toolset.version!),
						)
					) {
						vsInstall = vs;
						break;
					...
					!vsGeneratorVersion ||
					vs.installationVersion.startsWith(
						vsGeneratorVersion.toString(),
					)
				) {
	...
	if (!vsInstall) {
		if (opts.compilerName) {
			log.error(
				localize(
					...
					opts.preset.name,
					`"${opts.compilerName}.exe"`,
					toolset.version
						? `"${toolset.version},${toolset.host}"`
						: `"${toolset.host}"`,
					`"${arch}"`,
				),
			);
			...
			log.error(
				localize(
					"vs.not.found",
					"Configure preset {0}: No Visual Studio installation found that supports the specified toolset {1} and architecture {2}, you may need to run the 'CMake: Scan for Compilers' command if this toolset exists on your computer.",
					opts.preset.name,
					toolset.version
						? `"${toolset.version},${toolset.host}"`
						: `"${toolset.host}"`,
		...
		log.info(
			localize(
				"using.vs.instance",
				"Using developer environment from Visual Studio (instance {0}, version {1}, installed at {2})",
				vsInstall.instanceId,
		...
		if (opts.shouldInterrogateForNinja) {
			...
			if (vsCMakePaths.ninja) {
				log.warning(
					localize(
						"ninja.not.set",
						"Ninja is not set on PATH, trying to use {0}",
						vsCMakePaths.ninja,
					),
				);
				compilerEnv["PATH"] =
 ...
 */
export async function tryApplyVsDevEnv(
	preset: ConfigurePreset,
	workspaceFolder: string,
	sourceDir: string,
): Promise<void> {
		...
		if (useVsDeveloperEnvironmentMode === "auto") {
			if (preset.cacheVariables) {
				...
				const compilerName: string | undefined =
					util.isSupportedCompiler(cxxCompiler) ||
					util.isSupportedCompiler(cCompiler);

				// find where.exe using process.env since we're on windows.
				let whereExecutable;
				// assume in this call that it exists
				const whereOutput = await execute(
					"where.exe",
					["where.exe"],
					null,
					{
				...
				if (compilerName && whereExecutable) {
					...
					const env = EnvironmentUtils.mergePreserveNull([
						process.env,
						preset.environment,
					]);
					const expansionOpts: ExpansionOptions =
					...
					if (presetEnv) {
						for (const key in presetEnv) {
							if (presetEnv[key]) {
								presetEnv[key] = await expandString(
									presetEnv[key]!,
									expansionOpts,
								);
							}
					...
					const compilerLocation = await execute(
						whereExecutable,
						[compilerName],
						null,
						{
 ...
 */
export async function getConfigurePresetInherits(
	folder: string,
	name: string,
	allowUserPreset: boolean = false,
	usePresetsPlusIncluded: boolean = false,
	...
	const preset = await getConfigurePresetInheritsImpl(
		folder,
		name,
		allowUserPreset,
		usePresetsPlusIncluded,
...
async function getConfigurePresetInheritsImpl(
	folder: string,
	name: string,
	allowUserPreset: boolean = false,
	usePresetsPlusIncluded: boolean = false,
	errorHandler?: ExpansionErrorHandler,
	inheritedByPreset?: ConfigurePreset,
): Promise<ConfigurePreset | null> {
	let preset = getPresetByName(
		configurePresets(folder, usePresetsPlusIncluded),
		name,
	...
	log.error(
		localize(
			"config.preset.not.found.full",
			"Could not find configure preset with name {0}",
			name,
		),
	);
	errorHandler?.errorList.push([
...
async function getConfigurePresetInheritsHelper(
	folder: string,
	preset: ConfigurePreset,
	allowUserPreset: boolean = false,
	usePresetsPlusIncluded: boolean = false,
		...
		if (preset.__file.version <= 2) {
			...
			if (preset.toolchainFile) {
				log.error(
					localize(
						...
						"Configure preset {0}: Property {1} is unsupported in presets v2",
						preset.name,
						'"toolchainFile"',
					),
				);
				errorHandler?.errorList.push([
					localize(
						"property.unsupported.v2",
						'Property "toolchainFile" is unsupported in presets v2',
					),
					preset.name,
				]);
				return null;
			...
			if (preset.installDir) {
				...
				errorHandler?.errorList.push([
					localize(
						"property.unsupported.v2",
						'Configure preset {0}: Property "installDir" is unsupported in presets v2',
					),
					preset.name,
				]);
				return null;
			...
			if (parent) {
				...
				for (const name in parent.cacheVariables) {
					if (preset.cacheVariables[name] === undefined) {
						preset.cacheVariables[name] =
							parent.cacheVariables[name];
					}
				}
	...
	preset.environment = EnvironmentUtils.mergePreserveNull([
		inheritedEnv,
		preset.environment,
	]);
...
export async function expandConfigurePresetVariables(
	preset: ConfigurePreset,
	folder: string,
	name: string,
	workspaceFolder: string,
	...
	expansionOpts.envOverride = EnvironmentUtils.mergePreserveNull([
		env,
	...
	if (preset.__file && preset.__file.version >= 3) {
		...
		if (!preset.binaryDir) {
			...
			log.debug(
				localize(
					"binaryDir.undefined",
					"Configure preset {0}: No binaryDir specified, using default value {1}",
					preset.name,
					`"${defaultValue}"`,
				),
			);
	...
	if (expandedPreset.binaryDir) {
		expandedPreset.binaryDir = util.lightNormalizePath(
			await expandString(
	...
	if (preset.cmakeExecutable) {
		expandedPreset.cmakeExecutable = util.lightNormalizePath(
			await expandString(
				preset.cmakeExecutable,
				expansionOpts,
				errorHandler,
			),
		);
	...
	if (preset.installDir) {
		expandedPreset.installDir = util.resolvePath(
			await expandString(preset.installDir, expansionOpts),
			sourceDir,
		);
	}

	if (preset.toolchainFile) {
		expandedPreset.toolchainFile = util.lightNormalizePath(
			await expandString(
		...
		for (const cacheVarName in preset.cacheVariables) {
			...
			if (typeof cacheVar === "boolean") {
				...
				if (util.isString(cacheVar)) {
					expandedPreset.cacheVariables[cacheVarName] =
						await expandString(
							cacheVar,
							expansionOpts,
							errorHandler,
	...
	if (preset.condition) {
		expandedPreset.condition = await expandCondition(
			preset.condition,
			expansionOpts,
			errorHandler,
		);
 ...
 */
export function expandConfigurePresetForPresets(
	folder: string,
	presetType: "build" | "test" | "package" | "workflow",
): void {
	if (presetType === "build") {
...
function getConfigurePresetForPreset(
	folder: string,
	name: string,
	presetType: "build" | "test" | "package" | "workflow",
	allowUserPreset: boolean = false,
	...
	return getConfigurePresetForPresetImpl(
		folder,
		name,
		presetType,
		allowUserPreset,
...
function getConfigurePresetForPresetImpl(
	folder: string,
	name: string,
	presetType: "build" | "test" | "package" | "workflow",
	allowUserPreset: boolean = false,
...
function getConfigurePresetForPresetHelper(
	folder: string,
	preset: BuildPreset | TestPreset,
	presetType: "build" | "test" | "package" | "workflow",
	allowUserPreset: boolean = false,
): string | null {
		...
		if (refs.has(preset.name)) {
			...
			log.error(
				localize(
					...
					"Circular inherits in build preset {0}",
					preset.name,
				),
			);
			return null;
		...
		if (refs.has(preset.name)) {
			log.error(
				localize(
					...
					"Circular inherits in test preset {0}",
					preset.name,
				),
			);
			return null;
		...
		if (refs.has(preset.name)) {
			log.error(
				localize(
					...
					"Circular inherits in package preset {0}",
					preset.name,
				),
			);
			return null;
		...
		if (refs.has(preset.name)) {
			log.error(
				localize(
					...
					"Circular inherits in workflow preset {0}",
					preset.name,
				),
			);
			return null;
...
export async function getBuildPresetInherits(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	allowUserPreset: boolean = false,
	configurePreset?: string,
	usePresetsPlusIncluded: boolean = true,
	errorHandler?: ExpansionErrorHandler,
	...
	const preset = await getBuildPresetInheritsImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
		parallelJobs,
		preferredGeneratorName,
		allowUserPreset,
		configurePreset,
		usePresetsPlusIncluded,
		errorHandler,
...
async function getBuildPresetInheritsImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	log.error(
		localize(
			"build.preset.not.found.full",
			"Could not find build preset with name {0}",
			name,
		),
	);
	errorHandler?.errorList.push([
...
async function getBuildPresetInheritsHelper(
	...
	if (preset.configurePreset) {
		let expandedConfigurePreset = getPresetByName(
		...
		if (!expandedConfigurePreset) {
			log.error(
				localize(
					"configure.preset.not.found.full",
					"Could not find configure preset with name {0}",
					preset.configurePreset,
				),
			);
		...
		if (preset.inheritConfigureEnvironment !== false) {
			// Check false explicitly since defaults to true
			inheritedEnv = EnvironmentUtils.mergePreserveNull([
				inheritedEnv,
...
export async function expandBuildPresetVariables(
	preset: BuildPreset,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	if (preset.targets) {
		if (util.isString(preset.targets)) {
			expandedPreset.targets = await expandString(
				preset.targets,
				expansionOpts,
				errorHandler,
			);
...
export async function getTestPresetInherits(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await getTestPresetInheritsImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function getTestPresetInheritsImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	log.error(
		localize(
			"test.preset.not.found.full",
			"Could not find test preset with name {0}",
			name,
		),
	);
	errorHandler?.errorList.push([
...
async function getTestPresetInheritsHelper(
	folder: string,
	preset: TestPreset,
	workspaceFolder: string,
	sourceDir: string,
...
export async function expandTestPresetVariables(
	preset: TestPreset,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	if (preset.overwriteConfigurationFile) {
		...
		for (
			...
			expandedPreset.overwriteConfigurationFile[index] =
				await expandString(
					preset.overwriteConfigurationFile[index],
					expansionOpts,
					errorHandler,
				);
		}
...
export async function getPackagePresetInherits(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await getPackagePresetInheritsImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function getPackagePresetInheritsImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	log.error(
		localize(
			"package.preset.not.found.full",
			"Could not find package preset with name {0}",
			name,
		),
	);
	errorHandler?.errorList.push([
...
async function getPackagePresetInheritsHelper(
	folder: string,
...
export async function expandPackagePresetVariables(
	preset: PackagePreset,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
...
export async function getWorkflowPresetInherits(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await getWorkflowPresetInheritsImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
	...
	const expandedPreset: WorkflowPreset = {
		name,
		steps: [{ type: "configure", name: "_placeholder_" }],
	};
...
async function getWorkflowPresetInheritsImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	usePresetsPlusIncluded: boolean = true,
	errorHandler?: ExpansionErrorHandler,
	inheritedByPreset?: WorkflowPreset,
): Promise<WorkflowPreset | null> {
	let preset = getPresetByName(
			...
			steps: [
				{
					...
					name: configurePreset
						? configurePreset
						: "_placeholder_configure_preset_",
				},
			],
		};
		return getWorkflowPresetInheritsHelper(
	...
	log.error(
		localize(
			...
			"Could not find workflow preset with name {0}",
			name,
		),
	);
	errorHandler?.errorList.push([
...
async function getWorkflowPresetInheritsHelper(
	folder: string,
	preset: WorkflowPreset,
	workspaceFolder: string,
	sourceDir: string,
	allowUserPreset: boolean = false,
	enableTryApplyDevEnv: boolean = true,
	errorHandler?: ExpansionErrorHandler,
) {
	if (preset.__expanded) {
		return preset;
	...
	if (workflowConfigurePreset) {
		...
		for (const step of preset.steps) {
			switch (step.type) {
				case "build":
					const buildStepPr = getPresetByName(
...
export function configureArgs(preset: ConfigurePreset): string[] {
	...
	if (preset.trace) {
		preset.trace.mode &&
			(preset.trace.mode === TraceMode.On
				? result.push("--trace")
...
export function buildArgs(
	preset: BuildPreset,
	tempOverrideArgs?: string[],
	tempOverrideBuildToolArgs?: string[],
): string[] {
	const result: string[] = [];
...
export function testArgs(preset: TestPreset): string[] {
	...
	if (preset.output) {
		...
		preset.output.maxTestNameWidth &&
			result.push(
				"--max-width",
				preset.output.maxTestNameWidth.toString(),
			);
	}
...
export function configurePresetChangeNeedsClean(
	newPreset: ConfigurePreset,
	oldPreset: ConfigurePreset | null,
): boolean {
	if (!oldPreset) {
		// First configure preset? We never clean
		log.debug(
			localize(
				"clean.not.needed.no.prior.config.preset",
	...
	if (util.compare(new_imp, old_imp) !== util.Ordering.Equivalent) {
		log.debug(
			localize(
				"clean.needed.config.preset.changed",
...
export function getStringValueFromCacheVar(
	variable?: CacheVarType,
): string | null {
