...
export function allConfigurePresets(
	...
	return configurePresets(folder, usePresetsPlusIncluded).concat(
 ...
 */
async function tryApplyVsDevEnv(
	preset: ConfigurePreset,
	workspaceFolder: string,
	sourceDir: string,
): Promise<void> {
		...
		if (useVsDeveloperEnvironmentMode === "auto") {
			if (preset.cacheVariables) {
				...
				if (compilerName && whereExecutable) {
					...
					tempPreset.environment = EnvironmentUtils.mergePreserveNull(
						[process.env, tempPreset.environment],
					);
					const expansionOpts: ExpansionOptions =
 ...
 */
export async function expandConfigurePreset(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	allowUserPreset: boolean = false,
	enableTryApplyDevEnv: boolean = false,
	errorHandler?: ExpansionErrorHandler,
	...
	const preset = await expandConfigurePresetImpl(
		folder,
		name,
		allowUserPreset,
	...
	const expandedPreset = await expandConfigurePresetVariables(
		preset,
		folder,
		name,
		workspaceFolder,
 ...
 */
async function expandConfigurePresetImpl(
	folder: string,
	name: string,
	allowUserPreset: boolean = false,
	usePresetsPlusIncluded: boolean = false,
	...
	if (preset) {
		return expandConfigurePresetHelper(
			folder,
			preset,
			false,
			usePresetsPlusIncluded,
...
async function expandConfigurePresetHelper(
	...
	if (preset.inherits) {
		...
		for (const parentName of preset.inherits) {
			const parent = await expandConfigurePresetImpl(
				folder,
				parentName,
				allowUserPreset,
				usePresetsPlusIncluded,
...
export async function expandConfigurePresetVariables(
	...
	if (preset.binaryDir) {
		expandedPreset.binaryDir = util.lightNormalizePath(
			await expandString(preset.binaryDir, expansionOpts, errorHandler),
		);
...
export async function expandBuildPreset(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await expandBuildPresetImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function expandBuildPresetImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	if (preset) {
		return expandBuildPresetHelper(
			folder,
			preset,
			workspaceFolder,
			sourceDir,
...
async function expandBuildPresetHelper(
	folder: string,
	preset: BuildPreset,
	workspaceFolder: string,
	sourceDir: string,
	...
	if (preset.inherits) {
		...
		for (const parentName of preset.inherits) {
			const parent = await expandBuildPresetImpl(
				folder,
				parentName,
				workspaceFolder,
				sourceDir,
	...
	if (preset.configurePreset) {
		const configurePreset = await expandConfigurePreset(
			folder,
			preset.configurePreset,
			workspaceFolder,
			sourceDir,
...
export async function expandTestPreset(
	...
	const preset = await expandTestPresetImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function expandTestPresetImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
...
async function expandTestPresetHelper(
	...
	if (preset.inherits) {
		...
		for (const parentName of preset.inherits) {
			const parent = await expandTestPresetImpl(
				folder,
				parentName,
				workspaceFolder,
				sourceDir,
...
export async function expandPackagePreset(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await expandPackagePresetImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function expandPackagePresetImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	if (preset) {
		return expandPackagePresetHelper(
			folder,
			preset,
			workspaceFolder,
			sourceDir,
...
async function expandPackagePresetHelper(
	folder: string,
	...
	if (preset.inherits) {
		...
		for (const parentName of preset.inherits) {
			const parent = await expandPackagePresetImpl(
				folder,
				parentName,
				workspaceFolder,
				sourceDir,
...
export async function expandWorkflowPreset(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
	...
	const preset = await expandWorkflowPresetImpl(
		folder,
		name,
		workspaceFolder,
		sourceDir,
...
async function expandWorkflowPresetImpl(
	folder: string,
	name: string,
	workspaceFolder: string,
	sourceDir: string,
...
async function expandWorkflowPresetHelper(
	folder: string,
